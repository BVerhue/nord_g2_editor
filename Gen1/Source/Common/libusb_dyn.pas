...
var
  libusb_init : function (ctx:PPlibusb_context):longint;cdecl = nil;
  libusb_exit : procedure (ctx:Plibusb_context);cdecl = nil;

  libusb_set_debug : procedure (ctx:Plibusb_context; level:integer);cdecl = nil;

  libusb_get_device_list : function (ctx:Plibusb_context; list: PPPlibusb_device): size_t;cdecl = nil;
  libusb_free_device_list : procedure (list:PPlibusb_device; unref_devices:integer);cdecl = nil;
  libusb_get_configuration : function (dev: Plibusb_device_handle; config: PLongint): Integer;cdecl = nil;

  libusb_get_device_descriptor : function (dev: Plibusb_device; desc:Plibusb_device_descriptor): Integer;cdecl = nil;
  libusb_get_active_config_descriptor : function (dev: Plibusb_device; config:PPlibusb_config_descriptor): Integer;cdecl = nil;

  libusb_get_config_descriptor : function (dev: Plibusb_device;config_index:uint8_t; config: PPlibusb_config_descriptor):Integer;cdecl = nil;

  libusb_get_max_packet_size : function (dev: Plibusb_device; endpoint: Byte): Integer;cdecl = nil;
  libusb_get_max_iso_packet_size : function (dev: Plibusb_device; endpoint: Byte): Integer;cdecl = nil;

  libusb_open : function (dev: Plibusb_device; handle: PPlibusb_device_handle): Integer;cdecl = nil;
  libusb_close : procedure (dev_handle:Plibusb_device_handle);cdecl = nil;
  libusb_get_device : function (dev_handle: Plibusb_device_handle): libusb_device;cdecl = nil;

  libusb_set_configuration : function (dev: Plibusb_device_handle; configuration: Integer): Integer;cdecl = nil;

  libusb_claim_interface : function (dev: Plibusb_device_handle; iface: Integer): Integer;cdecl = nil;
  libusb_release_interface : function (dev: Plibusb_device_handle; iface: Integer): Integer;cdecl = nil;

  libusb_open_device_with_vid_pid : function (ctx: Plibusb_context; vendor_id: uint16_t; product_id: uint16_t): Plibusb_device_handle;cdecl = nil;

  libusb_set_interface_alt_setting : function (dev: Plibusb_device_handle;interface_number: Integer;alternate_setting: Integer): Integer;cdecl = nil;
  libusb_clear_halt : function (dev: Plibusb_device_handle; endpoint: BYTE): Integer;cdecl = nil;
  libusb_reset_device : function (dev: Plibusb_device_handle): Integer;cdecl = nil;

  libusb_kernel_driver_active : function (dev: Plibusb_device_handle; theinterface: Integer): Integer;cdecl = nil;
  libusb_detach_kernel_driver : function (dev: Plibusb_device_handle; theinterface: Integer): Integer;cdecl = nil;
  libusb_attach_kernel_driver : function (dev: Plibusb_device_handle; theinterface: Integer): Integer;cdecl = nil;
//------------------------------------------------------------------------------
// Async I/O
//------------------------------------------------------------------------------
  libusb_submit_transfer : function (transfer: Pstructlibusb_transfer):Integer;cdecl = nil;
  libusb_cancel_transfer : function (transfer: Pstructlibusb_transfer):Integer;cdecl = nil;
  libusb_free_transfer : procedure (transfer: Pstructlibusb_transfer);cdecl = nil;
  libusb_alloc_transfer : function (iso_packets:integer):Pstructlibusb_transfer;cdecl = nil;
//------------------------------------------------------------------------------
// Sync I/O
//------------------------------------------------------------------------------
  libusb_control_transfer : function (dev_handle: Plibusb_device_handle; request_type: uint8_t; request: uint8_t; value: uint16_t; index: uint16_t; data: PChar; length:uint16_t;timeout: DWORD):Integer;cdecl = nil;
  libusb_bulk_transfer : function (dev_handle: Plibusb_device_handle; endpoint:BYTE; data: Pchar; length:integer;actual_length:Plongint; timeout:DWORD):Integer;cdecl = nil; //tested
  libusb_interrupt_transfer : function (dev_handle: Plibusb_device_handle; endpoint:byte; data:Pchar; length:integer; actual_length:Plongint; timeout:DWORD):Integer;cdecl = nil;    //tested

  libusb_get_string_descriptor_ascii : function (dev:plibusb_device_handle ;index:uint8_t; data:Pchar; length:integer):Integer;cdecl = nil;

{/* polling and timeouts */}
  libusb_try_lock_events : function (ctx:Plibusb_context):integer;cdecl = nil;
  libusb_lock_events : procedure (ctx:Plibusb_context);cdecl = nil;
  libusb_unlock_events : procedure (ctx:Plibusb_context);cdecl = nil;
  libusb_event_handling_ok : function (ctx:Plibusb_context):integer;cdecl = nil;
  libusb_event_handler_active : function (ctx:Plibusb_context):integer;cdecl = nil;
  libusb_lock_event_waiters : procedure (ctx:Plibusb_context); cdecl = nil;
  libusb_unlock_event_waiters : procedure (ctx:Plibusb_context); cdecl = nil;
...
begin
  ...
  begin
    ...
    //libusb_get_string := GetProcAddress( LibUSBHandle, '_usb_get_string');
    //libusb_get_string_simple := GetProcAddress( LibUSBHandle, '_usb_get_string_simple');
    ...
    libusb_free_device_list := GetProcAddress( LibUSBHandle, 'libusb_free_device_list');
    libusb_kernel_driver_active := GetProcAddress( LibUSBHandle, 'libusb_kernel_driver_active');
    libusb_detach_kernel_driver := GetProcAddress( LibUSBHandle, 'libusb_detach_kernel_driver');

    //libusb_get_descriptor_by_endpoint := GetProcAddress( LibUSBHandle, '_usb_get_descriptor_by_endpoint');
    //libusb_get_descriptor := GetProcAddress( LibUSBHandle, '_usb_get_descriptor');

    //libusb_bulk_write := GetProcAddress( LibUSBHandle, '_usb_bulk_write');
    //libusb_bulk_read := GetProcAddress( LibUSBHandle, '_usb_bulk_read');
    libusb_control_transfer := GetProcAddress( LibUSBHandle, 'libusb_control_transfer');
    libusb_bulk_transfer := GetProcAddress( LibUSBHandle, 'libusb_bulk_transfer');
    libusb_interrupt_transfer := GetProcAddress( LibUSBHandle, 'libusb_interrupt_transfer');

    //libusb_interrupt_write := GetProcAddress( LibUSBHandle, '_usb_interrupt_write');
    //libusb_interrupt_read := GetProcAddress( LibUSBHandle, '_usb_interrupt_read');
    //libusb_control_msg := GetProcAddress( LibUSBHandle, '_usb_control_msg');
    libusb_set_configuration := GetProcAddress( LibUSBHandle, 'libusb_set_configuration');
    libusb_claim_interface := GetProcAddress( LibUSBHandle, 'libusb_claim_interface');// was interface, a pascal reserved word
    libusb_release_interface := GetProcAddress( LibUSBHandle, 'libusb_release_interface');
    //libusb_set_altinterface := GetProcAddress( LibUSBHandle, '_usb_set_altinterface');
    //libusb_resetep := GetProcAddress( LibUSBHandle, '_usb_resetep');
    libusb_clear_halt := GetProcAddress( LibUSBHandle, 'libusb_clear_halt');
    //libusb_reset := GetProcAddress( LibUSBHandle, '_usb_reset');

    //libusb_strerror := GetProcAddress( LibUSBHandle, '_usb_strerror');

    libusb_init := GetProcAddress( LibUSBHandle, 'libusb_init');
    libusb_exit := GetProcAddress( LibUSBHandle, 'libusb_exit');

    libusb_set_debug := GetProcAddress( LibUSBHandle, 'libusb_set_debug');
    //libusb_find_busses := GetProcAddress( LibUSBHandle, '_usb_find_busses');
    //libusb_find_devices := GetProcAddress( LibUSBHandle, '_usb_find_devices');
    libusb_get_device := GetProcAddress( LibUSBHandle, 'libusb_get_device'); // renamed from usb_device because of same named record
    //libusb_get_busses := GetProcAddress( LibUSBHandle, '_usb_get_busses');

    //libusb_install_service_np := GetProcAddress( LibUSBHandle, '_usb_install_service_np');
    //libusb_uninstall_service_np := GetProcAddress( LibUSBHandle, '_usb_uninstall_service_np');
    //libusb_install_driver_np := GetProcAddress( LibUSBHandle, '_usb_install_driver_np');
    //libusb_get_version := GetProcAddress( LibUSBHandle, '_usb_get_version');
    //libusb_isochronous_setup_async := GetProcAddress( LibUSBHandle, '_usb_isochronous_setup_async');
    //libusb_bulk_setup_async := GetProcAddress( LibUSBHandle, '_usb_bulk_setup_async');
    //libusb_interrupt_setup_async := GetProcAddress( LibUSBHandle, '_usb_interrupt_setup_async');
    //libusb_submit_async := GetProcAddress( LibUSBHandle, '_usb_submit_async');
    //libusb_reap_async := GetProcAddress( LibUSBHandle, '_usb_reap_async');
    //libusb_reap_async_nocancel := GetProcAddress( LibUSBHandle, '_usb_reap_async_nocancel');
    //libusb_cancel_async := GetProcAddress( LibUSBHandle, '_usb_cancel_async');
    //libusb_free_async := GetProcAddress( LibUSBHandle, '_usb_free_async');
  end
end;
