...
function CompareModulePosition( Module1 : pointer; Module2 : pointer): integer;
begin
  if TG2FileModule(Module1).Row > TG2FileModule(Module2).Row then
    Result := -1
  else
    if TG2FileModule(Module1).Row = TG2FileModule(Module2).Row then begin
      if TG2FileModule(Module1).Col > TG2FileModule(Module2).Col then
        Result := -1
      else
        if TG2FileModule(Module1).Col = TG2FileModule(Module2).Col then
          Result := 0
...
var SrceModuleList, DestModuleList : TList;
    MorphValue, MorphValueSign : byte;
    sl : TStringList;
    i, m, p, r : integer;
    MemStream, UndoMemStream : TG2Message;
    Chunk, UndoChunk : TPatchChunk;
    SrceParamValues, DestParamValues : TParams;
    SrceVariationList, DestVariationList : TParamSet;
    SrceParamList, DestParamList : TModuleParameters;
begin
  ...
  try
    SrceModuleList := TList.Create;
    DestModuleList := TList.Create;
    sl := TStringList.Create;
    SrceParamList := TModuleParameters.Create(True);
    DestParamList := TModuleParameters.Create(True);
    Chunk := TPatchChunk.Create( MemStream);
    ...
    try
      ...
      for i := 0 to sl.Count - 1 do begin
        ...
        SrceModuleList.Sort(@CompareModulePosition);
        DestModuleList.Sort(@CompareModulePosition);

        // Copy Morphs
        for m := 0 to SrceModuleList.Count - 1 do begin

          SrceVariationList := TParamSet.Create(True, SrceParamList);
          SrceVariationList.ModuleIndex := TG2FileModule(DestModuleList[m]).ModuleIndex;
          SrceVariationList.ParameterCount := TG2FileModule(DestModuleList[m]).ParameterCount;
          SrceParamList.AddParamSet( SrceVariationList);

          SrceParamValues := TParams.Create( SrceVariationList);
          SrceVariationList.AddVariation( SrceParamValues);
          SrceParamValues.Variation := aToVariation;
          SrceParamValues.ParamCount := TG2FileModule(DestModuleList[m]).ParameterCount;

          DestVariationList := TParamSet.Create(True, DestParamList);
          DestVariationList.ModuleIndex := TG2FileModule(DestModuleList[m]).ModuleIndex;
          DestVariationList.ParameterCount := TG2FileModule(DestModuleList[m]).ParameterCount;
          DestParamList.AddParamSet( DestVariationList);

          DestParamValues := TParams.Create( SrceVariationList);
          DestVariationList.AddVariation( DestParamValues);
          DestParamValues.Variation := aToVariation;
          DestParamValues.ParamCount := TG2FileModule(DestModuleList[m]).ParameterCount;

          for p := 0 to TG2FileModule(DestModuleList[m]).ParameterCount - 1 do begin
            SrceParamValues.ParamValues[p] := aSrcePatch.FindParamValue( TG2FileModule(SrceModuleList[m]).ModuleIndex, aFromVariation, p);
            DestParamValues.ParamValues[p] := GetParameterValue( aToLocation, TG2FileModule(DestModuleList[m]).ModuleIndex, p, aToVariation);

            for r := 0 to 7 do begin

              MorphValue := aSrcePatch.GetMorphValue(TG2FileModule(SrceModuleList[m]).ModuleIndex, p, r, aFromVariation);
              ...
              if MorphValue <> 0 then begin
                AddSetMorphMessage( SendMessage,
                                    ord(aToLocation),
                                    TG2FileModule(DestModuleList[m]).ModuleIndex,
                                    p,
                                    r,
                                    MorphValue,
                                    MorphValueSign,
                ...
                MorphValue := GetMorphValue(aToLocation, TG2FileModule(DestModuleList[m]).ModuleIndex, p, r, aToVariation);
                if MorphValue >= 128 then begin
                  MorphValueSign := 1;
                  MorphValue := abs(MorphValue - 256);
                end else
      ...
      Chunk.WriteBits( ord(aToLocation), 2);
      SrceParamList.Write(Chunk, 1); // TODO : Variationcount wordt overschreven!!!
...
function TG2MessPatch.CreateCopyParametersMessage(aSrcePatch: TG2FilePatchPart;
  aToLocation: TLocationType; aFromVariation,
  aToVariation: integer): TG2SendMessage;
begin
  Result := CreatePatchMessage;
