...

{
 ...
 Notes:
  ...
  BVerhue 3-9-2012: added method to check if a midi message exists in the input buffer
}
...
type
  ...
  // BVerhue
  TTMidiBufferItem = array[0..1024-1] of TMidiBufferItem;
  PTMidiBufferItem = ^TTMidiBufferItem;
  ...
  public
    ...
    function GetMidiMessageInBufferCount( aMidiEvent : TMyMidiEvent): integer;
...
function TMidiInput.GetMidiMessageInBufferCount(
  aMidiEvent: TMyMidiEvent): integer;
var
	PItem: PMidiBufferItem;
begin
  Result := 0;
	if (FState = misOpen) then begin

    // Check from NextGet to NextPut
    PItem := Pbuffer^.PNextGet;
    while PItem <> PBuffer^.pNextPut do begin
			if (PItem.Sysex = Nil) then begin
				{ Short message }
        if (aMidiEvent.MidiMessage = LoByte(LoWord(PItem.Data)))
          and (aMidiEvent.Data1 = HiByte(LoWord(PItem.Data))) then
          inc(Result);
      end else begin
        // Todo Sysex!
      end;
  		Inc( PItem );
	  	If (PItem = PBuffer^.PEnd) then
		  	PItem := PBuffer^.PStart;
    end;
  end;
end;
