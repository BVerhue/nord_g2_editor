...
type
  ...
  public
    constructor Create(aPatch: IG2Patch; const aLocationIndex : integer); reintroduce; virtual;
    ...
    procedure   ReadModuleList( aModuleList : TList<IG2Module>; aChunk : TPatchChunk);
    ...
    procedure   ReadParamLabelList( aModuleList : TList<IG2Module>; aChunk : TPatchChunk);
    procedure   ReadModuleLabelList( aModuleList : TList<IG2Module>; aChunk : TPatchChunk);
    ...
    procedure   WriteParamList( aModuleList : TList<IG2Module>; aVariationCount : byte; aChunk : TPatchChunk);
    procedure   WriteParamLabelList( aModuleList : TList<IG2Module>; aChunk : TPatchChunk);
    procedure   WriteModuleLabelList( aModuleList : TList<IG2Module>; aChunk : TPatchChunk);
    ...
    procedure   AddMessAddPatchPart( aSrcePatchPart : IG2PatchPart; aSendMessage, aUndoMessage : TG2SendMessage);
    ...
    procedure   AddMessUprateChanges( aModuleUprateList : TList<IG2Module>;  aSendMessage, aUndoMessage : TG2SendMessage);
    ...
    function    CreateLed( const aLedType : TLedType; const aModuleIndex, aGroupID : byte; const aGroupCount : integer): IG2Led; virtual;
    ...
    function(const Module1, Module2: IG2Module): integer
      begin
        if Module1.Row > Module2.Row then
          ...
          if Module1.Row = Module2.Row then begin
    ...
    function(const Module1, Module2: IG2Module): integer
      begin
        if Module1.TypeID > Module2.TypeID then
          ...
          if Module1.TypeID = Module2.TypeID then begin
            if Module1.Row > Module2.Row then
              Result := -1
            else
              if Module1.Row = Module2.Row then begin
...
var Module : IG2Module;
begin
  if aModuleList.Count = 0 then begin
    Result := Rect(0,0,0,0);
  end else begin
    Result := Rect(10000, 10000, -10000, -10000);
    for Module in aModuleList do begin
...
function ModuleListModuleAtLocation( aModuleList : TList<IG2Module>; const aRow, aCol: integer): IG2Module;
var Module : IG2Module;
begin
  Result := nil;

  for Module in aModuleList do begin
    if (Module.Col = aCol) and (aRow >= Module.Row) and (aRow < (Module.Row + Module.HeightUnits)) then begin
      Result := Module;
      exit;
    end;
  end;
...
function ModuleListFindModule( aModuleList : TList<IG2Module>; const aModuleIndex : byte) : IG2Module;
var Module : IG2Module;
begin
  Result := nil;

  for Module in aModuleList do begin
    if (Module.ModuleIndex = aModuleIndex) then begin
      Result := Module;
      exit;
    end;
  end;
...
function ModuleListModuleAbove( aModuleList : TList<IG2Module>; const aModuleIndex : byte) : IG2Module;
var Module, NextModule : IG2Module;
    dx, dy, ds : single;
begin
  ...
  ds := 0;
  for NextModule in aModuleList do begin
    if NextModule.ParameterCount > 0 then begin
      if NextModule.Row < Module.Row then begin
...
function ModuleListModuleUnder( aModuleList : TList<IG2Module>; const aModuleIndex : byte) : IG2Module;
var Module, NextModule : IG2Module;
    dx, dy, ds : single;
begin
  Module := ModuleListFindModule( aModuleList, aModuleIndex);
...
function ModuleListModuleLeft( aModuleList : TList<IG2Module>; const aModuleIndex : byte) : IG2Module;
var Module, NextModule : IG2Module;
    dx, dy, ds : single;
begin
  Module := ModuleListFindModule( aModuleList, aModuleIndex);
  ...
  for NextModule in aModuleList do begin
    if NextModule.ParameterCount > 0 then begin
      if NextModule.Col < Module.Col then begin
        dx := NextModule.Col * UNITS_COL - Module.Col * UNITS_COL;
        dy := NextModule.Row * UNITS_ROW - Module.Row * UNITS_ROW;
        if (ds = 0) or (sqrt(dx*dx+dy*dy) < ds) then begin
          Result := NextModule;
...
function ModuleListModuleRight( aModuleList : TList<IG2Module>; const aModuleIndex : byte) : IG2Module;
var Module, NextModule : IG2Module;
    dx, dy, ds : single;
begin
  Module := ModuleListFindModule( aModuleList, aModuleIndex);
...
function ModuleListNoOffModuleType( aModuleList : TList<IG2Module>; const aModuleType : byte) : integer;
var Module : IG2Module;
begin
  Result := 0;
  for Module in aModuleList do
...
begin
  ...
  if aModuleList.Count > 0 then begin
    ...
    i := 0;
    fit := false;
    while ( i < aModuleList.Count) and not fit do begin

      if aModule.NewCol < aModuleList[i].NewCol then
        inc(i)
      else
        if aModule.NewCol = aModuleList[i].NewCol then begin
          ...
          if overlap > 0 then begin
            ...
            if (c < a) and (aModule.HeightUnits <= hole) then begin
...
function CableListFindCable( aCableList : TList<IG2Cable>;
  const aModuleIndexFrom, aConnIndexFrom, aModuleIndexTo, aConnIndexTo : byte): IG2Cable;
var Cable : IG2Cable;
begin
  Result := nil;

  for Cable in aCableList do begin
    if (Cable.ModuleIndexFrom = aModuleIndexFrom)
       and (Cable.ConnIndexFrom = aConnIndexFrom)
       and (Cable.ModuleIndexTo = aModuleIndexTo)
       and (Cable.ConnIndexTo = aConnIndexTo) then begin
      Result := Cable;
      exit;
    end;
  end;
...
begin
  ...
  Chunk := TPatchChunk.Create( MemStream);
  try
    WriteChunkSelectedModules(SelectedModuleList, C_MODULE_LIST, VariationCount, Chunk);
    WriteChunkSelectedModules(SelectedModuleList, C_CABLE_LIST, VariationCount, Chunk);
    WriteChunkSelectedModules(SelectedModuleList, C_PARAM_LIST, VariationCount, Chunk);
    WriteChunkSelectedModules(SelectedModuleList, C_PARAM_NAMES, VariationCount, Chunk);
    WriteChunkSelectedModules(SelectedModuleList, C_MODULE_NAMES, VariationCount, Chunk);
...
begin
  Result := TG2FileLed.Create( aLedType, FLocationIndex, aModuleIndex, aGroupID, aGroupCount);
end;
  ...
  for i := 0 to High(LedDefs) do begin
    if LedDefs[i].ModuleID = aModuleType then begin
      ...
      if GroupCount = 1 then begin
        Led := CreateLed(LedDefs[i].LedType, aModuleIndex, LedDefs[i].GroupID, GroupCount);
        FLedList.Add(Led);
      end else begin
        j := 0;
        while (j < FLedStripList.Count) and not((FLedStripList[j].LocationIndex = FLocationIndex)
...
begin
  ...
  for Module in FModuleList do begin
...
begin
  ...
  if FLocationIndex = LOCATION_PATCH then begin
    // Init patch parameters
    InitParams;
    // Init morph labels
    Module := FindModule(PATCH_MORPH);
    if not assigned(Module) then
      raise Exception.Create('TG2FilePatchPart.Init : Module ' + IntToStr(PATCH_MORPH) + ' not found.');
...
procedure TG2PatchPart.NotifyObservers(aG2Event: TG2Event; aG2Object: IG2Object);
begin
  if assigned(Slot) then
    Slot.NotifyObservers(aG2Event, aG2Object);
end;

function TG2PatchPart.ProcessResponseMsg( aSender : TObject; aMsg : TG2Message): boolean;
begin
  Result := True;
end;

procedure TG2PatchPart.ProcessMsgModuleParamLabel( aMsg: TG2Message);
var ModuleIndex, Length : byte;
    Module : IG2Module;
    Chunk : TPatchChunk;
begin
  ...
  try
    ...
    if not assigned(Module) then
      raise Exception.Create('ProcessMsgModuleParamLabel : Module index ' + IntToStr(ModuleIndex) + ' not found.');
...
procedure TG2PatchPart.InitParams;
  ...
  procedure AddParam(ID: integer; ParamType : TParamType; ModuleIndex : byte; ModuleName : string;
...
begin
  AddParam(182, ptParam, PATCH_VOLUME,      'Vol',      VOLUME_LEVEL,  'VolLevel',   '', 0, 127, 100, '', 515);
  AddParam(183, ptParam, PATCH_VOLUME,      'Vol',      VOLUME_MUTE,   'VolMute',    '', 0,   1,   0, 'Off;On', 514);
  AddParam(184, ptParam, PATCH_GLIDE,       'Glide',    GLIDE_TYPE,    'GlideType',  '', 0,   2,   0, 'Auto;Normal;Off', 512);
  AddParam(185, ptParam, PATCH_GLIDE,       'Glide',    GLIDE_SPEED,   'GlideSpeed', '', 0, 127,   0, '', 511);
  AddParam(186, ptParam, PATCH_BEND,        'Bend',     BEND_ON_OFF,   'BendOnOff',  '', 0,   1,   0, 'Off;On', 514);
  AddParam(187, ptParam, PATCH_BEND,        'Bend',     BEND_RANGE,    'BendRange',  '', 0,  23,   0, '', 513);
  AddParam(188, ptParam, PATCH_VIBRATO,     'Vibrato',  VIBRATO_MOD,   'VibrMod',    '', 0,   2,   0, 'Wheel;AfTouch;Off', 510);
  AddParam(189, ptParam, PATCH_VIBRATO,     'Vibrato',  VIBRATO_DEPTH, 'VibrDepth',  '', 0, 127,   0, '', 509);
  AddParam(190, ptParam, PATCH_VIBRATO,     'Vibrato',  VIBRATO_RATE,  'VibrRate',   '', 0, 127,   0, '', 0);
  AddParam(191, ptParam, PATCH_ARPEGGIATOR, 'Arp',      ARP_ON_OFF,    'ArpOnOff',   '', 0,   1,   0, 'Off;On', 506);
  AddParam(192, ptParam, PATCH_ARPEGGIATOR, 'Arp',      ARP_SPEED,     'ArpSpeed',   '', 0,   3,   0, '1/8;1/8T;1/16;1/16T', 505);
  AddParam(193, ptParam, PATCH_ARPEGGIATOR, 'Arp',      ARP_DIRECTION, 'ArpDir',     '', 0,   3,   0, 'Up;Dn;UpDn;Rnd', 507);
  AddParam(194, ptParam, PATCH_ARPEGGIATOR, 'Arp',      ARP_OCTAVES,   'ArpOct',     '', 0,   3,   0, '1;2;3;4', 508);
  AddParam(195, ptParam, PATCH_SUSTAIN,     'Oct Shft', OCTAVE_SHIFT,  'Oct.Shft',   '', 0,   0,   0, '-2;-1;0;1;2', 518);
  AddParam(196, ptParam, PATCH_SUSTAIN,     'Sustain',  SUSTAIN_PEDAL, 'Sustain',    '', 0,   1,   0, 'Off;On', 517);

  AddParam(197, ptParam, PATCH_MORPH,       'Morph',    0,             'Wheel', 'Wheel',     0,   127,   0, '', 179);
  AddParam(197, ptParam, PATCH_MORPH,       'Morph',    1,             'Vel', 'Vel',         0,   127,   0, '', 179);
  AddParam(197, ptParam, PATCH_MORPH,       'Morph',    2,             'Keyb', 'Keyb',       0,   127,   0, '', 179);
  AddParam(197, ptParam, PATCH_MORPH,       'Morph',    3,             'Aft.Tch', 'Aft.Tch', 0,   127,   0, '', 179);
  AddParam(197, ptParam, PATCH_MORPH,       'Morph',    4,             'Sust.Pd', 'Sust.Pd', 0,   127,   0, '', 179);
  AddParam(197, ptParam, PATCH_MORPH,       'Morph',    5,             'Ctrl.Pd', 'Ctrl.Pd', 0,   127,   0, '', 179);
  AddParam(197, ptParam, PATCH_MORPH,       'Morph',    6,             'P.Stick', 'P.Stick', 0,   127,   0, '', 179);
  AddParam(197, ptParam, PATCH_MORPH,       'Morph',    7,             'G.Wh2', 'G.Wh2',     0,   127,   0, '', 179);

  AddParam(198, ptParam, PATCH_MORPH,       'Morph',    8,             'Wheel',      '', 0,   1,   0, 'Knob;Wheel', 519);
  AddParam(199, ptParam, PATCH_MORPH,       'Morph',    9,             'Vel',        '', 0,   1,   0, 'Knob;Vel', 520);
  AddParam(200, ptParam, PATCH_MORPH,       'Morph',    10,            'Keyb',       '', 0,   1,   0, 'Knob;Keyb', 521);
  AddParam(201, ptParam, PATCH_MORPH,       'Morph',    11,            'Aft.Tch',    '', 0,   1,   0, 'Knob;Aft.Tch', 522);
  AddParam(202, ptParam, PATCH_MORPH,       'Morph',    12,            'Sust.Pd',    '', 0,   2,   0, 'Knob;Sust.Pd;G Wh 1', 523);
  AddParam(203, ptParam, PATCH_MORPH,       'Morph',    13,            'Ctrl.Pd',    '', 0,   1,   0, 'Knob;Ctrl.Pd', 524);
  AddParam(204, ptParam, PATCH_MORPH,       'Morph',    14,            'P.Stick',    '', 0,   1,   0, 'Knob;P.Stick', 525);
  AddParam(205, ptParam, PATCH_MORPH,       'Morph',    15,            'G.Wh2',      '', 0,   1,   0, 'Knob;G.Wh2', 526);

  // "Virtual" paramaters for use in parameter pages
  AddParam(206, ptMasterClock, PATCH_MASTERCLOCK, 'Mast Clk',  0,      'M.Clk',    '', 30,  240,  0, '', 501);
  AddParam(207, ptMasterClock, PATCH_MASTERCLOCK, 'Clk Run',   1,      'Clk.Rn',   '', 0,   1,    0, 'Off;On', 502);

  AddParam(208, ptVoiceMode, PATCH_VOICES,      'Voice Cnt',  0,       'Voices',     '', 0,   31,   0, '', 503);
  AddParam(209, ptVoiceMode, PATCH_VOICES,      'Voice Mod',  1,       'Vce.Mde',    '', 0,   2,    0, 'Poly;Mono;Legato', 504);
end;
...
var SrceModuleList, DestModuleList, SelectedModuleList : TList<IG2Module>;
    sl : TStringList;
    i, m : integer;
begin
  Result := False;
  ...
  try
    ...
    for i := 0 to sl.Count - 1 do begin
      ...
      for m := 0 to SelectedModuleList.Count - 1 do begin
...
function TG2PatchPart.ModuleAtLocation(const aRow,
  aCol: integer): IG2Module;
begin
  Result := nil;
...
begin
  ...
  for i := 0 to NumCables - 1 do begin
    ...
    if assigned( aChunk.Log) then
      aChunk.Log.add_log_line( Format('%3d %3d %3d %3d %3d %3d', [Color, ModuleIndexFrom, ConnIndexFrom, LinkType, ModuleIndexTo, ConnIndexTo]), LOGCMD_NUL);

    Cable := CableListFindCable( aCableList, ModuleIndexFrom, ConnIndexFrom, ModuleIndexTo, ConnIndexTo);
    if assigned(Cable) then begin
      ...
      if assigned(ModuleFrom) and assigned(ModuleTo) then begin
        if LinkType = 0 then
        ...
        if assigned(ConnFrom) and assigned(ConnTo) then begin
...
procedure TG2PatchPart.ReadModuleLabelList(aModuleList: TList<IG2Module>;
  aChunk: TPatchChunk);
var NumNames, i, ModuleIndex : integer;
    Module : IG2Module;
begin
  // FLocation    : TBits2;
  // FUnknown     : TBits6;
  ...
  if assigned(aChunk.Log) then begin
    aChunk.Log.add_log_line('Module labels, count = ' + IntToStr(NumNames), LOGCMD_NUL);
  ...
  for i := 0 to NumNames - 1 do begin
    ...
    begin
      ...
      if assigned( aChunk.Log) then
        aChunk.Log.add_log_line( Format('%3d', [ModuleIndex]) + ' ' + Module.ModuleName, LOGCMD_NUL);
    end else
      if assigned( aChunk.Log) then
        aChunk.Log.add_log_line( Format('%3d', [ModuleIndex]) + ' not found.', LOGCMD_NUL);
...
begin
  ...
  if assigned(aChunk.Log) then begin
    aChunk.Log.add_log_line('Module list, count = ' + IntToStr(ModuleCount), LOGCMD_NUL);
...
procedure TG2PatchPart.ReadParamLabelList(aModuleList: TList<IG2Module>;
  aChunk: TPatchChunk);
var i, ModuleCount, ModuleIndex : integer;
    Module : IG2Module;
begin
  // FLocation       : Tbits2;
  // FModuleCount    : TBits8
  ...
  if assigned(aChunk.Log) then begin
    aChunk.Log.add_log_line('Param labels, count = ' + IntToStr(ModuleCount), LOGCMD_NUL);
...
begin
  ...
  if assigned( aChunk.Log) then begin
    aChunk.Log.add_log_line('Num sets = ' + IntToStr(ModuleCount) + ', var cnt = ' + IntToStr(VariationCount), LOGCMD_NUL);
...
procedure TG2PatchPart.WriteChunk( aID : byte; aChunk : TPatchChunk; aVariationCount : byte);
begin
  case aID of
  ...
  C_CABLE_LIST : // $52 Cable list
        begin
          ...
          try
            ...
            for i := 0 to InnerCableList.Count - 1 do begin
...
begin
  ...
  try
    ...
    for Module in TempList do begin
...
begin
  ...
  for i := 0 to aModule.ModeCount - 1 do begin
...
procedure TG2PatchPart.WriteMessDeleteModule( const aModuleIndex : byte; aStream : TStream);
begin
...
procedure TG2PatchPart.AddMessAddCable( const aFromModuleIndex : byte;
      ...
      aToConnectorKind : TConnectorKind; aToConnectorIndex : byte;
      aToConnectorColor : byte; aSendMessage, aUndoMessage : TG2SendMessage);
var FromModule, ToModule : IG2Module;
...
begin
  ...
  try
    ...
    if aToConnectorKind = ckOutput then begin
      ...
      aUndoMessage.AddMessCableDelete(FLocationIndex, aToModuleIndex, aToConnectorIndex,
        ord(aToConnectorKind), aFromModuleIndex, aFromConnectorIndex, ord(aFromConnectorKind));

      aSendMessage.AddMessCableAdd(FLocationIndex, aToModuleIndex, aToConnectorIndex,
        ord(aToConnectorKind), aFromModuleIndex, aFromConnectorIndex, ord(aFromConnectorKind),
        aToConnectorColor);
    end else begin
      ...
      aUndoMessage.AddMessCableDelete(FLocationIndex, aFromModuleIndex, aFromConnectorIndex,
        ord(aFromConnectorKind), aToModuleIndex, aToConnectorIndex, ord(aToConnectorKind));

      aSendMessage.AddMessCableAdd(FLocationIndex, aFromModuleIndex, aFromConnectorIndex,
        ord(aFromConnectorKind), aToModuleIndex, aToConnectorIndex, ord(aToConnectorKind),
        aFromConnectorColor);
    end;
...
procedure TG2PatchPart.AddMessAddModule( const aModuleTypeID, aCol,
  aRow: byte; aSendMessage, aUndoMessage: TG2SendMessage);
var ModuleChangeList : TModuleChangeList;
...
begin
  ...
  try
    ...
    ModuleName := ModuleDefs[mi].ModuleName + IntToStr(GetUniqueModuleNameSeqNr( ModuleDefs[mi].ModuleName));
  ...
  TG2FileModule.WriteMessNewModule(
    FLocationIndex,
    ModuleIndex,
    aModuleTypeID,
    aRow,
...
begin
  ...
  try
    ...
    for Module in SelectedModuleList do begin
    ...
    try
      ...
      for Cable in aSrcePatchPart.CableList do begin
        if Cable.ConnFrom.Module.Selected and
           Cable.ConnTo.Module.Selected then begin
          // Inner cable
      ...
      for Cable in aSrcePatchPart.CableList do begin
        if (not Cable.ConnFrom.Module.Selected) and
           Cable.ConnTo.Module.Selected then begin

          aSrcePatchPart.CreateModuleUprateList(Cable.ConnTo, Cable.ConnTo.DefRate, ModuleUprateList);
        end else
          if Cable.ConnFrom.Module.Selected and
             (not Cable.ConnTo.Module.Selected) then begin

            aSrcePatchPart.CreateModuleUprateList( Cable.ConnFrom, Cable.ConnFrom.DefRate, ModuleUprateList);
          end;
      end;
    ...
    aSrcePatchPart.WriteChunkSelectedModules(SelectedModuleList, C_CABLE_LIST, GetVariationCount, Chunk);
    Chunk.WriteBits(FLocationIndex, 2);
    aSrcePatchPart.WriteChunkSelectedModules(SelectedModuleList, C_PARAM_LIST, GetVariationCount, Chunk);
    Chunk.WriteBits(FLocationIndex, 2);
    aSrcePatchPart.WriteChunkSelectedModules(SelectedModuleList, C_PARAM_NAMES, GetVariationCount, Chunk);
    Chunk.WriteBits(FLocationIndex, 2);
    Chunk.WriteBits( 0, 6);
    aSrcePatchPart.WriteChunkSelectedModules(SelectedModuleList, C_MODULE_NAMES, GetVariationCount, Chunk);
...
procedure TG2PatchPart.AddMessDeleteCable( aCable : IG2Cable;
  aSendMessage, aUndoMessage: TG2SendMessage);
var ModuleUprateList : TList<IG2Module>;
begin
  ModuleUprateList := TList<IG2Module>.Create;
  try
    ResetModuleUprates;
    ...
    CreateModuleUprateList(aCable.ConnTo, aCable.ConnTo.DefRate, ModuleUprateList);
...
begin
  ...
  try
    ...
    for i := 0 to TempPatchPart.ModuleList.Count - 1 do begin
      Module := TempPatchPart.ModuleList[i];
      if Module.Selected then begin

        for j := 0 to Module.InConnectorCount - 1 do begin
        ...
        for j := 0 to Module.OutConnectorCount - 1 do begin
          Connector := Module.OutConnector[j];
          CableList := Connector.CreateCableList;
          try
            for Cable in CableList do begin
              Cable.AddMessCableDelete( aSendMessage, aUndoMessage);
              ...
              if not ToConnector.Module.Selected then begin
                TempPatchPart.CreateModuleUprateList( ToConnector, 0, ModuleUprateList);
              end;
              Cable.ConnFrom := nil;
              Cable.ConnTo := nil;
            end;
...
procedure TG2PatchPart.AddMessPasteParams( aSrcePatchPart: IG2PatchPart;
  const aFromVariation, aToVariation : byte; aSendMessage,
  aUndoMessage: TG2SendMessage);
var i, j, k, count : integer;
    ...
    SubMess : TG2Message;
    SrceSelectedModuleList,
    DestSelectedModuleList: TList<IG2Module>;
begin
  ...
  try
    ...
    if SrceSelectedModuleList.Count <> DestSelectedModuleList.Count then
      raise Exception.Create('Source modules do not match destination modules.');
    ...
    for i := 0 to count - 1 do
      if SrceSelectedModuleList[i].TypeID <> DestSelectedModuleList[i].TypeID then
        raise Exception.Create('Source modules do not match destination modules.')
      else
        SrceSelectedModuleList[i].ModuleIndex := DestSelectedModuleList[i].ModuleIndex;
    ...
    try
      ...
      for i := 0 to count - 1 do begin
        for j := 0 to SrceSelectedModuleList[i].ParameterCount - 1 do begin
          ...
          for k := 0 to NMORPHS-1 do begin
            ...
            DestParam.AddMessSetMorph( SrceMorph, aToVariation, aSendMessage, aUndoMessage);
          end;
        end;
      end;
      ...
      // Write param chunk
      Chunk.WriteBits(FLocationIndex, 2);
      aSrcePatchPart.WriteChunkSelectedModules(SrceSelectedModuleList, C_Param_LIST, GetVariationCount, Chunk);
      SubMess.Clear;
      UndoChunk.WriteBits(FLocationIndex, 2);
      WriteChunkSelectedModules(DestSelectedModuleList, C_Param_LIST, GetVariationCount, UndoChunk);
      aUndoMessage.Add(SubMess);
...
begin
  ...
  if (aNewRate = 0) then begin
    // Check if module is uprated by other cables than this one
    i := 0;
    while (i < Module.InConnectorCount) do begin

      Connector := Module.InConnector[i];
      if Connector <> aConnector then begin
        CableList := Connector.CreateCableList;
        try
          j := 0;
          while (j < CableList.Count) do begin

            if (not CableList[j].ToDelete)
              and (CableList[j].ConnFrom.NewRate = 1)
              and (CableList[j].ConnFrom.Module <> Connector.Module) then
  ...
  i := 0;
  while ( i < Module.OutConnectorCount) do begin
    ...
    try
      ...
      while ( j < CableList.Count) do begin
        ...
        Cable := CableList[j];
        if (not Cable.ToDelete)
            and (Cable.ConnFrom.NewRate <> Cable.ConnTo.NewRate) then begin

          if aModuleList.IndexOf( Cable.ConnTo.Module) = -1 then begin
...
var Module: IG2Module;
    ...
    i, j: integer;
begin
  for Module in aModuleUprateList do begin
    aSendMessage.AddMessModuleUprate(Module.LocationIndex, Module.ModuleIndex, Module.NewUprate);
    aUndoMessage.AddMessModuleUprate(Module.LocationIndex, Module.ModuleIndex, Module.Uprate);
      ...
      try
        ...
        while ( j < CableList.Count) do begin
          ...
          if (Cable.CableColor <> Connector.NewConnectorColor) and
             (Connector.NewConnectorColor in [ccRed, ccBlue, ccYellow, ccOrange]) then begin
            // Write cable change message
            Cable.AddMessCableColor( ord(Connector.NewConnectorColor), aSendMessage, aUndoMessage);
          end;
...
var SelectedModuleList: TList<IG2Module>;
begin
  SelectedModuleList := CreateSelectedModuleList;
  try
...
function TG2PatchPart.GetUniqueModuleNameSeqNr( aModuleFileName: string): integer;
var m : integer;
    found : boolean;
begin
  Result := 1;
...
var OldModule, Module : IG2Module;
begin
  if aModuleIndex <> FFocusedModuleIndex then begin
    ...
    if assigned(OldModule) then begin
...
begin
  ...
  for i := FLedList.Count - 1 downto 0 do
    if (FLedList[i].ModuleIndex = aModuleIndex) then begin
      FLedList[i].NotifyDestroy;
      FLedList.Delete(i);
    end;

  for i := FLedStripList.Count - 1 downto 0 do
    if (FLedStripList[i].ModuleIndex = aModuleIndex) then begin
...
function TG2PatchPart.GetInnerCableList( const aModuleList : TList<IG2Module>): TList<IG2Cable>;
var Cable : IG2Cable;
begin
  Result := TList<IG2Cable>.Create;
  for Cable in FCableList do begin
...
begin
  Result := CableListFindCable( FCableList, aFromModuleIndex, aFromConnIndex, aToModuleIndex, aToConnIndex);
end;
...
function TG2PatchPart.FindParam(const aModuleIndex,
...
begin
  ...
  Result := 0;
  for i := 0 to FModuleList.Count - 1 do begin
    if (FModuleList.Items[i].Col = aModule.Col) and (FModuleList.Items[i].Row < aModule.Row) then
      inc(Result);
  end;
end;
...
var MorphParameter : IG2ParamMorph;
begin
  Result := 0;
  MorphParameter := Patch.FindMorph(FLocationIndex, aModuleIndex, aParamIndex, aMorphIndex, aVariation);
...
procedure TG2PatchPart.SelectModule(const aModuleIndex: byte; const aValue,
  aExclusive: boolean);
var Module: IG2Module;
begin
  Module := FindModule(aModuleIndex);
...
function TG2PatchPart.CreateModule(const aModuleIndex, aModuleType : byte): IG2Module;
begin
  Result := TG2FileModule.Create(
    self as IG2PatchPart,
    aModuleType,
    aModuleIndex);
