...
type
  ...
  public
    ...
    function FindNext(aPatchFileType: TPatchFileType; aBank, aPatch: byte)
      : IG2BankItem;
  ...
  public
    constructor Create(const aSource: TG2ParamSource;
  ...
  public
    ...
    function FindBankItem(aPatchFileType: TPatchFileType; aBank, aPatch: byte)
      : IG2BankItem;
    procedure DeleteBankItem(aPatchFileType: TPatchFileType;
      aBank, aPatch: byte);
...
function TBankList.FindNext(aPatchFileType: TPatchFileType; aBank, aPatch: byte)
  : IG2BankItem;
var
  i: integer;
begin
...
procedure TG2Synth.DoAfterClear(PatchFileType: TPatchFileType;
  BankIndex, PatchIndex: byte);
begin
...
procedure TG2Synth.AddMsgStartStopCommunication(aMsg: TG2SendMessage;
  const Stop: byte);
begin
  if Stop = 0 then
...
procedure TG2Synth.AddMsgListMessage(aMsg: TG2SendMessage;
  const aPatchFileType: TPatchFileType; const aBank, aPatch: byte);
begin
...
procedure TG2Synth.AddMsgNoteMessage(aMsg: TG2SendMessage; aNote: byte;
  aOnoff: byte);
begin
...
procedure TG2Synth.AddMsgUploadBankMessage(aMsg: TG2SendMessage;
  aPatchFileType: TPatchFileType; aBank, aLocation: byte);
begin
...
procedure TG2Synth.AddMsgDownloadPatchBankMessage(aMsg: TG2SendMessage;
  aBank, aLocation: byte; aPatchName: string; aPatch: IG2Patch);
var
  Chunk: TPatchChunk;
...
procedure TG2Synth.AddMsgDownloadPerfBankMessage(aMsg: TG2SendMessage;
  aBank, aLocation: byte; aPerfName: string; aPerf: IG2Perf);
var
  Chunk: TPatchChunk;
  MemStream: TMemoryStream;
...
procedure TG2Synth.AddMsgRetrieveMessage(aMsg: TG2SendMessage;
  aSlot, aBank, aPatch: byte);
begin
  if aSlot = 4 then
...
procedure TG2Synth.AddMsgStoreMessage(aMsg: TG2SendMessage;
  aSlot, aBank, aPatch: byte);
begin
  if aSlot = 4 then
...
procedure TG2Synth.AddMsgClearBankMessage(aMsg: TG2SendMessage;
  aPatchFileType: TPatchFileType; aBank, aFromLocation, aToLocation: byte);
begin
...
procedure TG2Synth.AddMsgClearMessage(aMsg: TG2SendMessage;
  aPatchFileType: TPatchFileType; aBank, aPatch: byte);
begin
...
procedure TG2Synth.AddMsgSetPerformance(aMsg: TG2SendMessage; aPerfName: string;
  aPerf: IG2Perf);
var
  Chunk: TPatchChunk;
  i: integer;
...
function TG2Synth.ProcessResponseMsg(aSender: TObject;
  aMsg: TG2Message): boolean;
var
  R, Cmd, Version, SubCmd, Slot, b: byte;
  i: integer;
begin
    ...
    $01:
      begin
        ...
        case Cmd of
          $0C:
            begin
              ...
              case Version of
                  ...
                  begin
                    ...
                    repeat
                      ...
                      case SubCmd of
                        ...
                        begin
                          add_log_line('Unknown subcommand ' + IntToHex(R, 2) +
                            ' ' + IntToHex(Cmd, 2) + ' ' + IntToHex(Version, 2)
                            + ' ' + IntToHex(SubCmd, 2), LOGCMD_ERR);
                          Result := False;
                          break;
                        end;
                      end;
                ...
                begin
                  if Version = Perf.GetPerfVersion then
                  begin
                  ...
                  begin
                    add_log_line('Performance version differs ' + IntToHex(R, 2)
                      + ' ' + IntToHex(Cmd, 2) + ' ' + IntToHex(Version, 2),
                      LOGCMD_ERR);
                    Result := False;
                  end;
          ...
          begin
            add_log_line('Unknown command ' + IntToHex(R, 2) + ' ' +
              IntToHex(Cmd, 2), LOGCMD_ERR);
            Result := False;
          end;
        end;
      end;
...
begin
  ...
  if (aMsg.Size - aMsg.Position) < 4 then
  begin
              ...
              case SubCmd of
                ...
                C_PERF_NAME:
                  begin
                    ...
                    i := 1;
                    aMsg.Read(b, 1);
                    while (aMsg.Position < aMsg.Size - 1) and (i <= 16) and
                      (b <> 0) do
                    begin
  ...
  begin
    case b of
      ...
      begin
        ...
        FslBanks.Add('Category : ' + IntToStr(Category) + ' Mode:' +
          IntToStr(ord(PatchFileType)) + ' Bank: ' + IntToStr(Bank) + ' Patch: '
          + IntToStr(Patch) + ' ' + string(patch_name));
      ...
      begin
        ...
        begin
          ...
          aMsg.Read(Category, 1);
          FslBanks.Add('Category : ' + IntToStr(Category) + ' Mode:' +
            IntToStr(ord(PatchFileType)) + ' Bank: ' + IntToStr(Bank) +
...
var
  FromBank, ToBank, FromLocation, ToLocation: byte;
...
begin
  ...
  while (FromLocation <= ToLocation) and (FromBank = ToBank) do
  begin
...
var
  Bank, Patch, b, hb, lb, PatchVersion, PatchType: byte;
...
begin
  ...
  patch_name := '';
  while (patch_name.Length < 16) and (aMsg.Read(b, 1) = 1) and (b <> 0) do
  begin
    patch_name := patch_name + Char(b);
  end;
  ...
  try
    ...
    i := 0;
    NameExt := '';
    if TPatchFileType(PatchFileType) = pftPatch then
    begin
      FileExt := '.pch2';
    ...
    while FileExists(FBankDumpFolder + patch_name + NameExt + FileExt) do
    begin
      inc(i);
      NameExt := IntToStr(i);
    end;
    ...
    aMsg.Read(PStaticByteBuffer(PatchData.Memory)^[PatchData.Position],
      hb * 256 + lb);
    PatchData.SaveToFile(FBankDumpFolder + file_name);

    // Seems clavia software starts first file entry with 2????
    FBankDumpList.Add(IntToStr(Bank + 1) + ':' + IntToStr(FBankDumpList.Count +
      1) + ': ' + file_name);
  finally
    PatchData.Free;
  end;
...
procedure TG2Synth.DeleteBankItem(aPatchFileType: TPatchFileType;
  aBank, aPatch: byte);
var
  i: integer;
begin
  i := 0;
  while (i < FBankList.Count) and
    not((FBankList[i].PatchFileType = aPatchFileType) and
    (FBankList[i].Bank = aBank) and (FBankList[i].Patch = aPatch)) do
    inc(i);
  if i < FBankList.Count then
    FBankList.Delete(i);
end;

function TG2Synth.FindBankItem(aPatchFileType: TPatchFileType;
  aBank, aPatch: byte): IG2BankItem;
var
  i: integer;
begin
  i := 0;
...
begin
  ...
  while (i < FMidiToKnobList.Count) and
    not((FMidiToKnobList[i].Source = aSource) and
    (FMidiToKnobList[i].ControlType = aControlType) and
    (FMidiToKnobList[i].Knob = aKnob)) do
    inc(i);
...
begin
  i := 0;
  while (i < FMidiToKnobList.Count) and
    not(FMidiToKnobList[i].Controller = aMidiCC) do
    inc(i);

  Result := (i < FMidiToKnobList.Count);
end;

function TG2Synth.TextFunction(No: integer;
  ...
  try
    case FInitStep of
      0:
        aConnection.SynthInit;
      1:
        aConnection.SynthStartStopCommunication(STOP_COMM);
      2:
        aConnection.SynthGetPatchVersion;
      3:
        aConnection.SynthSettings;
      4:
        aConnection.SynthUnknown1Message;
      5:
        Perf.StartInit(aConnection, False);
      6:
        if FNextBankListCmd.PatchFileType <> pftEnd then
        begin
          aConnection.SynthGetList(FNextBankListCmd.PatchFileType,
            FNextBankListCmd.Bank, FNextBankListCmd.Patch);
          do_next_step := False;
        end
        else
        begin
...
procedure TG2Synth.SetBankItem(const aIndex: integer;
  const aValue: IG2BankItem);
begin
...
function TG2Synth.BankFindFirstLast(aPatchFileType: TPatchFileType; aBank: byte;
  var aFirstLocation, aLastLocation: byte): boolean;
begin
  Result := FBankList.FindFirstLast(aPatchFileType, aBank, aFirstLocation,
    aLastLocation);
end;

function TG2Synth.BankFindNext: IG2BankItem;
begin
  Result := FBankList.FindNext(FNextBankListCmd.PatchFileType,
    FNextBankListCmd.Bank, FNextBankListCmd.Patch);
end;
