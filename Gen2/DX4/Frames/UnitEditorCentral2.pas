...
type
  ...
  public
    ...
    procedure ProcessKeyDown(var Key: Word; var KeyChar: Char;
      Shift: TShiftState);
    procedure ProcessKeyUp(var Key: Word; var KeyChar: Char;
      Shift: TShiftState);
    ...
    property SelectedSynthIndex: Integer read GetSelectedSynthIndex
      write SetSelectedSynthIndex;
    property OnShowAppMessage: TAppShowMessageEvent read FOnShowAppMessage
      write SetOnShowAppMessage;
    property OnCloseAppMessage: TAppCloseMessageEvent read FOnCloseAppMessage
      write SetOnCloseAppMessage;
  end;
...
begin
  ...
  if (FG2List.Count - 1) * 4 >= tcPatch.TabCount then
  begin
    for i := 0 to 3 do
    begin
      // Add frameslots
...
begin
  ...
  begin
    ...
    frmPatchSettings.framePatchSettings.Patch :=
      FSynth.SelectedSlot.Patch as TG2GraphPatchFMX;
...
var
  ...
  frameSlot: TframeSlot;
begin
  for i := 0 to (4 * FG2List.Count) - 1 do
  begin
    frameSlot := FSlotList[i];
    if (rbSynth.Value = i div 4) and (i mod 4 = FSynth.Performance.SelectedSlot)
    then
    begin
...
begin
  ...
  case aG2Event of
    ...
    EvtMidiClockReceive:
      begin
        if Synth.Performance.MasterClockRun = 1 then
        begin
    ...
    EvtClockRunChange:
      begin
        if Synth.Performance.MasterClockRun = 0 then
        begin
          btClockRun.Value := 0;
    ...
    EvtMidiCCRecieve:
      begin
        frameParam.UpdateAssignCCButton
          ((Synth.Performance as TG2MessPerformance).LastMidiCC);
      end;
    ...
    EvtSelectLocation:
      begin
        if Synth.SelectedSlot.Patch.SelectedLocation in [ltFX, ltVA, ltPatch]
        then
...
begin
  ...
  frameSlot := GetSelectedFrameSlot;
  if assigned(frameSlot) and assigned(frameSlot.Patch) then
  begin
  ...
  case Key of
    ...
    vkUp:
      begin
        ...
        else if frameSlot.OperationMode = omAddModule then
...
begin
  if assigned(FSynth) then
  begin
    if (FSynth.IgnoreExternalClock = 0)
    { and (FSynth.Performance.MasterClockRun = 1) } then
...
begin
  case aValue of
    0:
      SlotSetZoomSetting(0);
    1:
      SlotSetZoomSetting(1);
    2:
      SlotSetZoomSetting(2);
  end;
end;
...
procedure TframeEditorCentralStrip.SelectLocation(const aLocation
  : TLocationType);
begin
  if not assigned(FSynth) then
    Exit;
...
procedure TframeEditorCentralStrip.SetStateStyles(aStateStyleList: TG2StateStyleList);
var
  i: Integer;
begin
