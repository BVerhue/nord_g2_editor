...
uses
  ...
  System.Variants, System.Generics.Collections, System.IOUtils,
  FMX.Types, FMX.Controls, FMX.Forms, FMX.Dialogs,
  FMX.StdCtrls, FMX.Edit, xmldom, XMLDoc, XMLIntf,
  BVE.NMG2Types, BVE.NMG2GraphFMX, BVE.NMG2File,
  BVE.NMG2ControlsFMX, BVE.NMG2AppSettings, FMX.TabControl, UnitAppSynthSettings,
...
type
  ...
  public
    ...
    procedure AddMidiToKnob( aSynth : TG2GraphFMX; aSource : TG2ParamSource; aControlType : TG2ControlType; aKnob : integer; aMidiCC : byte);
    procedure DeleteMidiToKnob( aSynth : TG2GraphFMX; aSource : TG2ParamSource; aControlType : TG2ControlType; aKnob : integer);
    function  FindMidiToKnob(  aSynth : TG2GraphFMX; aSource : TG2ParamSource; aControlType : TG2ControlType; aKnob : integer): integer;
    ...
    property OnKnobControlChange : TNotifyEvent read FOnKnobControlChange write FOnKnobControlChange;
  end;
implementation
...
function TframeAppSettings.FindOrAddframeAppSynthSettings( aID,
    aHost : string; aPort : integer; aUSBConnection : boolean;
    aSynth : TG2GraphFMX): TframeAppSynthSettings;
var i : integer;
    ti : TTabItem;
begin
  i := 0;
  while (i<FframeAppSynthSettingsList.Count) and not(FframeAppSynthSettingsList[i].ID = aID) do
    inc(i);
  if (i<FframeAppSynthSettingsList.Count) then begin
    ...
    Result.Name := 'frameAppSynthSettings' + IntToStr(FframeAppSynthSettingsList.Count);
    Result.Parent := ti;
...
begin
  ...
  i := 0;
  while (i<FframeAppSynthSettingsList.Count) and not(FframeAppSynthSettingsList[i].Synth = aSynth) do
    inc(i);

  if (i<FframeAppSynthSettingsList.Count) then begin
    FframeAppSynthSettingsList[i].Synth.AddMidiToKnob( aSource, aControlType, aKnob, aMidiCC);
    SaveSettings;
  end;
end;
...
begin
  ...
  if (i<FframeAppSynthSettingsList.Count) then begin
    FframeAppSynthSettingsList[i].Synth.DeleteMidiToKnob( aSource, aControlType, aKnob);
    SaveSettings;
  end;
end;

function TframeAppSettings.FindMidiToKnob(aSynth: TG2GraphFMX;
  aSource : TG2ParamSource; aControlType : TG2ControlType; aKnob: integer): integer;
var i : integer;
begin
  Result := 0;
  ...
  if (i<FframeAppSynthSettingsList.Count) then
    Result := FframeAppSynthSettingsList[i].Synth.FindMidiToKnob( aSource, aControlType, aKnob);
end;

function TframeAppSettings.MidiToKnobCheckCC( aSynth: TG2GraphFMX; aMidiCC: byte): boolean;
var i : integer;
begin
...
procedure TframeAppSettings.AutoAssignMidiToKnobs(aSynth : TG2GraphFMX; aSlotIndex: integer);
...
begin
  ...
  0, 1, 2, 3 : // Slots
    begin
      ...
      for i := 0 to Patch.KnobList.Count - 1 do begin
        ...
        if MidiCC <> 0 then begin
          ...
          if assigned(Knob) and (Knob.IsAssigned = 1) and assigned(Knob.Parameter) then begin
            if (not assigned(Knob.Parameter.Controller)) or (Knob.Parameter.Controller.MidiCC <> MidiCC) then
              Patch.MessAssignMidiCC( Knob.Parameter.Location, Knob.Parameter.ModuleIndex, Knob.Parameter.ParamIndex, MidiCC);
          end;
        end;
        MidiCC := FindMidiToKnob(aSynth, g2psParam, g2ctButton, i);
        if MidiCC <> 0 then begin
          Knob := Patch.KnobList[i];
          if assigned(Knob) and (Knob.IsAssigned = 1) and assigned(Knob.Parameter) and assigned(Knob.Parameter.ButtonParam) then begin
            if (not assigned(Knob.Parameter.ButtonParam.Controller)) or (Knob.Parameter.ButtonParam.Controller.MidiCC <> MidiCC) then
              Patch.MessAssignMidiCC( Knob.Parameter.ButtonParam.Location, Knob.Parameter.ButtonParam.ModuleIndex, Knob.Parameter.ButtonParam.ParamIndex, MidiCC);
          end;
        end;
      end;
        ...
        if MidiCC <> 0 then begin
          ...
          if assigned(Parameter) then
            if (not assigned(Parameter.Controller)) or (Parameter.Controller.MidiCC <> MidiCC) then
              Patch.MessAssignMidiCC( Parameter.Location, Parameter.ModuleIndex, Parameter.ParamIndex, MidiCC);
        end;
      ...
      for i := 0 to 7 do begin
        ...
        if MidiCC <> 0 then begin
          Parameter := aSynth.GetSlot(aSlotIndex).Patch.GetPatchKnobParameter(i+8);
          if assigned(Parameter) and (Parameter.MidiAssignmentsAllowed) then
            if (not assigned(Parameter.Controller)) or (Parameter.Controller.MidiCC <> MidiCC) then
              Patch.MessAssignMidiCC( Parameter.Location, Parameter.ModuleIndex, Parameter.ParamIndex, MidiCC);
        end;
  ...
  4 : // Performance
    begin
      for i := 0 to aSynth.Performance.GlobalKnobList.Count - 1 do begin
        MidiCC := FindMidiToKnob(aSynth, g2psGlobal, g2ctKnob, i);
        if MidiCC <> 0 then begin
          GlobalKnob :=  aSynth.Performance.GlobalKnobList[i];
          if assigned(GlobalKnob) and (GlobalKnob.IsAssigned = 1) and assigned(GlobalKnob.Parameter) then begin
...
begin
  ...
  if (i<FframeAppSynthSettingsList.Count) then begin
    ...
    for j := 0 to 3 do
      (aSynth.GetSlot(j).Patch as TG2GraphPatchFMX).SplitterVisible := FSplitterVisible = 1;
  end else begin
    ti := TTabItem.Create( tcSynthSettings);
    ti.Parent := tcSynthSettings;
    ti.Text := aID;
    ...
    frameAppSynthSettings.Name := 'frameAppSynthSettings' + IntToStr(FframeAppSynthSettingsList.Count);
...
var XMLAppSettings : TXMLDocument;
...
{$ENDIF}
  ...
  for i := 0 to FframeAppSynthSettingsList.Count - 1 do begin
    ...
    if assigned(Synth) then begin
      for j := 0 to Synth.MidiToKnobList.Count - 1 do begin
        ...
        XMLMidiToKnobType.ControlType := integer(Synth.MidiToKnobList[j].FControlType);
...
begin
  ...
  for i := 0 to FframeAppSynthSettingsList.Count - 1 do
    ComponentSetStateStylesRecursive(FframeAppSynthSettingsList[i], aStateStyleList);
end;
