...
type
  ...
  private
    ...
    FStartX, FStartY, FMouseX, FMouseY: single;
  ...
  TframeSlot = class(TFrame, IG2Observer)
    ...
    procedure sbVADragOver(Sender: TObject; const Data: TDragObject;
      const Point: TPointF; var Operation: TDragOperation);
    procedure sbVADragDrop(Sender: TObject; const Data: TDragObject;
      const Point: TPointF);
    procedure sbFXDragDrop(Sender: TObject; const Data: TDragObject;
      const Point: TPointF);
    procedure sbFXDragOver(Sender: TObject; const Data: TDragObject;
      const Point: TPointF; var Operation: TDragOperation);
  private
    ...
    FPrevOperationMode, FOperationMode: TOperationMode;
    FPanEnabled: boolean;
    FModuleCursor: TModuleCursor;
    FCursorVARect, FCursorFXRect: TRect;
    FDragSelector: TDragSelector;
    FSelectionVA, FSelectionFX: TSelection;
    ...
    procedure CreateModuleFMX(Sender: TObject; aLocation: TLocationType;
      aModule: TG2GraphModuleFMX);
    procedure CreateCableFMX(Sender: TObject; aLocation: TLocationType;
    ...
    procedure DoDragOver(Sender: TObject; const Data: TDragObject;
      const Point: TPointF; var Operation: TDragOperation);
    procedure DoDragDrop(Sender: TObject; const Data: TDragObject;
      const Point: TPointF);
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    ...
    procedure ActivateSelection(aPatchPart: TG2FilePatchPart;
      aSelectionMode: TSelectionMode; MouseX, MouseY: single);
    ...
    procedure SlotDeleteModule(Sender: TObject; SenderID: integer;
    ...
    procedure ControlMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: single);
    procedure ControlGesture(Sender: TObject;
      const EventInfo: TGestureEventInfo; var Handled: boolean);
    procedure ModuleMouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: single);
    procedure ModuleMouseMove(Sender: TObject; Shift: TShiftState;
      X, Y: single);
    procedure ModuleMouseUp(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: single);
    ...
    property SelectedControl: TG2BaseControl read GetSelectedControl
      write SetSelectedControl;
    property SelectedModule: TG2GraphModuleFMX read FModule
      write SetSelectedModule;
    property SelectedLocation: TLocationType read GetSelectedLocation
    ...
    property MouseOverControl: TG2BaseControl read FMouseOverControl
      write SetMouseOverControl;
    property MouseOverModule: TG2GraphModuleFMX read FMouseOverModule
      write SetMouseOverModule;
    property MouseOverLocation: TLocationType read FMouseOverLocation
      write SetMouseOverLocation;
    property OperationMode: TOperationMode read FOperationMode
      write SetOperationMode;
    property OnOperationModeChange: TNotifyEvent read FOnOperationModeChange
      write FOnOperationModeChange;
    property OnMiniVUClick: TMiniVUClickEvent read FOnMiniVUClick
      write FOnMiniVUClick;
  end;
...
begin
  ...
  begin
    ...
    begin
      for Module in FPatch.PatchPart[ord(ltFX)].ModuleList do
      begin
        (Module as TG2GraphModuleFMX).FreePanel;
      end;
      for Module in FPatch.PatchPart[ord(ltVA)].ModuleList do
      begin
        (Module as TG2GraphModuleFMX).FreePanel;
      end;
      for Cable in FPatch.PatchPart[ord(ltFX)].CableList do
      begin
        (Cable as TG2GraphCableFMX).G2Control.Free;
      end;
      for Cable in FPatch.PatchPart[ord(ltVA)].CableList do
      begin
        (Cable as TG2GraphCableFMX).G2Control.Free;
      end;
...
procedure TframeSlot.CreateModuleFMX(Sender: TObject; aLocation: TLocationType;
  aModule: TG2GraphModuleFMX);
begin
  case aLocation of
    ltPatch:
...
begin
  if Sender is TG2GraphModuleFMX then
  begin
...
begin
  if assigned(FPatch) then
  begin
    if assigned(FlZoomVA) then
    begin
      Rect := FPatch.ModuleList[ord(ltVA)].UnitsRect;
      FlZoomVA.SetBounds(0, 0, (Rect.Right + 4) * (UNITS_COL + UNIT_MARGIN * 2),
        (Rect.Bottom + 20) * (UNITS_ROW + UNIT_MARGIN * 2));
      lSizeVA.Width := FlZoomVA.Width * FZoom;
      lSizeVA.Height := FlZoomVA.Height * FZoom;
    end;
    if assigned(FlZoomFX) then
    begin
      Rect := FPatch.ModuleList[ord(ltFX)].UnitsRect;
      FlZoomFX.SetBounds(0, 0, (Rect.Right + 4) * (UNITS_COL + UNIT_MARGIN * 2),
        (Rect.Bottom + 20) * (UNITS_ROW + UNIT_MARGIN * 2));
...
begin
  ...
  sbVA.ViewportPosition := PointF(mx * FZoom - (sbVA.Width / 2),
    my * FZoom - (sbVA.Height / 2));
end;
...
begin
  ...
  lSizeFX.Height := FlZoomFX.Height * FZoom;
  // Adjust scrollbox to keep centered on original mid point
  sbFX.ViewportPosition := PointF(mx * FZoom - (sbFX.Width / 2),
    my * FZoom - (sbFX.Height / 2));
end;
...
var
  Module: TG2FileModule;
begin
  if assigned(GetPatchPart.FocusedModule) then
  begin
    FModuleCursor.SetModuleLocation(GetPatchPart.FocusedModule);
    Module := GetPatchPart.ModuleList.ModuleAtLocation(FModuleCursor.Row - 1,
      FModuleCursor.Col);
  end
  else
  begin
...
begin
  ...
  begin
    FModuleCursor.SetModuleLocation(GetPatchPart.FocusedModule);
    Module := GetPatchPart.ModuleList.ModuleAtLocation(FModuleCursor.Row,
      FModuleCursor.Col - 1);
  end
  else
  begin
...
begin
  ...
  begin
    FModuleCursor.SetModuleLocation(GetPatchPart.FocusedModule);
    Module := GetPatchPart.ModuleList.ModuleAtLocation(
      FModuleCursor.Row + FModuleCursor.UnitsHeight, FModuleCursor.Col);
...
var
  mx, my, px, py: Single;
begin
  mx := (FocusRect.Left + (FocusRect.Right - FocusRect.Left) / 2)
    * (UNITS_COL + UNIT_MARGIN * 2);
  my := (FocusRect.Top + (FocusRect.Bottom - FocusRect.Top) / 2)
...
begin
  if not FPatch.SplitterVisible then
  begin
    ...
    else if SelectedLocation = ltFX then
    begin
      lVA.Visible := False;
      lFX.Visible := True;
      CalcLayoutDimensions;
...
procedure TframeSlot.DoDragDrop(Sender: TObject; const Data: TDragObject;
  const Point: TPointF);
var
  i: integer;
begin
  ...
  begin
    if (Lowercase(ExtractFileExt(Data.Files[0])) = FILE_PATCH_EXT)
    or (Lowercase(ExtractFileExt(Data.Files[0])) = FILE_PERF_EXT)
    then
    begin
      (FPatch.G2 as TG2GraphFMX).LoadFileStream(Data.Files[0]);
    end;
  end;
end;

procedure TframeSlot.DoDragOver(Sender: TObject; const Data: TDragObject;
  const Point: TPointF; var Operation: TDragOperation);
begin
...
begin
  ...
  if SelectedLocation = ltFX then
  begin
    if FModuleCursor.Parent <> FlZoomFX then
    begin
      FModuleCursor.Parent := FlZoomFX;
      FModuleCursor.CursorPos := FCursorFXRect;
    end;
  end
  else if SelectedLocation = ltVA then
  begin
    if FModuleCursor.Parent <> FlZoomVA then
    begin
      FModuleCursor.Parent := FlZoomVA;
...
begin
  ...
  if not assigned(FframePianoroll) then
  begin
...
begin
  ...
  if not assigned(FframeAddCable) then
  begin
  ...
  begin
    ...
    if assigned(FframeAddCable.Connector) then
    begin
...
begin
  ...
  begin
    if FframeAddCable.Connector.CableCount > 0 then
    begin
      for i := 0 to FframeAddCable.Connector.CableCount - 1 do
      begin
        Patch.MessDeleteConnection(FframeAddCable.Connector.Location,
          FframeAddCable.Connector.Cables[i]);
      end;
    end;
  end;
  FinishOperation;
...
procedure TframeSlot.ActivateSelection(aPatchPart: TG2FilePatchPart;
  aSelectionMode: TSelectionMode; MouseX, MouseY: Single);
var
  X, Y, w, h, b: Single;
begin
  ...
  if aPatchPart.Location = ltVA then
  begin
...
begin
  ...
  if FSelectionMode = smCopy then
  begin
    if MouseOverLocation = ltVA then
    begin
      EnablePan(sbVA);
      FSelectionVA.CalcNewPositions;
...
begin
  ...
  FDragSelector.StartPoint := PointF(
    (sb.ViewportPosition.X + X) / FZoom,
    (sb.ViewportPosition.Y + Y) / FZoom);
end;
...
begin
  ...
  for i := 0 to PatchPart.ModuleList.Count - 1 do
  begin
    PatchPart.ModuleList[i].SelectModule(
      FDragSelector.InSelection(
        PatchPart.ModuleList[i].Col * (UNITS_COL + UNIT_MARGIN * 2),
        PatchPart.ModuleList[i].Row * (UNITS_ROW + UNIT_MARGIN * 2)),
        False);
  end;
  FDragSelector.Active := False;
end;
...
begin
  ...
  // UpdateControls;
  // end;
  if Value <> FSelectedLocation then
  begin
...
begin
  if Value <> FMouseOverControl then
  begin
...
begin
  if Value <> FMouseOverLocation then
  begin
...
begin
  if Value <> FMouseOverModule then
  begin
...
begin
  ...
  if FOperationMode = omPianoroll then
  begin
    FPatch.OnLed := nil;
...
begin
  ...
  begin
    if (FOperationMode = omPianoroll) and (FframePianoroll.Visible) then
    begin
      FPatch.OnLed := nil;
      FframePianoroll.SeqModule := nil;
      FframePianoroll.Visible := False;
...
begin
  ...
  begin
    FinishOperation;
    if assigned(FframePianoroll.SeqModule) then
    begin
      if (FframePianoroll.SeqModule.Location = Location) and
        (FframePianoroll.SeqModule.ModuleIndex = ModuleIndex) then
        FframePianoroll.SeqModule := nil;
    end;
  end;
end;
...
begin
  case OperationMode of
    omNormal, omModule, omParam, omAddModule:
      begin
        if Sender is TG2MiniVU then
        begin
          if assigned(FOnMiniVUClick) then
            FOnMiniVUClick(self, Sender as TG2MiniVU);
        end
        else if Sender is TG2BaseControl then
        begin
...
procedure TframeSlot.ControlMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Single);
var
  P, CP: TPointF;
  dx, dy: Single;
...
begin
  ...
  MouseOverModule := FPatch.Modules[ord(FMouseOverControl.Location),
    FMouseOverControl.ModuleIndex] as TG2GraphModuleFMX;
  ...
  case OperationMode of
    ...
    omDragSelect:
      begin
        ...
        FDragSelector.DragPoint := PointF((sb.ViewportPosition.X + P.X) / FZoom,
          (sb.ViewportPosition.Y + P.Y) / FZoom);
      end;
  ...
  case OperationMode of
    omNormal, omModule, omParam, omAddModule:
      begin
        if assigned(SelectedControl) then
        begin
          if SelectedControl is TG2Connector then
          begin
            ActivateDrawCable;
          end
          else
    ...
    omDrawCable:
      begin
        if (Sender <> SelectedControl) and (Sender is TG2Connector) then
        begin
          if assigned(FframeAddCable) and assigned(FframeAddCable.Connector)
          then
            FframeAddCable.Connector.CreateCable(self,
              (Sender as TG2Connector));
        end;
        FinishOperation;
      end;
  end;
...
var
  P: TPointF;
begin
  if (Sender is TG2Module) then
  begin
    case OperationMode of
      omNormal, omModule, omParam, omAddModule, omDrawCable, omSelection:
        begin
          SelectedModule := FPatch.Modules[
            ord((Sender as TG2Module).Location),
            (Sender as TG2Module).ModuleIndex] as TG2GraphModuleFMX;
...
procedure TframeSlot.ModuleMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Single);
var
  P, CP: TPointF;
  dx, dy: Single;
...
begin
  ...
  MouseOverModule := FPatch.Modules[
    ord((Sender as TG2Module).Location),
    (Sender as TG2Module).ModuleIndex] as TG2GraphModuleFMX;
  ...
  case OperationMode of
    omNormal, omModule, omParam, omAddModule:
      begin
        ...
        else if (ssLeft in Shift) or (ssTouch in Shift) then
        begin
          if assigned(SelectedModule) then
          begin
            if (abs(dx) > 0.1) or (abs(dy) > 0.1) then
            begin
              // Activate selection
              // If the module wasn't selected yet, then select it here
              if not SelectedModule.Selected then
              begin
              ...
              ActivateSelection(Patch.PatchPart[ord(SelectedLocation)], smMove,
                P.X / FZoom, P.Y / FZoom);
            end;
          end;
        end;
...
begin
  ...
  case OperationMode of
    omNormal, omModule, omParam:
      begin
        if (Sender is TG2Module) then
        begin
          // Add the control to the selection
          SelectedModule := FPatch.Modules[ord((Sender as TG2Module).Location),
            (Sender as TG2Module).ModuleIndex] as TG2GraphModuleFMX;;
          if assigned(SelectedControl) then
          begin
            CP := SelectedControl.AbsoluteToLocal(P);
          ...
          begin
            if (ssShift in Shift) or (SelectMultiple) then
            begin
...
procedure TframeSlot.sbFXDragDrop(Sender: TObject; const Data: TDragObject;
  const Point: TPointF);
begin
  DoDragDrop(Sender, Data, Point);
end;

procedure TframeSlot.sbFXDragOver(Sender: TObject; const Data: TDragObject;
  const Point: TPointF; var Operation: TDragOperation);
begin
  DoDragOver(Sender, Data, Point, Operation);
end;
...
begin
  ...
  if (Sender as TScrollBox).Name = 'sbVA' then
  begin
    SelectedLocation := ltVA;
...
procedure TframeSlot.sbMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Single);
var
  dx, dy: Single;
  P, CP: TPointF;
...
begin
  ...
  case OperationMode of
    ...
    omDragSelect:
      begin
        FDragSelector.DragPoint := PointF((sb.ViewportPosition.X + X) / FZoom,
          (sb.ViewportPosition.Y + Y) / FZoom);
      end;
  ...
  case OperationMode of
    ...
    omAddModule:
      begin
        ...
        else if (Sender as TScrollBox).Name = 'sbFX' then
        begin
          FModuleCursor.Parent := FlZoomFX;
        end;
        FModuleCursor.Col :=
          Trunc((((Sender as TScrollBox).ViewportPosition.X + X) / FZoom) / (UNITS_COL + UNIT_MARGIN * 2));
        FModuleCursor.Row :=
          Trunc((((Sender as TScrollBox).ViewportPosition.Y + Y) / FZoom) / (UNITS_ROW + UNIT_MARGIN * 2));
        SaveCursorPos;
      end;
...
procedure TframeSlot.sbVADragDrop(Sender: TObject; const Data: TDragObject;
  const Point: TPointF);
begin
  DoDragDrop(Sender, Data, Point);
end;

procedure TframeSlot.sbVADragOver(Sender: TObject; const Data: TDragObject;
  const Point: TPointF; var Operation: TDragOperation);
begin
  DoDragOver(Sender, Data, Point, Operation);
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        Module.NewCol :=
          Max(0, Trunc((Module.Col - ModulesRect.Left) + (Position.X + FMouseX -
          (UNITS_COL + UNIT_MARGIN * 2) * (ModulesRect.Right - ModulesRect.Left)
          / 2) / (UNITS_COL + UNIT_MARGIN * 2)));
        Module.NewRow :=
          Max(0, Trunc((Module.Row - ModulesRect.Top) + (Position.Y + FMouseY -
          (UNITS_ROW + UNIT_MARGIN * 2) * (ModulesRect.Bottom - ModulesRect.Top)
          / 2) / (UNITS_ROW + UNIT_MARGIN * 2)));
      end;
    end
    else
    begin
      // After move
      for i := 0 to FPatchPart.SelectedModuleList.Count - 1 do
      begin
...
procedure TSelection.ProcessMouseMove(Sender: TObject; Shift: TShiftState;
  AbsX, AbsY: Single);
var
  P: TPointF;
begin
...
begin
  ...
  aCanvas.Stroke.Color := claGreen;
  aCanvas.Stroke.Thickness := 3;
  R := RectF(UnscaledBoundsRect.Left + UNIT_MARGIN, UnscaledBoundsRect.Top +
    UNIT_MARGIN, UnscaledBoundsRect.Right - UNIT_MARGIN,
...
begin
  ...
  bl := nil;
  while assigned(FMXObject) and not(FMXObject is TScrollBox) do
  begin
  ...
  if assigned(bl) then
    Zoom := bl.Zoom;
  R := RectF(Position.X * Zoom, Position.Y * Zoom, (Position.X + Width) * Zoom,
  ...
  else if R.Left < sb.ViewportPosition.X then
  ...
  else if R.Top < sb.ViewportPosition.Y then
...
begin
  if FUnitsHeight <> Value then
  begin
    FUnitsHeight := Value;
    UnscaledHeight := FUnitsHeight * (UNITS_ROW + UNIT_MARGIN * 2) +
      FMargin * 2;
  end;
end;
...
begin
  Result := ((X + (UNITS_COL + UNIT_MARGIN * 2) / 2) >= UnscaledRect.Left +
    FMargin) and (X <= UnscaledRect.Right - FMargin) and
    ((Y + (UNITS_ROW + UNIT_MARGIN * 2) / 2) >= UnscaledRect.Top + FMargin) and
    (Y <= UnscaledRect.Bottom - FMargin);
end;
...
begin
  ...
  aCanvas.Stroke.Color := claGreen;
  aCanvas.Stroke.Thickness := 3;
  R := RectF(UnscaledBoundsRect.Left + FMargin, UnscaledBoundsRect.Top +
    FMargin, UnscaledBoundsRect.Right - FMargin, UnscaledBoundsRect.Bottom
    - FMargin);
