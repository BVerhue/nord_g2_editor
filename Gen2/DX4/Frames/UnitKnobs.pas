...
uses
  ...
  {UnitParam, }UnitAppSettings, FMX.Objects, FMX.Controls.Presentation;
type
...
begin
  for i := 0 to 7 do begin
    ...
    AssignParamToButton( i, FSynth.SelectedSlot.Patch.GetMorphKnobParameter(i + 8));
  end;
end;
procedure TframeKnobs.AssignParamToKnob(aIndex: integer; aParam: TG2FileParameter);
begin
...
procedure TframeKnobs.AssignParamToButton(aIndex: integer; aParam: TG2FileParameter);
begin
...
begin
  ...
  0, 1 : begin
           ...
           if (eMidiCC.Text = '') or (c>0) or (Value = 0) or (not CheckCCAllowed(Value)) then begin
             // Auto Deassign Midi
             Knob := nil;
             ...
             if assigned(Knob) and (Knob.IsAssigned = 1) then begin
               if ControlType = g2ctKnob then begin
                 if assigned(Knob.Parameter) and assigned(Knob.Parameter.Controller) then
                   (FSynth.SelectedSlot.Patch as TG2GraphPatchFMX).MessDeassignMidiCC( Knob.Parameter.Controller.MidiCC);
               end else begin
                 if assigned(Knob.Parameter) and assigned(Knob.Parameter.ButtonParam) and assigned(Knob.Parameter.ButtonParam.Controller) then
                   (FSynth.SelectedSlot.Patch as TG2GraphPatchFMX).MessDeassignMidiCC( Knob.Parameter.ButtonParam.Controller.MidiCC);
               end;
             end;
             eMidiCC.Text := '';
             FframeAppSettings.DeleteMidiToKnob(FSynth, TG2ParamSource(rbSource.Value), ControlType, KnobIndex);
           end else begin
             if FframeAppSettings.MidiToKnobCheckCC(FSynth, Value) then
               raise Exception.Create('Midi CC ' + IntToStr(Value) + ' is already used.');
             FframeAppSettings.AddMidiToKnob(FSynth, TG2ParamSource(rbSource.Value), ControlType, KnobIndex, Value);
             // Auto assign Midi
             Knob := nil;
             if rbSource.Value = 0 then
               Knob := FSynth.SelectedSlot.Patch.GetKnob(KnobIndex)
             ...
             if assigned(Knob) and (Knob.IsAssigned = 1) then begin
               if ControlType = g2ctKnob then begin
                 if assigned(Knob.Parameter) then
                   (FSynth.SelectedSlot.Patch as TG2GraphPatchFMX).MessAssignMidiCC( Knob.Parameter.Location, Knob.Parameter.ModuleIndex, Knob.Parameter.ParamIndex, Value);
               end else begin
                 if assigned(Knob.Parameter) and assigned(Knob.Parameter.ButtonParam) then
                   (FSynth.SelectedSlot.Patch as TG2GraphPatchFMX).MessAssignMidiCC( Knob.Parameter.ButtonParam.Location, Knob.Parameter.ButtonParam.ModuleIndex, Knob.Parameter.ButtonParam.ParamIndex, Value);
               end;
             end;
           end;
  ...
  2, 3 : begin
           ...
           if (rbSource.Value = 3) and ((KnobNo = 0) or (KnobNo = 1)) then begin
           ...
           if (eMidiCC.Text = '') or (c>0) or (Value = 0) or (not CheckCCAllowed(Value)) then begin
             eMidiCC.Text := '';
             FframeAppSettings.DeleteMidiToKnob(FSynth, TG2ParamSource(rbSource.Value), ControlType, KnobNo);
             // Auto deassign Midi
             ...
             if assigned(Parameter) and assigned(Parameter.Controller) then
               (FSynth.SelectedSlot.Patch as TG2GraphPatchFMX).MessDeassignMidiCC( Parameter.Controller.MidiCC);
           end else begin
             if FframeAppSettings.MidiToKnobCheckCC(FSynth, Value) then
               raise Exception.Create('Midi CC ' + IntToStr(Value) + ' is already used.');
             FframeAppSettings.AddMidiToKnob(FSynth, TG2ParamSource(rbSource.Value), ControlType, KnobNo, Value);
             // Auto Assign Midi
             if rbSource.Value = 2 then
               Parameter := FSynth.SelectedSlot.Patch.GetMorphKnobParameter(KnobIndex)
             else
             ...
             if assigned(Parameter) then
               (FSynth.SelectedSlot.Patch as TG2GraphPatchFMX).MessAssignMidiCC( Parameter.Location, Parameter.ModuleIndex, Parameter.ParamIndex, Value);
           end;
           FframeAppSettings.SaveSettings;
         end;
  end;
...
begin
  case aIndex of
  0 : Result := tfDisplayA1;
  1 : Result := tfDisplayA2;
  2 : Result := tfDisplayA3;
  3 : Result := tfDisplayA4;
  4 : Result := tfDisplayA5;
  5 : Result := tfDisplayA6;
  6 : Result := tfDisplayA7;
  7 : Result := tfDisplayA8;
  else
...
begin
  case aIndex of
  0 : Result := tfDisplayB1;
  1 : Result := tfDisplayB2;
  2 : Result := tfDisplayB3;
  3 : Result := tfDisplayB4;
  4 : Result := tfDisplayB5;
  5 : Result := tfDisplayB6;
  6 : Result := tfDisplayB7;
  7 : Result := tfDisplayB8;
  else
...
begin
  case aIndex of
  0 : Result := tfDisplayC1;
  1 : Result := tfDisplayC2;
  2 : Result := tfDisplayC3;
  3 : Result := tfDisplayC4;
  4 : Result := tfDisplayC5;
  5 : Result := tfDisplayC6;
  6 : Result := tfDisplayC7;
  7 : Result := tfDisplayC8;
  else
...
begin
  case aIndex of
  0 : Result := tfDisplayD1;
  1 : Result := tfDisplayD2;
  2 : Result := tfDisplayD3;
  3 : Result := tfDisplayD4;
  4 : Result := tfDisplayD5;
  5 : Result := tfDisplayD6;
  6 : Result := tfDisplayD7;
  7 : Result := tfDisplayD8;
  else
...
begin
  if assigned(FSynth) then
    case rbSource.Value of
    0 : FSynth.SelectedSlot.Patch.SelectedParamPage := aValue;
    1 : FSynth.Performance.SelectedParamPage := aValue;
    end;
  UpdateControls;
end;
...
begin
  if assigned(FSynth) then
    case rbSource.Value of
    0 : FSynth.SelectedSlot.Patch.SelectedParamPageColumn := aValue;
    1 : FSynth.Performance.SelectedParamPageColumn := aValue;
    end;
  UpdateControls;
end;
...
procedure TframeKnobs.SetEditMidiCC(aIndex: integer; aValue: string; aVisible : boolean);
begin
  case aIndex of
   0 : begin eMidiCC1.Text := aValue; eMidiCC1.Visible := aVisible; end;
   1 : begin eMidiCC2.Text := aValue; eMidiCC2.Visible := aVisible; end;
   2 : begin eMidiCC3.Text := aValue; eMidiCC3.Visible := aVisible; end;
   3 : begin eMidiCC4.Text := aValue; eMidiCC4.Visible := aVisible; end;
   4 : begin eMidiCC5.Text := aValue; eMidiCC5.Visible := aVisible; end;
   5 : begin eMidiCC6.Text := aValue; eMidiCC6.Visible := aVisible; end;
   6 : begin eMidiCC7.Text := aValue; eMidiCC7.Visible := aVisible; end;
   7 : begin eMidiCC8.Text := aValue; eMidiCC8.Visible := aVisible; end;
   8 : begin eMidiCC9.Text := aValue; eMidiCC9.Visible := aVisible; end;
   9 : begin eMidiCC10.Text := aValue; eMidiCC10.Visible := aVisible; end;
  10 : begin eMidiCC11.Text := aValue; eMidiCC11.Visible := aVisible; end;
  11 : begin eMidiCC12.Text := aValue; eMidiCC12.Visible := aVisible; end;
  12 : begin eMidiCC13.Text := aValue; eMidiCC13.Visible := aVisible; end;
  13 : begin eMidiCC14.Text := aValue; eMidiCC14.Visible := aVisible; end;
  14 : begin eMidiCC15.Text := aValue; eMidiCC15.Visible := aVisible; end;
  15 : begin eMidiCC16.Text := aValue; eMidiCC16.Visible := aVisible; end;
  end;
end;
...
begin
  ...
  0 : begin
        ...
        Knob := FSynth.SelectedSlot.Patch.GetKnob(KnobIndex);
        if assigned(Knob) and (Knob.IsAssigned = 1) then begin
          aTextFunc := FSynth.SelectedSlot.Patch.GetModuleLabel( TLocationType(Knob.Location), Knob.ModuleIndex);
        end;
      end;
  1 : begin
        KnobIndex := GetKnobIndexOffset + TextField.Tag;
        ...
        if assigned(GlobalKnob) and (GlobalKnob.IsAssigned = 1) then begin
          ...
          aTextFunc := aTextFunc + FSynth.Performance.Slot[GlobalKnob.SlotIndex].Patch.GetModuleLabel( TLocationType(GlobalKnob.Location), GlobalKnob.ModuleIndex);
        end;
      end;
  2 : begin
  ...
  3 : begin
        ...
        0 : Module := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_MASTERCLOCK];
        1 : Module := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_VOICES];
        2 : Module := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_ARPEGGIATOR];
        3 : Module := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_ARPEGGIATOR];
        4 : Module := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_VIBRATO];
        5 : Module := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_GLIDE];
        6 : Module := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_BEND];
        7 : Module := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_VOLUME];
        end;
...
begin
  ...
  3 : begin
        Param := nil;
        case TextField.Tag of
        0 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_MASTERCLOCK].Parameter[1];
        1 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_VOICES].Parameter[1];
        2 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_ARPEGGIATOR].Parameter[ARP_ON_OFF];
        3 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_ARPEGGIATOR].Parameter[ARP_OCTAVES];
        4 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_VIBRATO].Parameter[VIBRATO_MOD];
        5 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_GLIDE].Parameter[GLIDE_TYPE];
        6 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_BEND].Parameter[BEND_ON_OFF];
        7 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_VOLUME].Parameter[VOLUME_MUTE];
        end;
...
begin
  ...
  2 : begin
        //aTextFunc := STD_MORPH_NAMES[TextField.Tag];
        aTextFunc := Patch.PatchPart[ord(ltPatch)].ParameterLabelList.FindParamLabel(PATCH_MORPH, TextField.Tag + 8, 0);
      end;
  3 : begin
        Param := nil;
        case TextField.Tag of
        0 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_MASTERCLOCK].Parameter[0];
        1 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_VOICES].Parameter[0];
        2 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_ARPEGGIATOR].Parameter[ARP_SPEED];
        3 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_ARPEGGIATOR].Parameter[ARP_DIRECTION];
        4 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_VIBRATO].Parameter[VIBRATO_DEPTH];
        5 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_GLIDE].Parameter[GLIDE_SPEED];
        6 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_BEND].Parameter[BEND_RANGE];
        7 : Param := FSynth.SelectedSlot.Patch.Modules[ord(ltPatch), PATCH_VOLUME].Parameter[VOLUME_LEVEL];
        end;
        if assigned(Param) then
          aTextFunc := Param.ParamName;
      end;
...
begin
  ...
  0 : begin
        if assigned(FSynth) then begin
          ...
          rbPageIndex.Value := FSynth.SelectedSlot.Patch.SelectedParamPageColumn;
        end;
        AssignKnobs;
  ...
  for i := 0 to 7 do begin
    DisplayA[i].Redraw;
    DisplayC[i].Redraw;
    visible := not((rbSource.Value=3) and (i in [0,1,7])); // No assignments allowed for clock, voices and volume level
    MidiCC := FframeAppSettings.FindMidiToKnob( FSynth, TG2ParamSource(rbSource.Value), g2ctKnob, KnobOffset + i);
    if MIdiCC = 0 then
      SetEditMidiCC(i, '', visible)
    else
      SetEditMidiCC(i, IntToStr(MidiCC), visible);
    visible := not((rbSource.Value=3) and (i in [0,1])); // No assignments allowed for clock, voices
    MidiCC := FframeAppSettings.FindMidiToKnob( FSynth, TG2ParamSource(rbSource.Value), g2ctButton, KnobOffset + i);
    if MIdiCC = 0 then
