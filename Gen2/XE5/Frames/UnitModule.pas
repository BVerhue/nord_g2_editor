...
uses
  ...
  FMX.StdCtrls, FMX.TabControl, FMX.Edit, FMX.Layouts,
  BVE.NMG2Types, BVE.NMG2USB,  BVE.NMG2GraphFMX, BVE.NMG2ControlsFMX, UnitSlot;
...
type
  TframeModule = class(TFrame, IG2Observer)
    ...
    procedure btModuleColorDefaultChangeValue(Sender: TObject; const aValue: Integer);
...
begin
  if assigned(FframeSlot) and assigned(FframeSlot.PatchPart) then begin
    if assigned( FCopyPatch) then
      FCopyPatch.DisposeOF;

    FCopyPatch := TG2GraphPatchPartFMX.CopyModules( FFrameSlot.Patch, FFrameSlot.PatchPart, FframeSlot.PatchPart.SelectedModuleList);
...
begin
  if assigned(FframeSlot) and assigned(FframeSlot.PatchPart) then begin
    if assigned( FCopyPatch) then begin
      ...
      FframeSlot.ActivateSelection( FCopyPatch,
                                    smCopy,
...
begin
  if assigned(FframeSlot) and assigned(FframeSlot.Patch) then begin
    if assigned( FCopyPatch) and FframeSlot.Patch.CheckParametersFit(FCopyPatch, FframeSlot.Patch.SelectedLocation) then begin
      FframeSlot.Patch.MessCopyParameters( FCopyPatch, FframeSlot.Patch.SelectedLocation, FCopyVariation, FframeSlot.Patch.ActiveVariation);
    end;
  end;
end;
...
begin
  ...
  if FframeSlot.PatchPart.SelectedModuleList.Count = 1 then begin
    Module := FframeSlot.PatchPart.SelectedModuleList[0] as TG2GraphModuleFMX;
    FframeSlot.PatchPart.Patch.MessSetModuleLabel( Module.Location, Module.ModuleIndex, eModuleLabel.Text);
  end;
end;
...
begin
  ...
  if aValue = 0 then
    for i := 0 to FframeSlot.PatchPart.SelectedModuleList.Count - 1 do begin
      Module := FframeSlot.PatchPart.SelectedModuleList[i] as TG2GraphModuleFMX;

      FframeSlot.PatchPart.Patch.MessSetModuleColor( FframeSlot.PatchPart.Location, Module.ModuleIndex, 0);
    end;
end;
...
procedure TframeModule.rbModuleColorPaintElement(Sender: TObject;
  ...
  aStyleSet: TG2StyleSet);
begin
  aStyleSet.Fill.Color := ConvertToAlpha( ModuleColors[ ModuleColorOrder[ aElementIndex + 1]]);
end;
...
begin
  ...
  if FframeSlot.PatchPart.SelectedModuleList.Count = 1 then begin
    ...
    if Module.AssignableKnobCount > 0 then
      (FframeSlot.PatchPart.Patch as TG2GraphPatchFMX).MessModuleAssignGlobalKnobs( Module, aPageIndex);
  end;
end;
...
begin
  ...
  if FframeSlot.PatchPart.SelectedModuleList.Count = 1 then begin
    ...
    if Module.AssignableKnobCount > 0 then
      (FframeSlot.PatchPart.Patch as TG2GraphPatchFMX).MessModuleAssignKnobs( Module, aPageIndex);
  end;
end;
...
begin
  if FCopyPatch <> Value then begin
    FCopyPatch := Value;
    UpdateControls;
  end;
end;
...
begin
  if FframeSlot <> Value then begin
...
begin
  if assigned(FframeSlot) and assigned(FframeSlot.PatchPart) then begin
    if (FframeSlot.PatchPart.SelectedModuleList.Count > 0) then begin
      if FframeSlot.PatchPart.SelectedModuleList.Count = 1 then begin
        eModuleName.Text := FframeSlot.PatchPart.SelectedModuleList[0].ModuleName;
        eModuleLabel.Enabled := True;
        eModuleLabel.Text := FframeSlot.PatchPart.GetModuleLabel(FframeSlot.PatchPart.SelectedModuleList[0].ModuleIndex);;
        ...
        eModuleName.Text := 'Multiple (' + IntToStr(FframeSlot.PatchPart.SelectedModuleList.Count) + ')';
        eModuleLabel.Text := '';
    ...
    if assigned(FCopyPatch) and (FCopyPatch.ModuleList.Count > 0) then begin
      btModulePaste.State := csDefault;
      if FframeSlot.Patch.CheckParametersFit(FCopyPatch, FframeSlot.PatchPart.Location) then
