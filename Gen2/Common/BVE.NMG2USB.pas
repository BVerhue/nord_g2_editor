...
{$ENDIF}
  ...
  public
    ...
    function    AddParamUpdRec( aSubCmd, aLocation, aModule, aParam, aMorph, aValue, aNegative, aVariation : byte): boolean;
...
{$IF Defined(RECIEVEQUEUE)}
  ...
  try
    for i := 0 to RecieveMessageList.Count - 1 do begin
...
var TempBuffer : TJavaArray<Byte>;
begin
  // Write a bulk message over USB unix
...
begin
  ...
  bytes_read := 0;
  buf_position := 0;
  while (retries > 0) and (buf_position < size) {(size <> bytes_read)} and (bytes_read >= 0) do begin
  ...
  if buf_position <> size then begin
...
{$ENDIF}
    MessageList.Delete(0);
    Mess.DisposeOf;
    if bytes_send = g_bout_buf.size then begin
...
begin
  ...
  for SlotIndex := 0 to 3 do begin
    ...
    for i := 0 to Slot.FParamUpdBufCount - 1 do begin
      if (Slot.FParamUpdBuf[i].Changed) and (Slot.FParamUpdBuf[i].Value < 128) then begin
...
begin
  ...
  if (aSendMessage.HasResponse) then begin
...
var MessageList : TList<TG2SendMessage>;
begin
...
begin
  ...
  if FInitMidiStep < G2.MidiToKnobList.Count then begin
            ...
            g2ctKnob :
              begin
                if assigned(Knob) and (Knob.IsAssigned = 1) and assigned(Knob.Parameter) then begin
                  ...
                  if AssignMidiCC( Patch, Knob.Parameter, MidiCC) then begin
                    Result := True;
                    exit;
                  end;
                end else
            ...
            g2ctButton :
              begin
                if assigned(Knob) and (Knob.IsAssigned = 1) and assigned(Knob.Parameter) and assigned(Knob.Parameter.ButtonParam) then begin
                  Patch := Slot[ Knob.SlotIndex].Patch as TG2USBPatch;
                  if AssignMidiCC( Patch, Knob.Parameter.ButtonParam, MidiCC) then begin
                    Result := True;
                    exit;
                  end;
                end else
...
var i : integer;
    ...
    Patch : TG2USBPatch;
begin
  for i := 0 to GlobalKnobList.Count - 1 do begin
        ...
        begin
          ...
          case G2.MidiToKnobList[FInitMidiStep].FControlType of
            g2ctKnob :
              begin
                if assigned(Knob) and (Knob.IsAssigned = 1) and assigned(Knob.Parameter) then begin
                  if AssignMidiCC(Knob.Parameter, MidiCC) then begin
                    Result := True;
                    exit;
                  end;
                end else
            ...
            g2ctButton :
              begin
                if assigned(Knob) and (Knob.IsAssigned = 1) and assigned(Knob.Parameter) and assigned(Knob.Parameter.ButtonParam) then begin
                  if AssignMidiCC(Knob.Parameter.ButtonParam, MidiCC) then begin
                    Result := True;
                    exit;
                  end;
                end else
