...
const
  MIDI_BLOCK_SIZE = 479;
  // 419; // The number of whole octets in  a full size MIDI packet
...
type
  ...
  TCreateModuleEvent = procedure(Sender: TObject; SenderID: Integer;
    Module: TG2FileModule) of object;
  TAddModuleEvent = procedure(Sender: TObject; SenderID: Integer;
    Module: TG2FileModule) of object;
  TDeleteModuleEvent = procedure(Sender: TObject; SenderID: Integer;
    Location: TLocationType; ModuleIndex: Integer) of object;
  TAddCableEvent = procedure(Sender: TObject; SenderID: Integer;
    Module: TG2FileCable) of object;
  TDeleteCableEvent = procedure(Sender: TObject; SenderID: Integer;
    Location: TLocationType; FromModuleIndex, FromConnectorIndex, ToModuleIndex,
    ToConnectorIndex: Integer) of object;
  TAssignKnobEvent = procedure(Sender: TObject; SenderID: Integer; Slot: Byte;
    KnobIndex: Integer) of object;
  TAssignGlobalKnobEvent = procedure(Sender: TObject; SenderID: Integer;
    KnobIndex: Integer) of object;
  TDeassignKnobEvent = procedure(Sender: TObject; SenderID: Integer; Slot: Byte;
    KnobIndex: Integer) of object;
  TDeassignGlobalKnobEvent = procedure(Sender: TObject; SenderID: Integer;
    KnobIndex: Integer) of object;
  TParameterChangeEvent = procedure(Sender: TObject; SenderID: Integer;
    Slot, Variation: Byte; Location: TLocationType;
    ModuleIndex, ParamIndex: Byte; aValue: Byte) of object;
  TMorphChangeEvent = procedure(Sender: TObject; SenderID: Integer; Slot: Byte;
    Location: TLocationType; ModuleIndex, ParamIndex, aMorphIndex,
    aVariationIndex, aValue: Byte) of object;
  TLabelValueChangeEvent = procedure(Sender: TObject; SenderID: Integer;
    Param: TG2FileParameter) of object;
  TModuleModeChangeEvent = procedure(Sender: TObject; SenderID: Integer;
    Slot: Byte; Location: TLocationType; ModuleIndex, ParamIndex: Byte;
    aValue: Byte) of object;
  TSelectSlotEvent = procedure(Sender: TObject; SenderID: Integer;
    Slot: Integer) of object;
  TSelectModuleEvent = procedure(Sender: TObject; SenderID: Integer;
    Module: TG2FileModule) of object;
  TSelectParamEvent = procedure(Sender: TObject; SenderID: Integer;
    Param: TG2FileParameter) of object;
  TLogLineEvent = procedure(Sender: TObject; const LogLine: string;
    LogCmd: Integer) of object;
  TSelectLocationEvent = procedure(Sender: TObject; SenderID: Integer;
    Slot: Byte; Location: TLocationType) of object;
  TLedEvent = procedure(Sender: TObject; const aLocation: TLocationType;
    const aModuleIndex, aGroupIndex, aCodeRef, aValue: Byte) of Object;
  ...
  public
    ...
    function CreateFromMidiStream(const aStream: TStream; aLogLines: TStrings)
      : TMemoryStream;
    procedure CreateMidiBlock(aBlockNr, aBlockCount, anOffset, aSize: Integer;
      aChunk: TPatchChunk; aMidiStream: TStream);
    ...
    class function LoadFileData(AOwner: TComponent; aStream: TStream;
      aLogLines: TStrings): TG2FileDataStream; virtual;
    class function LoadFileStream(AOwner: TComponent; aStream: TStream;
      aChunk: TPatchChunk): TG2FileDataStream; virtual;
    class function LoadMidiData(AOwner: TComponent; aStream: TStream;
      aLogLines: TStrings): TG2FileDataStream;
  ...
  public
    ...
    procedure InitModule(aLocation: TLocationType;
      aModuleType: Integer); virtual;
    ...
    property ModeInfo[Index: Integer]: TBits6 read GetModeInfo
      write SetModeInfo;
    ...
    property InConnector[Index: Integer]: TG2FileConnector
      read GetInputConnector;
    property OutConnector[Index: Integer]: TG2FileConnector
  ...
  public
    ...
    constructor CopySelected(AOwnsObjects: boolean;
      aPatchPart: TG2FilePatchPart; aModuleList: TModuleList);
    destructor Destroy; override;
    procedure Init;
    ...
    property Items[aIndex: Integer]: TG2FileModule read GetModule
  ...
  public
    ...
    property FromConnector: TG2FileConnector read FFromConnector
  ...
  public
    ...
    constructor CopySelected(AOwnsObjects: boolean;
      aPatchPart: TG2FilePatchPart; aModuleList: TModuleList;
      aCableList: TCableList);
    destructor Destroy; override;
    procedure Init;
    procedure DeleteCable(aCable: TG2FileCable);
    procedure Read(aChunk: TPatchChunk);
    procedure Write(aChunk: TPatchChunk);
    function FindCable(FromModule: Byte; FromConnector: Byte; ToModule: Byte;
      ToConnector: Byte): TG2FileCable;
    function AddCable(aValue: TG2FileCable): Integer;
    property Items[aIndex: Integer]: TG2FileCable read GetCable
      write SetCable; default;
  ...
  public
    constructor Create(AOwnsObjects: boolean; aPatchPart: TG2FilePatchPart);
    constructor Copy(AOwnsObjects: boolean;
      aModuleVariationList: TModuleVariationList);
    destructor Destroy; override;
    procedure Init;
  ...
  private
    ...
    procedure SetModuleVariationList(aIndex: Integer;
      const aValue: TModuleVariationList);
  public
    constructor Create(AOwnsObjects: boolean; aPatchPart: TG2FilePatchPart);
    constructor CopySelected(AOwnsObjects: boolean;
      aPatchPart: TG2FilePatchPart; aModuleList: TModuleList;
      aModuleParameterList: TModuleParameterList);
    destructor Destroy; override;
    procedure Init;
    procedure Read(aChunk: TPatchChunk);
    ...
    procedure DeleteModule(aModuleIndex: Byte);
    procedure CopyVariation(aFromVariation, aToVariation: Byte);
    function FindModuleVariationList(aModuleIndex: Integer)
      : TModuleVariationList;
    function FindParamValue(aModuleIndex, aVariation,
      aParamIndex: Integer): TBits7;
    function AddModuleVariationList(aValue: TModuleVariationList): Integer;
    property Items[aIndex: Integer]: TModuleVariationList
      read GetModuleVariationList write SetModuleVariationList; default;
  ...
  public
    ...
    procedure AddNewMorphParam(aLocation, aModuleIndex, aParamIndex,
      aMorphIndex, aRange: Byte);
    procedure DelMorphParam(aLocation, aModuleIndex, aParamIndex,
      aMorphIndex: Byte);
    function FindMorphParam(aLocation: TLocationType;
      aModuleIndex, aParamIndex, aMorphIndex, aVariation: Byte)
      : TMorphParameter;
    procedure DeleteModule(aLocation, aModuleIndex: Byte);
    function AddMorphParam(aValue: TMorphParameter): Integer;
    property Items[aIndex: Integer]: TMorphParameter read GetMorphParam
      write SetMorphParam; default;
  end;
  ...
  public
    ...
    property KnobFloatValue: Single read GetKnobFloatValue
      write SetKnobFloatValue;
    property KnobButtonValue: Byte read GetKnobButtonValue
      write SetKnobButtonValue;
    property KnobButtonFloatValue: Single read GetKnobButtonFloatValue
      write SetKnobButtonFloatValue;
  ...
  public
    ...
    procedure Read(aChunk: TPatchChunk);
    procedure Write(aChunk: TPatchChunk);
    function FindKnob(aLocation: TLocationType;
      aModuleIndex, aParamIndex: Byte): Integer;
  ...
  public
    ...
    procedure Read(aChunk: TPatchChunk);
    procedure Write(aChunk: TPatchChunk);
    function FindMidiCC(aLocation: TLocationType;
      aModuleIndex, aParamIndex: Byte): Byte;
    function AddController(aValue: TController): Integer;
    property Items[aIndex: Integer]: TController read GetController
      write SetController; default;
  end;
  ...
  public
    constructor Create(AOwnsObjects: boolean; aPatchPart: TG2FilePatchPart);
    constructor CopyCreate(AOwnsObjects: boolean; aPatchPart: TG2FilePatchPart;
      aParamLabelParam: TParamLabelParamList);
    destructor Destroy; override;
    procedure Init;
    procedure Read(aChunk: TPatchChunk);
    procedure Write(aChunk: TPatchChunk);
    function AddParamLabel(aValue: TParamLabel): Integer;
    property Items[aIndex: Integer]: TParamLabel read GetParamLabel
      write SetParamLabel; default;
  ...
  private
    ...
    procedure SetParamLabelParam(aIndex: Integer;
      const aValue: TParamLabelParamList);
  public
    constructor Create(AOwnsObjects: boolean; aPatchPart: TG2FilePatchPart);
    constructor CopyCreate(AOwnsObjects: boolean; aPatchPart: TG2FilePatchPart;
      aParamLabelModule: TParamLabelModuleList);
    destructor Destroy; override;
    procedure Init;
    procedure InitParamLabelModuleList(aModuleIndex: Byte;
      aModuleType: Integer);
    ...
    property Items[aIndex: Integer]: TParamLabelParamList
      read GetParamLabelParam write SetParamLabelParam; default;
    property ModuleIndex: TBits8 read FModuleIndex write FModuleIndex;
  end;
  ...
  private
    ...
    procedure SetParamLabelModule(aIndex: Integer;
      const aValue: TParamLabelModuleList);
  public
    constructor Create(AOwnsObjects: boolean; aPatchPart: TG2FilePatchPart);
    constructor CopySelected(AOwnsObjects: boolean;
      aPatchPart: TG2FilePatchPart; aModuleList: TModuleList;
      aParameterLabels: TParameterLabelList);
    destructor Destroy; override;
    function FindParamLabelModule(aModuleIndex: Integer): TParamLabelModuleList;
    function FindParamLabel(aModuleIndex, aParamIndex,
      aLabelIndex: Byte): string;
    procedure AddParamLabel(aModuleIndex, aParamIndex, aLabelIndex: Byte;
      aName: string);
    ...
    property Items[aIndex: Integer]: TParamLabelModuleList
      read GetParamLabelModule write SetParamLabelModule; default;
  end;
  ...
  public
    ...
    constructor CopySelected(AOwnsObjects: boolean;
      aPatchPart: TG2FilePatchPart; aModuleList: TModuleList;
      aModuleLabels: TModuleLabelList);
    destructor Destroy; override;
    procedure Init;
    ...
    property Items[aIndex: Integer]: TModuleLabel read GetModuleLabel
      write SetModuleLabel; default;
    property Unknown: TBits6 read FUnknown write FUnknown;
  end;

  TPatchDescription = class
  private
    FUnknown1, FUnknown2, FUnknown3, FUnknown4, FUnknown5, FUnknown6,
  ...
  public
    ...
    constructor CopyModules(aPatch: TG2FilePatch;
      aSrcePatchPart: TG2FilePatchPart; aModuleList: TModuleList);
    destructor Destroy; override;
    procedure Init;
    procedure Read(aChunk: TPatchChunk);
    ...
    function GetParameterLabel(aModuleIndex, aParamIndex,
      aLabelIndex: Byte): string;
    procedure SetParameterLabel(aModuleIndex, aParamIndex, aLabelIndex: Byte;
      aValue: string);
    function GetParameterLabelCount(aModuleIndex, aParamIndex: Byte): Integer;
    ...
    function CreateCable(aColor: Byte; aFromModule: Byte; aFromConnector: Byte;
      aLinkType: Byte; aToModule: Byte; aToConnector: Byte)
      : TG2FileCable; virtual;
    procedure InvalidateParameters;
    procedure InvalidateConnectors;
    ...
    property SelectedParam: TG2FileParameter read GetSelectedParam;
    property FocusedModule: TG2FileModule read FFocusedModule
      write SetFocusedModule;
  ...
  private
    ...
    procedure DoMorphChange(Slot: Byte; Location: TLocationType;
  ...
  public
    constructor Create(AOwner: TComponent); override;
    constructor CopyModules(AOwner: TComponent; aPatch: TG2FilePatch;
      aModuleList: TModuleList);
    destructor Destroy; override;
    procedure Init; virtual;
    function ReadChunk(aChunk: TPatchChunk): boolean;
    ...
    function GetModule(LocationIndex, ModuleIndex: Integer): TG2FileModule;
    // virtual;
    ...
    function GetNoOffModuleType(aLocation: TLocationType;
      aModuleType: Byte): Integer;
    function GetNoOffExtendedModules: Integer;
    // Number of modules that are not compatible with original editor
    function GetActiveVariation: Byte;
    function GetModuleName(aLocation: TLocationType;
      aModuleIndex: Byte): string;
    function GetParameterValue(aLocation: TLocationType; aModuleIndex: Byte;
      aParamIndex: Byte; aVariation: Byte): Byte;
    procedure SetParamInPatch(aLocation: TLocationType; aModuleIndex: Byte;
      aParamIndex: Byte; aVariation: Byte; aValue: Byte); virtual;
    procedure SetParamValue(aLocation: TLocationType;
      aModuleIndex, aParamIndex, aVariation, aValue: Byte); virtual;
    procedure SelectParamInPatch(aLocation: TLocationType; aModuleIndex: Byte;
      aParameterIndex: Byte);
    procedure SelectParam(aLocation: TLocationType; aModuleIndex: Byte;
      aParameterIndex: Byte); virtual;
    procedure SetModeInPatch(aLocation: TLocationType; aModuleIndex: Byte;
      aParamIndex, aValue: Byte); virtual;
    function GetModeValue(aLocation: TLocationType; aModuleIndex: Byte;
      aParamIndex: Byte): Byte;
    procedure SetModeValue(aLocation: TLocationType; aModuleIndex: Byte;
      aParamIndex: Byte; aValue: Byte); virtual;
    procedure SetMorphValue(aLocation: TLocationType; aModuleIndex: Byte;
      aParamIndex: Byte; aMorphIndex: Byte; aValue: Byte;
      aVariation: Byte); virtual;
    function GetMorphValue(aLocation: TLocationType; aModuleIndex: Byte;
      aParamIndex: Byte; aMorphIndex: Byte; aVariation: Byte): Byte;
...
{$ENDIF}
    ...
    function CreatePatchPart(aLocation: TLocationType)
      : TG2FilePatchPart; virtual;
    function CreateModule(aLocation: TLocationType; aModuleIndex: Byte;
      aModuleType: Byte): TG2FileModule; virtual;
    function CreateCable(aLocation: TLocationType; aColor: Byte;
      aFromModule: Byte; aFromConnector: Byte; aLinkType: Byte; aToModule: Byte;
      aToConnector: Byte): TG2FileCable; virtual;
    function CreateParameter(aModuleIndex: Byte): TG2FileParameter; virtual;
    function CreateLed(const aLedType: TLedType; const aLocation: TLocationType;
      const aModuleIndex, aGroupID: Byte; const aGroupCount: Integer)
      : IG2DataLed; virtual;
    procedure CreateLeds(const aLocation: TLocationType;
      const aModuleIndex, aModuleType: Byte);
    procedure AddModuleToPatch(aLocation: TLocationType;
      aModule: TG2FileModule);
    procedure DeleteModuleFromPatch(aLocation: TLocationType;
      aModule: TG2FileModule);
    procedure AddCableToPatch(aLocation: TLocationType; aCable: TG2FileCable);
    procedure DeleteCableFromPatch(aLocation: TLocationType;
      aCable: TG2FileCable); virtual;
    function FindKnob(aLocation: TLocationType;
      aModuleIndex, aParamIndex: Byte): Integer;
    function FindMidiCC(aLocation: TLocationType;
      aModuleIndex, aParamIndex: Byte): Byte;
    function AssignKnobInPatch(aKnobIndex: Integer; aLocation: TLocationType;
      aModuleIndex, aParamIndex: Byte): TKnob;
    ...
    function AssignMidiCCInPatchX(aMidiCC: Byte; aLocation: TLocationType;
      aModuleIndex, aParamIndex: Byte): TController;
    procedure DeassignMidiCCInPatchX(aMidiCC: Byte);
    function GetMorph(aLocation: TLocationType;
      aModuleIndex, aParamIndex, aMorphIndex, aVariation: Byte)
      : TMorphParameter;
    function HasMorph(aLocation: TLocationType;
      aModuleIndex, aParamIndex, aVariation: Byte): boolean;
    procedure CopyVariation(aFromVariation, aToVariation: Byte);
    function GetParameterLabel(aLocation: TLocationType;
      aModuleIndex, aParamIndex, aLabelIndex: Byte): string;
    procedure SetParameterLabel(aLocation: TLocationType;
      aModuleIndex, aParamIndex, aLabelIndex: Byte; aValue: string);
    function GetParameterLabelCount(aLocation: TLocationType;
      aModuleIndex, aParamIndex: Byte): Integer;
    function GetModuleLabel(aLocation: TLocationType;
      aModuleIndex: Byte): string;
    procedure SetModuleLabel(aLocation: TLocationType; aModuleIndex: Byte;
      aValue: string);
    procedure UnselectModules(aLocation: TLocationType);
    function MessAddModule(aLocation: TLocationType;
      aModuleTypeID, aCol, aRow: Byte): boolean; virtual;
    function MessCopyModules(aSrcePatch: TG2FilePatchPart;
      aFromLocation, aToLocation: TLocationType): boolean; virtual;
    // Abstract functions
    function MessCopyParameters(aSrcePatch: TG2FilePatchPart;
      aToLocation: TLocationType; aFromVariation, aToVariation: Integer)
      : boolean; virtual;
    function MessAddConnection(aLocation: TLocationType;
      aFromConnection, aToConnection: TG2FileConnector): boolean; virtual;
    function MessDeleteConnection(aLocation: TLocationType;
      aCable: TG2FileCable): boolean; virtual;
    function MessDeleteModule(aLocation: TLocationType; aModuleIndex: Byte)
      : boolean; virtual;
    function MessDeleteModules(aLocation: TLocationType): boolean; virtual;
    function MessMoveModule(aLocation: TLocationType;
      aModuleIndex, aCol, aRow: Byte): boolean; virtual;
    function MessSetModuleColor(aLocation: TLocationType;
      aModuleIndex, aColor: Byte): boolean; virtual;
    function MessSetModuleParamLabels(aLocation: TLocationType;
      aModuleIndex, aParamIndex, aLabelIndex: Byte; aName: string)
      : boolean; virtual;
    function MessSetModuleLabel(aLocation: TLocationType; aModuleIndex: Byte;
      aName: string): boolean; virtual;
    property PatchName: string read GetPatchName write SetPatchName;
    property ActiveVariation: Byte read GetActiveVariation;
    // TODO Don't remember what....
    property Variation: Byte read GetVariation write SetVariation;
    property VariationCount: Integer read GetVariationCount;
    property SelectedMorphIndex: Integer read FSelectedMorphIndex
      write SetSelectedMorphIndex;
    property Modules[LocationIndex, ModuleIndex: Integer]: TG2FileModule
      read GetModule;
    property ModuleCount[LocationIndex: Integer]: Integer read GetModuleCount;
    property EditAllVariations: boolean read FEditAllVariations
      write FEditAllVariations;
    ...
    property ParameterList[Index: Integer]: TModuleParameterList
      read GetParameterList;
    property ModuleLabelList[Index: Integer]: TModuleLabelList
      read GetModuleLabelList;
    property ParameterLabelList[Index: Integer]: TParameterLabelList
      read GetParameterLabelList;
    ...
    property SelectedLocation: TLocationType read FSelectedLocation
      write SetSelectedLocation;
    property SelectedParamPage: Integer read FSelectedParamPage
      write FSelectedParamPage;
    property SelectedParamPageColumn: Integer read FSelectedParamPageColumn
      write FSelectedParamPageColumn;
    ...
    property Performance: TG2FilePerformance read FPerformance
      write SetPerformance;
  ...
  public
    constructor Create(aPatch: TG2FilePatch; aLocation: TLocationType;
      aModule: TG2FileModule); reintroduce; virtual;
    destructor Destroy; override;
    procedure InitConnector(aModuleType, aConnectorIndex: Byte;
    ...
    property ConnectorKind: TConnectorKind read FConnectorKind
      write SetConnectorKind;
    property ConnectorColor: Byte read GetConnectorColor;
    property ConnectorDefColor: Byte read FConnectorDefColor
      write SetConnectorDefColor;
    property ConnectorIndex: Byte read FConnectorIndex write FConnectorIndex;
    property ConnectorType: TConnectorType read FConnectorType
  ...
  private
    ...
    FDefaultKnob: Integer;
    // The knob nr, when a module is assigned to a parameter page
    FButtonParamIndex: Integer;
    // The parameter that is assigned to the button below the knob on the param page
    ...
    FSuspendUpdate: boolean;
    // Prevent large amount of updates for none-responseless updates (virtual params: Masterclock/Voices)
  ...
  public
    constructor Create(aPatch: TG2FilePatch; aLocation: TLocationType;
      aModuleIndex: Integer; aModule: TG2FileModule); reintroduce;
    destructor Destroy; override;
    procedure RegisterObserver(aObserver: IG2Observer);
    procedure RemoveObserver(aObserver: IG2Observer);
    procedure NotifyObservers(aG2Event: TG2Event);
    procedure InitParam(aID: Integer; aModuleName: string; aParamIndex: Byte;
      aParamType: TParamType; aParamName, aDefaultParamLabel: string;
    ...
    procedure AssignGlobalKnob(aPerf: TG2FilePerformance; aSlotIndex: Byte;
      aKnobIndex: Integer);
    procedure DeassignGlobalKnob(aPerf: TG2FilePerformance;
      aKnobIndex: Integer);
    ...
    procedure SetMorphValue(const aMorphIndex, aVariation: Integer;
      const aValue: Byte);
    ...
    function DelayDispValue(aType: Integer; aRange: Integer;
    ...
    property ParamLabel[index: Integer]: string read GetParamLabel
      write SetParamLabel;
    ...
    property InfoFunctionIndex: Integer read FInfoFunctionIndex
      write FInfoFunctionIndex;
    property TextFunctionIndex: Integer read FTextFunctionIndex
      write FTextFunctionIndex;
    property ButtonParamIndex: Integer read FButtonParamIndex
      write FButtonParamIndex;
    property DefaultKnob: Integer read FDefaultKnob write FDefaultKnob;
    property ButtonTextList: TStrings read FButtonText;
    property ButtonText[index: Integer]: string read GetButtonText
      write SetButtonText;
  ...
  public
    constructor Create(const aLedType: TLedType; const aLocation: TLocationType;
      const aModuleIndex, aGroupID: Byte; const aGroupCount: Integer);
    destructor Destroy; override;
    procedure RegisterObserver(aObserver: IG2Observer);
    procedure RemoveObserver(aObserver: IG2Observer);
  ...
  private
    ...
    [Weak] FG2: TG2File;
    FObserverList: TList<IG2Observer>;
    FUnknown1, FUnknown2, FUnknown3, FUnknown4: TBits8; // array[0..4] of
    FEnabled: TBits8;
    FKeyboard: TBits8;
    FHold: TBits8;
  ...
  public
    ...
    destructor Destroy; override;
    procedure Init;
    function FindGlobalKnobIndex(aSlotIndex: Byte; aLocation: TLocationType;
      aModuleIndex, aParamIndex: Byte): Integer;
    procedure DeleteModule(aSlotIndex, aLocation, aModuleIndex: Byte);
    procedure Read(aChunk: TPatchChunk);
    procedure Write(aChunk: TPatchChunk);
    function AddGlobalKnob(aValue: TGlobalKnob): Integer;
    property Items[aIndex: Integer]: TGlobalKnob read GetGlobalKnob
      write SetGlobalKnob; default;
  end;
  ...
  private
    ...
    FUnknown1, FUnknown2, FUnknown3, FUnknown4, FUnknown5, FUnknown6, FUnknown7,
      FUnknown8, FUnknown9: TBits8; // array[0..8] of
...
{$ENDIF}
    ...
    function GetNoOffExtendedModules: Integer;
    // Number of modules that are not compatible with original editor
    procedure DeleteModuleFromPerf(aSlotIndex: Byte; aLocation: TLocationType;
      aModule: TG2FileModule);
    function AssignGlobalKnobInPerf(aKnobIndex: Integer; aSlotIndex: Byte;
      aLocation: TLocationType; aModuleIndex, aParamIndex: Byte): TGlobalKnob;
    procedure DeassignGlobalKnobInPerf(aKnobIndex: Integer);
    function GetGlobalKnob(KnobIndex: Integer): TGlobalKnob;
    property PerformanceName: string read FPerformanceName
      write SetPerformanceName;
    ...
    property MasterClockRun: TBits8 read FMasterClockRun
      write SetMasterClockRun;
    property KeyboardRangeEnabled: TBits8 read FKeyboardRangeEnabled
      write SetKeyboardRangeEnabled;
  ...
  public
    ...
    destructor Destroy; override;
    function FindIndex(aPatchFileType: TPatchFileType;
      aBank, aPatch: Byte): Integer;
    ...
    function FindFirstLast(aPatchFileType: TPatchFileType; aBank: Byte;
      var aFirstLocation, aLastLocation: Byte): boolean;
    function AddBankItem(aValue: TBankItem): Integer;
    property Items[aIndex: Integer]: TBankItem read GetBankItem
      write SetBankItem; default;
  end;
  ...
  public
    ...
    procedure AddMidiToKnob(aSource: TG2ParamSource;
      aControlType: TG2ControlType; aKnob: Integer; aMidiCC: Byte);
    procedure DeleteMidiToKnob(aSource: TG2ParamSource;
      aControlType: TG2ControlType; aKnob: Integer);
    function FindMidiToKnob(aSource: TG2ParamSource;
    ...
    procedure ClearLog;
    function G2MessageDlg(tekst: string; code: Integer): Integer; virtual;
    // For sending messages to UI
  ...
  published
    ...
    property OnCreateModule: TCreateModuleEvent read FOnCreateModule
      write FOnCreateModule;
    ...
    property OnDeleteCable: TDeleteCableEvent read FOnDeleteCable
      write FOnDeleteCable;
    property OnAssignKnob: TAssignKnobEvent read FOnAssignKnob
      write FOnAssignKnob;
    property OnDeassignKnob: TDeassignKnobEvent read FOnDeassignKnob
      write FOnDeassignKnob;
    property OnAssignGlobalKnob: TAssignGlobalKnobEvent read FOnAssignGlobalKnob
      write FOnAssignGlobalKnob;
    property OnDeassignGlobalKnob: TDeassignGlobalKnobEvent
      read FOnDeassignGlobalKnob write FOnDeassignGlobalKnob;
    property OnParameterChange: TParameterChangeEvent read FOnParameterChange
      write FOnParameterChange;
    property OnMorphChange: TMorphChangeEvent read FOnMorphChange
      write FOnMorphChange;
    property OnLabelValueChange: TLabelValueChangeEvent read FOnLabelValueChange
      write FOnLabelValueChange;
    property OnModuleModeChange: TModuleModeChangeEvent read FOnModuleModeChange
      write FOnModuleModeChange;
    property OnSelectSlot: TSelectSlotEvent read FOnSelectSlot
      write FOnSelectSlot;
    property OnSelectLocation: TSelectLocationEvent read FOnSelectLocation
      write FOnSelectLocation;
    property OnSelectModule: TSelectModuleEvent read FOnSelectModule
      write FOnSelectModule;
    property OnSelectParam: TSelectParamEvent read FOnSelectParam
      write FOnSelectParam;
...
begin
  ...
  Result := TG2FileParameter.Create(Patch, TLocationType(FLocation),
    FModuleIndex, self);
end;

function TG2FileModule.CreateConnector: TG2FileConnector;
begin
  Result := TG2FileConnector.Create(FPatchPart.FPatch,
    TLocationType(FLocation), self);
end;
...
procedure TG2FileModule.InitModule(aLocation: TLocationType;
  aModuleType: Integer);
var
  i, mi, pi, pdi, count: Integer;
begin
  ...
  begin
    ...
    FParamArray[i].InitParam(ModuleParams[pi].ParamID,
      ModuleDefs[mi].ModuleName, i, ptParam, ModuleParams[pi].ParamName,
      ModuleParams[pi].slParamLabel, ParamDefs[pdi].LowValue,
      ParamDefs[pdi].HighValue, ModuleParams[pi].DefaultValue,
  ...
  begin
    ...
    FModeArray[i].InitParam(ModuleParams[pi].ParamID, ModuleDefs[mi].ModuleName,
      i, ptMode, ModuleParams[pi].ParamName, ModuleParams[pi].slParamLabel,
      ParamDefs[pdi].LowValue, ParamDefs[pdi].HighValue,
      ModuleParams[pi].DefaultValue, -1, -1, ParamDefs[pdi].slButtonText);
...
begin
  ...
  if assigned(FPatchPart) then
  begin
    for i := 0 to FPatchPart.ModuleList.count - 1 do
    begin
      if (FPatchPart.ModuleList.Items[i].FCol = FCol) and
        (FPatchPart.ModuleList.Items[i].FRow < FRow) then
        inc(Result);
    end;
  end;
end;
...
begin
  ...
  if assigned(aChunk.FLogLines) then
    aChunk.FLogLines.Add(Format('%3d %3d %3d %3d %3d %3d %3d %3d %3d',
      [TypeID, ModuleIndex, Row, Col, ModuleColor, Uprate, IsLed, FUnknown1,
      FModeCount]));
  SetLength(FModeInfoArray, FModeCount);
  for i := 0 to Length(FModeInfoArray) - 1 do
...
constructor TModuleList.Create(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart);
begin
  inherited Create(AOwnsObjects);
  FPatchPart := aPatchPart;
end;

constructor TModuleList.CopySelected(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart; aModuleList: TModuleList);
var
...
begin
  Result := nil;
  i := 0;
  while (i < count) and not((Items[i].Col = aCol) and (aRow >= Items[i].Row) and
    (aRow < (Items[i].Row + Items[i].HeightUnits))) do
    inc(i);
  if (i < count) then
    Result := Items[i];
end;
...
begin
  ...
  for i := 0 to count - 1 do
  begin
    if Items[i].ParameterCount > 0 then
    begin
      if Items[i].Row < Module.Row then
      begin
...
begin
  ...
  begin
    if Items[i].ParameterCount > 0 then
    begin
      if Items[i].Col < Module.Col then
      begin
        dx := Items[i].Col * UNITS_COL - Module.Col * UNITS_COL;
        dy := Items[i].Row * UNITS_ROW - Module.Row * UNITS_ROW;
        if (ds = 0) or (sqrt(dx * dx + dy * dy) < ds) then
...
constructor TG2FileCable.CopyCreate(aPatchPart: TG2FilePatchPart;
  aCable: TG2FileCable);
begin
...
constructor TCableList.Create(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart);
begin
  inherited Create(AOwnsObjects);
  Init;
...
constructor TCableList.CopySelected(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart; aModuleList: TModuleList;
  aCableList: TCableList);
var
  i: Integer;
...
begin
  ...
  begin
    ...
    if assigned(ModuleFrom) and assigned(ModuleTo)
    { and (ModuleFrom.FSelected) and (ModuleTo.FSelected) } then
    begin
...
begin
  ...
  begin
    ...
    if assigned(aChunk.FLogLines) then
      aChunk.FLogLines.Add(Format('%3d %3d %3d %3d %3d %3d', [Color, ModuleFrom,
        ConnectorFrom, LinkType, ModuleTo, ConnectorTo]));
    j := 0;
    while (j < count) and not((Items[j].ModuleFrom = ModuleFrom) and
      (Items[j].ConnectorFrom = ConnectorFrom) and
      (Items[j].ModuleTo = ModuleTo) and
      (Items[j].ConnectorTo = ConnectorTo)) do
      inc(j);
    if j < count then
    begin
      ...
      Add(FPatchPart.CreateCable(Color, ModuleFrom, ConnectorFrom, LinkType,
        ModuleTo, ConnectorTo));
  end;
  FCableCount := count;
end;
...
function TCableList.FindCable(FromModule: Byte; FromConnector: Byte;
  ToModule: Byte; ToConnector: Byte): TG2FileCable;
var
  i: Integer;
begin
  i := 0;
  while (i < count) and not((Items[i].FModuleFrom = FromModule) and
    (Items[i].FConnectorFrom = FromConnector) and
    (Items[i].FModuleTo = ToModule) and
    (Items[i].FConnectorTo = ToConnector)) do
    inc(i);
  if (i < count) then
    Result := Items[i]
  else
...
begin
  ...
  begin
    ...
    Note.Read(aChunk);
    if assigned(aChunk.FLogLines) then
      aChunk.FLogLines.Add(Format('%3d %3d %3d', [Note.FNote, Note.FAttack,
        Note.FRelease]));
    Add(Note);
  end;
end;
...
begin
  ...
  for i := 0 to FPatchPart.VariationCount - 1 do
  begin
    j := 0;
    ...
    for k := 0 to Length(ModuleParamValues.FParamValueArray) - 1 do
    begin
...
constructor TModuleVariationList.Copy(AOwnsObjects
  : boolean { ; aModuleParameterList : TModuleParameterList };
  aModuleVariationList: TModuleVariationList);
var
  i, j: Integer;
begin
  ...
  for i := 0 to aModuleVariationList.count - 1 do
  begin
    j := 0;
    while (j < count) and not(Items[j].FVariation = aModuleVariationList[i]
      .FVariation) do
      inc(j);
    if j < count then
    begin
...
begin
  ...
  begin
    ModuleParamValues := TModuleParamValues.Create { (self) };
    // ModuleParamValues.FVariation := FModuleParameterList.FVariationCount - 1;
    // FModuleParameters.FVariationCount must already be increased
...
begin
  ...
  if assigned(aChunk.FLogLines) then
  begin
    aChunk.FLogLines.Add('Module id = ' + IntToStr(FModuleIndex) +
      ', param cnt = ' + IntToStr(FParameterCount));
    ...
    begin
      ...
      for j := 0 to Length(ModuleParamValues.FParamValueArray) - 1 do
        param_str := param_str + IntToStr(ModuleParamValues.FParamValueArray
          [j]) + ' ';
      aChunk.FLogLines.Add(Format('%3d', [ModuleParamValues.FVariation]) + ' ' +
        param_str);
    end;
  end;
  i := aVariationCount;
...
constructor TModuleParameterList.Create(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart);
begin
  inherited Create(AOwnsObjects);
  FPatchPart := aPatchPart;
...
constructor TModuleParameterList.CopySelected(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart; aModuleList: TModuleList;
  aModuleParameterList: TModuleParameterList);
var
  i, j: Integer;
...
begin
  ...
  for i := 0 to aModuleParameterList.count - 1 do
  begin
    ...
    begin
      ...
      while (j < count) and not(Items[j].ModuleIndex = Module.ModuleIndex) do
        inc(j);
      if j >= count then
        Add(TModuleVariationList.Copy(True,
          { self, } aModuleParameterList.Items[i]));
    end;
  end;
end;
...
function TModuleParameterList.GetModuleVariationList(aIndex: Integer)
  : TModuleVariationList;
begin
  Result := TModuleVariationList(inherited Items[aIndex]);
end;

procedure TModuleParameterList.SetModuleVariationList(aIndex: Integer;
  const aValue: TModuleVariationList);
begin
  inherited Items[aIndex] := aValue;
end;

function TModuleParameterList.AddModuleVariationList
  (aValue: TModuleVariationList): Integer;
begin
  Result := inherited Add(aValue);
...
begin
  ...
  begin
    ...
    aChunk.FLogLines.Add('Num sets = ' + IntToStr(ModuleCount) + ', var cnt = '
      + IntToStr(VariationCount));
    aChunk.FLogLines.Add('Var Params');
  end;
  // for i := 0 to FModuleVariationListCount - 1 do begin
  ...
  begin
    ...
    begin
      ModuleVariationList := TModuleVariationList.Create(True,
        FPatchPart { , self } );
...
procedure TModuleParameterList.Write(aChunk: TPatchChunk;
...
procedure TModuleParameterList.CopyVariation(aFromVariation,
  aToVariation: Byte);
var
  i: Integer;
begin
...
function TModuleParameterList.FindModuleVariationList(aModuleIndex: Integer)
  : TModuleVariationList;
var
  i: Integer;
begin
...
function TModuleParameterList.FindParamValue(aModuleIndex, aVariation,
  aParamIndex: Integer): TBits7;
var
  ModuleVariationList: TModuleVariationList;
  ModuleParamValues: TModuleParamValues;
begin
  ...
  if assigned(ModuleVariationList) then
  begin
...
begin
  ...
  if assigned(aChunk.FLogLines) then
  begin
    aChunk.FLogLines.Add('Morph param list, varcount : ' +
  ...
  begin
    ...
    if assigned(aChunk.FLogLines) then
    begin
      aChunk.FLogLines.Add('Morph params, variation : ' +
    ...
    begin
      ...
      if assigned(aChunk.FLogLines) then
        aChunk.FLogLines.Add(Format('%3d %3d %3d %3d %3d',
          [Location, ModuleIndex, ParamIndex, Morph, Range]));
      k := 0;
      while (k < PatchParamList.count) and
        not((Location = PatchParamList[k].FLocation) and
        (ModuleIndex = PatchParamList[k].FModuleIndex) and
        (ParamIndex = PatchParamList[k].FParamIndex) and
        (Morph = PatchParamList[k].FMorph)) do
        inc(k);
...
procedure TPatchParamList.SetMorphParam(aIndex: Integer;
  const aValue: TMorphParameter);
begin
  inherited Items[aIndex] := aValue;
end;
...
function TPatchParamList.FindMorphParam(aLocation: TLocationType;
  aModuleIndex, aParamIndex, aMorphIndex, aVariation: Byte): TMorphParameter;
var
  i: Integer;
begin
  Result := nil;
  i := 0;
  while (i < count) and not((Items[i].FLocation = TBits2(aLocation)) and
    (Items[i].FModuleIndex = aModuleIndex) and
    (Items[i].FParamIndex = aParamIndex) and (Items[i].FMorph = aMorphIndex)) do
    inc(i);
  if (i < count) then
    Result := Items[i];
end;

procedure TPatchParamList.AddNewMorphParam(aLocation, aModuleIndex, aParamIndex,
  aMorphIndex, aRange: Byte);
var
  MorphParam: TMorphParameter;
begin
...
procedure TPatchParamList.DelMorphParam(aLocation, aModuleIndex, aParamIndex,
  aMorphIndex: Byte);
var
  i: Integer;
begin
...
procedure TPatchSettings.SetPatchParamList(aVariation: Integer;
...
begin
  ...
  if (FAssigned = 1) and assigned(FParameter) then
    if FParameter.HighValue - FParameter.LowValue <> 0 then
      Result := FParameter.GetValue /
        (FParameter.HighValue - FParameter.LowValue);
end;
  ...
  begin
    ...
    begin
      if ButtonParam.HighValue - ButtonParam.LowValue <> 0 then
        Result := ButtonParam.GetValue /
          (ButtonParam.HighValue - ButtonParam.LowValue);
    end;
  end;
end;
...
begin
  for i := 0 to count - 1 do
    if (Items[i].FModuleIndex = aModuleIndex) and
      (Items[i].FLocation = aLocation) then
      Items[i].FAssigned := 0;
end;
...
begin
  ...
  begin
    ...
    if assigned(aChunk.FLogLines) and (Items[i].FAssigned = 1) then
      aChunk.FLogLines.Add(Format('%3d %3d %3d %3d %3d', [i, Items[i].FLocation,
        Items[i].FModuleIndex, Items[i].FIsLed, Items[i].FParamIndex]));
  end;
end;
...
function TKnobList.FindKnob(aLocation: TLocationType;
  aModuleIndex, aParamIndex: Byte): Integer;
var
  i: Integer;
begin
  i := 0;
  while (i < count) and not((Items[i].FAssigned = 1) and
    (Items[i].FLocation = TBits2(aLocation)) and
    (Items[i].FModuleIndex = aModuleIndex) and
...
procedure TControllerList.SetController(aIndex: Integer;
  const aValue: TController);
begin
  inherited Items[aIndex] := aValue;
end;
...
begin
  ...
  if assigned(aChunk.FLogLines) then
  begin
    aChunk.FLogLines.Add('Controller list, count = ' +
      IntToStr(FControllerCount));
    aChunk.FLogLines.Add('CC# Loc Mod Par');
  end;
  for i := 0 to FControllerCount - 1 do
  begin
    Controller := TController.Create;
    Controller.Read(aChunk);
    if assigned(aChunk.FLogLines) then
      aChunk.FLogLines.Add(Format('%3d %3d %3d %3d', [Controller.FMidiCC,
        Controller.FLocation, Controller.FModuleIndex,
        Controller.FParamIndex]));
    Add(Controller);
  end;
end;
...
function TControllerList.FindMidiCC(aLocation: TLocationType;
  aModuleIndex, aParamIndex: Byte): Byte;
var
  i: Integer;
begin
...
constructor TParamLabelParamList.Create(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart);
begin
  inherited Create(AOwnsObjects);
  FPatchPart := aPatchPart;
...
constructor TParamLabelParamList.CopyCreate(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart; aParamLabelParam: TParamLabelParamList);
var
  i, j: Integer;
...
procedure TParamLabelParamList.SetParamLabel(aIndex: Integer;
  const aValue: TParamLabel);
begin
  inherited Items[aIndex] := aValue;
end;
...
constructor TParamLabelModuleList.Create(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart);
begin
  inherited Create(AOwnsObjects);
  FPatchPart := aPatchPart;
...
constructor TParamLabelModuleList.CopyCreate(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart; aParamLabelModule: TParamLabelModuleList);
var
  i: Integer;
begin
  inherited Create(AOwnsObjects);
  ...
  begin
    Add(TParamLabelParamList.CopyCreate(True, aPatchPart,
      aParamLabelModule.Items[i]));
  end;
end;
...
function TParamLabelModuleList.GetParameterLabelCount
  (aParamIndex: Byte): Integer;
begin
  Result := Items[aParamIndex].count;
end;

function TParamLabelModuleList.GetParamLabelParam(aIndex: Integer)
  : TParamLabelParamList;
begin
  Result := TParamLabelParamList(inherited Items[aIndex]);
end;

procedure TParamLabelModuleList.SetParamLabelParam(aIndex: Integer;
  const aValue: TParamLabelParamList);
begin
  inherited Items[aIndex] := aValue;
end;

function TParamLabelModuleList.AddParamLabelParam
  (aValue: TParamLabelParamList): Integer;
begin
  Result := inherited Add(aValue);
end;
...
procedure TParamLabelModuleList.AddParamLabel(aParamIndex, aLabelIndex: Byte;
  aName: string);
var
...
begin
  ...
  if not assigned(ParamLabelParam) then
  begin
...
begin
  ...
  begin
    ...
    if assigned(aChunk.FLogLines) then
    begin
      parlabels_str := Format('    %3d %3d %3d', [ParamLabelParam.FIsString,
        ParamLabelParam.FParamLen, ParamLabelParam.FParamIndex]);
    end;
    Add(ParamLabelParam);
    i := i - 3; // bytes
    if ParamLabelParam.FParamLen - 1 > 0 then
    begin // It's a string
      for j := 0 to ((ParamLabelParam.FParamLen - 1) div 7) - 1 do
      begin
        ParamLabel := TParamLabel.Create;
      ...
      begin
        for j := 0 to ParamLabelParam.count - 1 do
          parlabels_str := parlabels_str + ' ' + ParamLabelParam.Items
            [j].GetName;
      end;
    end;
    if assigned(aChunk.FLogLines) then
...
constructor TParameterLabelList.Create(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart);
begin
  inherited Create(AOwnsObjects);
  FPatchPart := aPatchPart;
...
constructor TParameterLabelList.CopySelected(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart; aModuleList: TModuleList;
  aParameterLabels: TParameterLabelList);
var
  i: Integer;
...
begin
  ...
  // FModuleCount    := 0;
  for i := 0 to aParameterLabels.count - 1 do
  begin
    Module := aModuleList.FindModule(aParameterLabels.Items[i].FModuleIndex);
    if assigned(Module) { and (Module.FSelected) } then
      Add(TParamLabelModuleList.CopyCreate(True, aPatchPart,
        aParameterLabels.Items[i]));
  end;
  // FModuleCount := Count;
end;
...
function TParameterLabelList.GetParameterLabelCount(aModuleIndex,
  aParamIndex: Byte): Integer;
begin
...
function TParameterLabelList.GetParamLabelModule(aIndex: Integer)
  : TParamLabelModuleList;
begin
  Result := TParamLabelModuleList(inherited Items[aIndex]);
end;

procedure TParameterLabelList.SetParamLabelModule(aIndex: Integer;
  const aValue: TParamLabelModuleList);
begin
  inherited Items[aIndex] := aValue;
end;

function TParameterLabelList.AddParamLabelModule
  (aValue: TParamLabelModuleList): Integer;
begin
  Result := inherited Add(aValue);
end;

procedure TParameterLabelList.AddParamLabels(aModuleIndex, aParamIndex: Byte;
  aNames: string);
var
  sl: TStringList;
  i: Integer;
...
function TParameterLabelList.FindParamLabelModule(aModuleIndex: Integer)
  : TParamLabelModuleList;
var
  i: Integer;
begin
...
function TParameterLabelList.FindParamLabel(aModuleIndex, aParamIndex,
  aLabelIndex: Byte): string;
var
...
procedure TParameterLabelList.AddParamLabel(aModuleIndex, aParamIndex,
  aLabelIndex: Byte; aName: string);
var
  ParamLabelModule: TParamLabelModuleList;
begin
  ParamLabelModule := FindParamLabelModule(aModuleIndex);
...
constructor TModuleLabelList.Create(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart);
begin
  inherited Create(AOwnsObjects);
  FPatchPart := aPatchPart;
...
constructor TModuleLabelList.CopySelected(AOwnsObjects: boolean;
  aPatchPart: TG2FilePatchPart; aModuleList: TModuleList;
  aModuleLabels: TModuleLabelList);
var
  i: Integer;
...
begin
  ...
  FNameCount := 0;
  for i := 0 to aModuleLabels.FNameCount - 1 do
  begin
...
procedure TModuleLabelList.SetModuleLabel(aIndex: Integer;
  const aValue: TModuleLabel);
begin
  inherited Items[aIndex] := aValue;
end;
...
var
  NumNames, i, j, ModuleIndex: Integer;
  ModuleLabel: TModuleLabel;
begin
  // FLocation    : TBits2;
  // FUnknown     : TBits6;
  ...
  begin
    ...
    if assigned(aChunk.FLogLines) then
      aChunk.FLogLines.Add(Format('%3d', [ModuleLabel.FModuleIndex]) + ' ' +
        ModuleLabel.FName);
  end;
  FNameCount := count;
end;
...
constructor TG2FilePatchPart.CopyModules(aPatch: TG2FilePatch;
  aSrcePatchPart: TG2FilePatchPart; aModuleList: TModuleList);
var
  ...
  case aChunk.FID of
    ...
    C_MODULE_NAMES: // $5a
      begin // Module names
        ...
        FPatch.FG2.dump_buffer((aChunk.FStream as TMemoryStream).Memory^,
          aChunk.FStream.Size);
      end;
  else
    begin
...
function TG2FilePatchPart.GetUniqueModuleNameSeqNr(aModuleFileName
  : string): Integer;
var
  m: Integer;
  found: boolean;
...
begin
  if assigned(FPatch) then
    ...
    raise Exception.Create
      ('Patch not assigned, cannot determine VariationCount'); // TODO Temporary
end;

procedure TG2FilePatchPart.SetFocusedModule(const aModule: TG2FileModule);
var
  OldFocusedModule: TG2FileModule;
begin
  if aModule <> FFocusedModule then
  begin
    OldFocusedModule := FFocusedModule;
    FFocusedModule := aModule;
    if assigned(aModule) then
      aModule.SelectModule(True, True);
    if assigned(OldFocusedModule) then
    begin
...
function TG2FilePatchPart.FindParamValue(aModuleIndex, aVariation,
  aParamIndex: Byte): Integer;
begin
  Result := FParameterList.FindParamValue(aModuleIndex, aVariation,
    aParamIndex);
end;

function TG2FilePatchPart.FindModuleVariationList(aModuleIndex: Byte)
  : TModuleVariationList;
begin
...
function TG2FilePatchPart.FindParamLabel(aModuleIndex, aParamIndex,
  aLabelIndex: Byte): string;
begin
  Result := FParameterLabelList.FindParamLabel(aModuleIndex, aParamIndex,
    aLabelIndex);
end;

procedure TG2FilePatchPart.AddParamLabel(aModuleIndex, aParamIndex,
  aLabelIndex: Byte; aValue: string);
begin
  FParameterLabelList.AddParamLabel(aModuleIndex, aParamIndex,
    aLabelIndex, aValue);
end;

procedure TG2FilePatchPart.AddNewModuleLabel(aModuleIndex: Byte;
  aValue: string);
begin
  FModuleLabelList.AddNewModuleLabel(aModuleIndex, aValue);
end;

function TG2FilePatchPart.GetParameterLabel(aModuleIndex, aParamIndex,
  aLabelIndex: Byte): string;
begin
  Result := FindParamLabel(aModuleIndex, aParamIndex, aLabelIndex)
end;

function TG2FilePatchPart.GetParameterLabelCount(aModuleIndex,
  aParamIndex: Byte): Integer;
begin
  Result := FParameterLabelList.GetParameterLabelCount(aModuleIndex,
    aParamIndex);
end;

procedure TG2FilePatchPart.SetParameterLabel(aModuleIndex, aParamIndex,
  aLabelIndex: Byte; aValue: string);
var
  Module: TG2FileModule;
  Param: TG2FileParameter;
...
begin
  Result := 0;
  MorphParameter := FPatch.GetMorph(FLocation, aModuleIndex, aParamIndex,
...
function TG2FilePatchPart.CreateModule(aModuleIndex: Byte; aModuleType: Byte)
  : TG2FileModule;
begin
  if assigned(FPatch) then
...
function TG2FilePatchPart.CreateCable(aColor: Byte; aFromModule: Byte;
  aFromConnector: Byte; aLinkType: Byte; aToModule: Byte; aToConnector: Byte)
  : TG2FileCable;
begin
  if assigned(FPatch) then
    Result := FPatch.CreateCable(FLocation, aColor, aFromModule, aFromConnector,
      aLinkType, aToModule, aToConnector)
  else
    raise Exception.Create('Patch not assigned to patchpart');
end;
  ...
  FLedComparison := TComparer<IG2DataLed>.Construct(
    ...
    begin
      if Led1.Location < Led2.Location then
        Result := 1
      else if Led1.Location = Led2.Location then
      begin
...
constructor TG2FilePatch.CopyModules(AOwner: TComponent; aPatch: TG2FilePatch;
aModuleList: TModuleList);
var
  i: Integer;
begin
  ...
  SetLength(FPatchPartArray, 3);
  for i := 0 to 2 do
    FPatchPartArray[i] := TG2FilePatchPart.CopyModules(self,
      aPatch.FPatchPartArray[i], aModuleList);
  FCurrentNoteList := TCurrentNoteList.Create(True, self);
  FPatchSettings := TPatchSettings.Create(self);
  FPatchDescription := TPatchDescription.Create;
  FMorphParameters := TMorphParameters.Create(self);
...
begin
  ...
  if assigned(G2) and assigned(G2.OnSelectModule) then
    G2.OnSelectModule(G2, G2.ID, PatchPart[ord(SelectedLocation)
      ].FocusedModule);
end;
...
begin
  ...
  if assigned(G2) and assigned(G2.OnSelectParam) then
  begin
...
begin
  ...
  begin
    for i := 0 to FPerformance.FGlobalKnobList.count - 1 do
      if FPerformance.FGlobalKnobList[i].SlotIndex = FSlot.SlotIndex then
      begin
  ...
  // Init morph labels
  for i := 0 to 7 do
    PatchPart[ord(ltPatch)].ParameterLabelList.AddParamLabel(PATCH_MORPH, 8 + i,
      0, STD_MORPH_NAMES[i]);
  FPatchNotes.Init;
  // for i := 0 to Length(FParamArray) - 1 do
  // FParamArray[i].Free;
...
procedure TG2FilePatch.SelectParamInPatch(aLocation: TLocationType;
aModuleIndex, aParameterIndex: Byte);
var
  Module: TG2FileModule;
  Param: TG2FileParameter;
begin
...
procedure TG2FilePatch.SelectParam(aLocation: TLocationType;
aModuleIndex, aParameterIndex: Byte);
begin
...
{ procedure AddParam( ParamType : TParamType; ModuleIndex : Byte; ModuleName : string; ParamIndex : Byte; ParamName : string; LowValue, HighValue, DefaultValue: Byte; ButtonText : string; InfoFunc : Integer);
  ...
  procedure AddParam(ParamType: TParamType; ModuleIndex: Byte;
  ...
  begin
    ...
    Param.InitParam(0, ModuleName, ParamIndex, ParamType, ParamName,
      DefaultLabel, LowValue, HighValue, DefaultValue, -1, -1, ButtonText);
...
begin
  ...
  case aIndex of
    0:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[0];
    1:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[1];
    2:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[2];
    3:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[3];
    4:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[4];
    5:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[5];
    6:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[6];
    7:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[7];
    8:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[8];
    9:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[9];
    10:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[10];
    11:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[11];
    12:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[12];
    13:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[13];
    14:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[14];
    15:
      Result := Modules[ord(ltPatch), PATCH_MORPH].Parameter[15];
  else
...
begin
  ...
  case aIndex of
    0:
      Result := Modules[ord(ltPatch), PATCH_MASTERCLOCK].Parameter[0];
    1:
      Result := Modules[ord(ltPatch), PATCH_VOICES].Parameter[0];
    2:
      Result := Modules[ord(ltPatch), PATCH_ARPEGGIATOR].Parameter[ARP_SPEED];
    3:
      Result := Modules[ord(ltPatch), PATCH_ARPEGGIATOR].Parameter [ARP_DIRECTION];
    4:
      Result := Modules[ord(ltPatch), PATCH_VIBRATO].Parameter[VIBRATO_DEPTH];
    5:
      Result := Modules[ord(ltPatch), PATCH_GLIDE].Parameter[GLIDE_SPEED];
    6:
      Result := Modules[ord(ltPatch), PATCH_BEND].Parameter[BEND_RANGE];
    7:
      Result := Modules[ord(ltPatch), PATCH_VOLUME].Parameter[VOLUME_LEVEL];
    8:
      Result := Modules[ord(ltPatch), PATCH_MASTERCLOCK].Parameter[1];
    9:
      Result := Modules[ord(ltPatch), PATCH_VOICES].Parameter[1];
    10:
      Result := Modules[ord(ltPatch), PATCH_ARPEGGIATOR].Parameter[ARP_ON_OFF];
    11:
      Result := Modules[ord(ltPatch), PATCH_ARPEGGIATOR].Parameter[ARP_OCTAVES];
    12:
      Result := Modules[ord(ltPatch), PATCH_VIBRATO].Parameter[VIBRATO_MOD];
    13:
      Result := Modules[ord(ltPatch), PATCH_GLIDE].Parameter[GLIDE_TYPE];
    14:
      Result := Modules[ord(ltPatch), PATCH_BEND].Parameter[BEND_ON_OFF];
    15:
      Result := Modules[ord(ltPatch), PATCH_VOLUME].Parameter[VOLUME_MUTE];
  else
    raise Exception.Create('Knob index out of range.');
  end;
end;
...
var
  Location, ModuleCount, dummy: Byte;
begin
  Result := True;
  case aChunk.FID of
    C_MODULE_LIST: // $4a
    ...
    C_PARAM_LIST: // $4d
      begin // Parameter list
        ...
        case Location of
          0 .. 1:
            FPatchPartArray[Location].FParameterList.Read(aChunk);
          2:
    ...
    C_PARAM_NAMES: // $5b
      begin // Parmeter names
        ...
        case Location of
          0 .. 1:
            FPatchPartArray[Location].FParameterLabelList.
              Read(aChunk, ModuleCount);
...
function TG2FilePatch.LoadFromFile(aStream: TStream;
aLogLines: TStrings): boolean;
var
  sl: TStringList;
  Chunk: TPatchChunk;
  s: string;
...
class function TG2FilePatch.LoadFileStream(AOwner: TComponent; aStream: TStream;
...
function TG2FilePatch.CreateModule(aLocation: TLocationType; aModuleIndex: Byte;
...
begin
  ...
  if assigned(G2) and (aLocation <> ltPatch) then
  begin
...
function TG2FilePatch.CreateCable(aLocation: TLocationType; aColor: Byte;
  aFromModule: Byte; aFromConnector: Byte; aLinkType: Byte; aToModule: Byte;
...
function TG2FilePatch.CreateLed(const aLedType: TLedType;
  const aLocation: TLocationType; const aModuleIndex, aGroupID: Byte;
  const aGroupCount: Integer): IG2DataLed;
begin
  Result := TG2FileLed.Create(aLedType, aLocation, aModuleIndex, aGroupID,
    aGroupCount);
end;
...
procedure TG2FilePatch.CreateLeds(const aLocation: TLocationType;
  const aModuleIndex, aModuleType: Byte);
var
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        Led := CreateLed(LedDefs[i].LedType, aLocation, aModuleIndex,
          LedDefs[i].GroupId, GroupCount);
        FLedList.Add(Led);
      end
      else
      begin
        ...
        while (j < FLedStripList.count) and
          not((FLedStripList[j].Location = aLocation) and
  ...
  begin
    ...
    begin
      Led := CreateLed(ltMiniVU, aLocation, aModuleIndex,
        MiniVUDefs[i].GroupId, 1);
      FLedStripList.Add(Led);
    end;
  end;
  FLedList.Sort;
...
procedure TG2FilePatch.AddModuleToPatch(aLocation: TLocationType;
  aModule: TG2FileModule);
begin
...
procedure TG2FilePatch.DeleteModuleFromPatch(aLocation: TLocationType;
  aModule: TG2FileModule);
var
  aModuleIndex: Byte;
  i: Integer;
begin
  ...
  for i := FLedList.count - 1 downto 0 do
    if (FLedList[i].Location = aLocation) and
      (FLedList[i].ModuleIndex = aModuleIndex) then
      FLedList.Delete(i);
  for i := FLedStripList.count - 1 downto 0 do
    if (FLedStripList[i].Location = aLocation) and
      (FLedStripList[i].ModuleIndex = aModuleIndex) then
...
procedure TG2FilePatch.AddCableToPatch(aLocation: TLocationType;
  aCable: TG2FileCable);
begin
  FPatchPartArray[ord(aLocation)].AddCable(aCable);
end;

procedure TG2FilePatch.DeleteCableFromPatch(aLocation: TLocationType;
  aCable: TG2FileCable);
begin
  FPatchPartArray[ord(aLocation)].DeleteCable(aCable);
end;

function TG2FilePatch.FindKnob(aLocation: TLocationType;
  aModuleIndex, aParamIndex: Byte): Integer;
begin
  Result := FKnobList.FindKnob(aLocation, aModuleIndex, aParamIndex);
end;

function TG2FilePatch.FindMidiCC(aLocation: TLocationType;
  aModuleIndex, aParamIndex: Byte): Byte;
begin
  Result := FControllerList.FindMidiCC(aLocation, aModuleIndex, aParamIndex);
end;

function TG2FilePatch.AssignKnobInPatch(aKnobIndex: Integer;
  aLocation: TLocationType; aModuleIndex, aParamIndex: Byte): TKnob;
var
  Knob: TKnob;
begin
  Result := nil;
  if aKnobIndex < 0 then
    Exit;
  if aKnobIndex < FKnobList.count then
  begin
    ...
    raise Exception.Create('Knob index ' + IntToStr(aKnobIndex) +
      ' out of range.');
end;

function TG2FilePatch.AssignMidiCCInPatchX(aMidiCC: Byte;
  aLocation: TLocationType; aModuleIndex, aParamIndex: Byte): TController;
var
  Controller: TController;
begin
  Result := nil;
...
function TG2FilePatch.GetNoOffModuleType(aLocation: TLocationType;
aModuleType: Byte): Integer;
begin
...
begin
  if (KnobIndex >= 0) and (KnobIndex < FKnobList.count) then
  begin
...
function TG2FilePatch.GetModuleName(aLocation: TLocationType;
  aModuleIndex: Byte): string;
begin
  Result := FPatchPartArray[ord(aLocation)].FindModuleLabel(aModuleIndex);
end;

function TG2FilePatch.GetParameterValue(aLocation: TLocationType;
  aModuleIndex: Byte; aParamIndex: Byte; aVariation: Byte): Byte;
begin
  Result := 0;
  if (aLocation = ltFX) or (aLocation = ltVA) then
  begin
    Result := FPatchPartArray[ord(aLocation)].FindParamValue(aModuleIndex,
      aVariation, aParamIndex);
  end
  else if aLocation = ltPatch then
  begin
    case aModuleIndex of
      PATCH_MORPH:
        begin
          case aParamIndex of
            0 .. 7:
              Result := FPatchSettings.FMorphArray[aParamIndex].FDialArray
                [aVariation];
            8 .. 15:
              Result := FPatchSettings.FMorphArray[aParamIndex - 8].FModeArray
                [aVariation];
          end;
        end;
      PATCH_VOLUME:
        begin
          case aParamIndex of
            VOLUME_LEVEL:
              Result := FPatchSettings.FVariationListArray[aVariation]
                .FPatchVol;
            VOLUME_MUTE:
              Result := FPatchSettings.FVariationListArray[aVariation]
                .FActiveMuted;
      ...
      PATCH_GLIDE:
        begin
          case aParamIndex of
            GLIDE_TYPE:
              Result := FPatchSettings.FVariationListArray[aVariation].FGlide;
            GLIDE_SPEED:
              Result := FPatchSettings.FVariationListArray[aVariation]
                .FGlideTime;
          end;
      ...
      PATCH_BEND:
        begin
          case aParamIndex of
            BEND_ON_OFF:
              Result := FPatchSettings.FVariationListArray[aVariation].FBend;
            BEND_RANGE:
              Result := FPatchSettings.FVariationListArray[aVariation].FSemi;
          end;
        end;
      PATCH_VIBRATO:
        begin
          case aParamIndex of
            VIBRATO_MOD:
              Result := FPatchSettings.FVariationListArray[aVariation].FVibrato;
            VIBRATO_DEPTH:
              Result := FPatchSettings.FVariationListArray[aVariation].FCents;
            VIBRATO_RATE:
              Result := FPatchSettings.FVariationListArray[aVariation].FRate;
          end;
        end;
      PATCH_ARPEGGIATOR:
        begin
          case aParamIndex of
            ARP_ON_OFF:
              Result := FPatchSettings.FVariationListArray[aVariation]
                .FArpeggiator;
            ARP_SPEED:
              Result := FPatchSettings.FVariationListArray[aVariation].FArpTime;
            ARP_DIRECTION:
              Result := FPatchSettings.FVariationListArray[aVariation].FArpType;
            ARP_OCTAVES:
              Result := FPatchSettings.FVariationListArray[aVariation].FOctaves;
          end;
        end;
      PATCH_SUSTAIN:
        begin
          case aParamIndex of
            SUSTAIN_PEDAL:
              Result := FPatchSettings.FVariationListArray[aVariation].FSustain;
            OCTAVE_SHIFT:
              Result := FPatchSettings.FVariationListArray[aVariation]
                .FOctaveShift;
          end;
...
procedure TG2FilePatch.SetParamInPatch(aLocation: TLocationType;
  aModuleIndex: Byte; aParamIndex: Byte; aVariation: Byte; aValue: Byte);
var
  ModuleVariationList: TModuleVariationList;
  ModuleParamValues: TModuleParamValues;
  Param: TG2FileParameter;
...
begin
  if (aLocation = ltFX) or (aLocation = ltVA) then
  begin
    ModuleVariationList := FPatchPartArray[ord(aLocation)
      ].FindModuleVariationList(aModuleIndex);
    if assigned(ModuleVariationList) then
    begin
      ModuleParamValues := ModuleVariationList.FindVariation(aVariation);
      if assigned(ModuleParamValues) then
    ...
    case aModuleIndex of
      PATCH_MORPH:
        begin
          case aParamIndex of
            0 .. 7:
              FPatchSettings.FMorphArray[aParamIndex].FDialArray[aVariation]
                := aValue;
            8 .. 15:
              FPatchSettings.FMorphArray[aParamIndex - 8].FModeArray[aVariation]
                := aValue;
          end;
        end;
      PATCH_VOLUME:
        begin
          case aParamIndex of
            VOLUME_LEVEL:
              FPatchSettings.FVariationListArray[aVariation].FPatchVol
                := aValue;
            VOLUME_MUTE:
              FPatchSettings.FVariationListArray[aVariation].FActiveMuted
                := aValue;
          end;
        end;
      PATCH_GLIDE:
        begin
          case aParamIndex of
            GLIDE_TYPE:
              FPatchSettings.FVariationListArray[aVariation].FGlide := aValue;
            GLIDE_SPEED:
              FPatchSettings.FVariationListArray[aVariation].FGlideTime
                := aValue;
          end;
        end;
      PATCH_BEND:
        begin
          case aParamIndex of
            BEND_ON_OFF:
              FPatchSettings.FVariationListArray[aVariation].FBend := aValue;
            BEND_RANGE:
              FPatchSettings.FVariationListArray[aVariation].FSemi := aValue;
          end;
        end;
      PATCH_VIBRATO:
        begin
          case aParamIndex of
            VIBRATO_MOD:
              FPatchSettings.FVariationListArray[aVariation].FVibrato := aValue;
            VIBRATO_DEPTH:
              FPatchSettings.FVariationListArray[aVariation].FCents := aValue;
            VIBRATO_RATE:
              FPatchSettings.FVariationListArray[aVariation].FRate := aValue;
          end;
        end;
      PATCH_ARPEGGIATOR:
        begin
          case aParamIndex of
            ARP_ON_OFF:
              FPatchSettings.FVariationListArray[aVariation].FArpeggiator
                := aValue;
            ARP_SPEED:
              FPatchSettings.FVariationListArray[aVariation].FArpTime := aValue;
            ARP_DIRECTION:
              FPatchSettings.FVariationListArray[aVariation].FArpType := aValue;
            ARP_OCTAVES:
              FPatchSettings.FVariationListArray[aVariation].FOctaves := aValue;
          end;
        end;
      PATCH_SUSTAIN:
        begin
          case aParamIndex of
            SUSTAIN_PEDAL:
              FPatchSettings.FVariationListArray[aVariation].FSustain := aValue;
            OCTAVE_SHIFT:
              FPatchSettings.FVariationListArray[aVariation].FOctaveShift
                := aValue;
          end;
        end;
    end;
  ...
  begin
    ...
    if assigned(G2.OnParameterChange) then
      G2.OnParameterChange(self, FG2.ID, Slot.SlotIndex, aVariation, aLocation,
        aModuleIndex, aParamIndex, aValue);
  end;
end;

procedure TG2FilePatch.SetParamValue(aLocation: TLocationType;
...
function TG2FilePatch.GetModeValue(aLocation: TLocationType; aModuleIndex: Byte;
  aParamIndex: Byte): Byte;
var
  Module: TG2FileModule;
begin
...
procedure TG2FilePatch.SetModeInPatch(aLocation: TLocationType;
  aModuleIndex, aParamIndex, aValue: Byte);
var
  Module: TG2FileModule;
begin
  Module := FPatchPartArray[ord(aLocation)].FindModule(aModuleIndex);
  if assigned(Module) then
    ...
    begin
      ...
      if assigned(G2.OnModuleModeChange) then
        G2.OnModuleModeChange(self, G2.ID, Slot.SlotIndex, aLocation,
          aModuleIndex, aParamIndex, aValue);
    end;
end;

procedure TG2FilePatch.SetModeValue(aLocation: TLocationType;
  aModuleIndex: Byte; aParamIndex: Byte; aValue: Byte);
begin
  SetModeInPatch(aLocation, aModuleIndex, aParamIndex, aValue);
end;

function TG2FilePatch.HasMorph(aLocation: TLocationType;
  aModuleIndex, aParamIndex, aVariation: Byte): boolean;
var
  i: Integer;
  PatchParamList: TPatchParamList;
...
function TG2FilePatch.GetMorph(aLocation: TLocationType;
  aModuleIndex, aParamIndex, aMorphIndex, aVariation: Byte): TMorphParameter;
begin
  Result := FPatchSettings.FVariationListArray[aVariation].FindMorphParam
    (aLocation, aModuleIndex, aParamIndex, aMorphIndex, aVariation);
end;

function TG2FilePatch.GetMorphValue(aLocation: TLocationType;
  aModuleIndex, aParamIndex, aMorphIndex, aVariation: Byte): Byte;
var
  MorphParameter: TMorphParameter;
begin
  Result := 0;
  if aLocation = ltPatch then
  begin
    MorphParameter := GetMorph(aLocation, aModuleIndex, aParamIndex,
      aMorphIndex, aVariation);
    if assigned(MorphParameter) then
      Result := MorphParameter.Range;
  end
  else
    Result := FPatchPartArray[ord(aLocation)].GetMorphValue(aModuleIndex,
      aParamIndex, aMorphIndex, aVariation);
end;

procedure TG2FilePatch.SetMorphValue(aLocation: TLocationType;
  aModuleIndex: Byte; aParamIndex: Byte; aMorphIndex: Byte; aValue: Byte;
  aVariation: Byte);
var
  MorphParameter: TMorphParameter;
begin
  MorphParameter := GetMorph(aLocation, aModuleIndex, aParamIndex, aMorphIndex,
    aVariation);
  if not assigned(MorphParameter) then
  begin
    if aValue <> 0 then
      FPatchSettings.FVariationListArray[aVariation].AddNewMorphParam
        (ord(aLocation), aModuleIndex, aParamIndex, aMorphIndex, aValue);
  end
  else
  begin
    if aValue = 0 then
      FPatchSettings.FVariationListArray[aVariation].DelMorphParam
        (ord(aLocation), aModuleIndex, aParamIndex, aMorphIndex)
    else
      MorphParameter.FRange := aValue;
  end;
  DoMorphChange(Slot.SlotIndex, TLocationType(aLocation), aModuleIndex,
    aParamIndex, aMorphIndex, aVariation, aValue);
  // if assigned(G2) then
  // if assigned(G2.FOnMorphChange) then
...
begin
  ...
  FPatchSettings.FVariationListArray[aToVariation]
    .Copy(FPatchSettings.FVariationListArray[aFromVariation]);
end;
...
begin
  if aIndex < Length(FPatchPartArray) then
    ...
    raise Exception.Create('Module list index ' + IntToStr(aIndex) +
      ' out of range.');
end;
...
begin
  if aIndex < Length(FPatchPartArray) then
    ...
    raise Exception.Create('Cable list index ' + IntToStr(aIndex) +
      ' out of range.');
end;
...
begin
  if aIndex < Length(FPatchPartArray) then
    ...
    raise Exception.Create('Parameter list index ' + IntToStr(aIndex) +
      ' out of range.');
end;
...
begin
  if aIndex < Length(FPatchPartArray) then
    ...
    raise Exception.Create('ModuleLabels index ' + IntToStr(aIndex) +
      ' out of range.');
end;

function TG2FilePatch.GetParameterLabelList(aIndex: Integer)
  : TParameterLabelList;
begin
  if aIndex < Length(FPatchPartArray) then
    Result := FPatchPartArray[aIndex].FParameterLabelList
  else
    raise Exception.Create('ParameterLabels index ' + IntToStr(aIndex) +
      ' out of range.');
end;
...
function TG2FilePatch.GetParameterLabel(aLocation: TLocationType;
  aModuleIndex, aParamIndex, aLabelIndex: Byte): string;
begin
  if (aLocation = ltFX) or (aLocation = ltVA) then
  begin
    Result := FPatchPartArray[ord(aLocation)].GetParameterLabel(aModuleIndex,
...
function TG2FilePatch.GetParameterLabelCount(aLocation: TLocationType;
  aModuleIndex, aParamIndex: Byte): Integer;
begin
  if (aLocation = ltFX) or (aLocation = ltVA) then
  begin
    Result := FPatchPartArray[ord(aLocation)].GetParameterLabelCount
      (aModuleIndex, aParamIndex)
  end
  else
    Result := 0;
end;

procedure TG2FilePatch.SetParameterLabel(aLocation: TLocationType;
  aModuleIndex, aParamIndex, aLabelIndex: Byte; aValue: string);
begin
  if (aLocation = ltFX) or (aLocation = ltVA) then
    FPatchPartArray[ord(aLocation)].SetParameterLabel(aModuleIndex, aParamIndex,
      aLabelIndex, aValue);
end;

function TG2FilePatch.GetModuleLabel(aLocation: TLocationType;
  aModuleIndex: Byte): string;
begin
  if (aLocation = ltFX) or (aLocation = ltVA) then
  begin
...
procedure TG2FilePatch.SetModuleLabel(aLocation: TLocationType;
  aModuleIndex: Byte; aValue: string);
begin
  if (aLocation = ltFX) or (aLocation = ltVA) then
...
function TG2FilePatch.MessAddModule(aLocation: TLocationType;
  aModuleTypeID, aCol, aRow: Byte): boolean;
begin
  Result := False;
...
function TG2FilePatch.MessCopyModules(aSrcePatch: TG2FilePatchPart;
  aFromLocation, aToLocation: TLocationType): boolean;
begin
  Result := False;
  // UnselectModules( ltVA);
  // UnselectModules( ltFX);
...
function TG2FilePatch.MessAddConnection(aLocation: TLocationType;
  aFromConnection, aToConnection: TG2FileConnector): boolean;
begin
  // Abstract
  Result := False;
end;

function TG2FilePatch.MessDeleteConnection(aLocation: TLocationType;
  aCable: TG2FileCable): boolean;
begin
  // Abstract
  Result := False;
end;

function TG2FilePatch.MessDeleteModule(aLocation: TLocationType;
  aModuleIndex: Byte): boolean;
begin
  // Abstract
  Result := False;
end;
...
function TG2FilePatch.MessMoveModule(aLocation: TLocationType;
aModuleIndex, aCol, aRow: Byte): boolean;
begin
  // Abstract
  Result := False;
end;

function TG2FilePatch.MessSetModuleColor(aLocation: TLocationType;
  aModuleIndex, aColor: Byte): boolean;
begin
  // Abstract
  Result := False;
end;

function TG2FilePatch.MessSetModuleParamLabels(aLocation: TLocationType;
  aModuleIndex, aParamIndex, aLabelIndex: Byte; aName: string): boolean;
begin
  // Abstract
  Result := False;
end;

function TG2FilePatch.MessSetModuleLabel(aLocation: TLocationType;
  aModuleIndex: Byte; aName: string): boolean;
begin
  // Abstract
  Result := False;
end;
...
begin
  if FSelectedLocation <> aLocation then
  begin
...
//
// ------------------------------------------------------------------------------
constructor TG2FileConnector.Create(aPatch: TG2FilePatch;
...
procedure TG2FileConnector.InitConnector(aModuleType, aConnectorIndex: Byte;
  aConnectorKind: TConnectorKind);
var
  i: Integer;
begin
...
begin
  if (FConnectorDefColor = COLOR_BLUE) and (FModule.Uprate = 1) and
    (FBandWidth = btDynamic) then
    Result := COLOR_RED
  else if (FConnectorDefColor = COLOR_YELLOW) and (FModule.Uprate = 1) and
    (FBandWidth = btDynamic) then
    Result := COLOR_ORANGE
  else
...
begin
  if (FConnectorDefColor = COLOR_BLUE) and (FModule.NewUprate = 1) and
    (FBandWidth = btDynamic) then
    Result := COLOR_RED
  else if (FConnectorDefColor = COLOR_YELLOW) and (FModule.NewUprate = 1) and
    (FBandWidth = btDynamic) then
    Result := COLOR_ORANGE
  else
    Result := FConnectorDefColor;
...
begin
  if aIndex < FCables.count then
    ...
    raise Exception.Create('Cable index ' + IntToStr(aIndex) +
      ' out of range.');
end;
...
begin
  ...
  else if FConnectorType = ctControl then
    ConnectorDefColor := COLOR_BLUE
  else if FConnectorType = ctAudio then
  begin
    if FBandWidth = btStatic then
      ConnectorDefColor := COLOR_RED;
    if FBandWidth = btDynamic then
      ConnectorDefColor := COLOR_BLUE;
  end;
end;
...
//
// ------------------------------------------------------------------------------
constructor TG2FileParameter.Create(aPatch: TG2FilePatch;
...
begin
  if assigned(FPatch) then
    Result := FPatch.GetParameterLabel(FLocation, FModuleIndex,
      FParamIndex, aIndex)
  else
    Result := '';
end;
...
begin
  if assigned(FPatch) then
    FPatch.SetParameterLabel(FLocation, FModuleIndex, FParamIndex,
      aIndex, aValue);
end;
...
begin
  if assigned(FPatch) then
    Result := FPatch.GetMorph(FLocation, FModuleIndex, FParamIndex, aMorphIndex,
      aVariation)
  else
    Result := nil;
end;
...
begin
  if assigned(FPatch) then
    Result := FPatch.HasMorph(FLocation, FModuleIndex, FParamIndex,
      FPatch.ActiveVariation)
  else
    Result := False;
end;

function TG2FileParameter.GetMorphValue(const aMorphIndex,
  aVariation: Integer): Byte;
var
  MorphParameter: TMorphParameter;
begin
...
begin
  if assigned(FPatch) then
  begin
    FPatch.SetMorphValue(FLocation, FModuleIndex, FParamIndex, aMorphIndex,
      aValue, aVariation);
  end;
end;
...
var
  Variation, FromVariation, ToVariation: Byte;
begin
  if FPatch.EditAllVariations then
  begin
    FromVariation := 0;
...
var
  Variation, FromVariation, ToVariation: Byte;
begin
  if aValue <> FSuspendUpdate then
  begin
    FSuspendUpdate := aValue;
    if FSuspendUpdate then
    begin
      if assigned(FPatch) then
      begin
        case FParamType of
          ptParam:
            FSuspendedValue := FPatch.GetParameterValue(FLocation, FModuleIndex,
              FParamIndex, FPatch.ActiveVariation);
          ptMode:
            FSuspendedValue := FPatch.GetModeValue(FLocation, FModuleIndex,
              FParamIndex);
          ptMasterClock:
            case FParamIndex of
              0:
                FSuspendedValue := FPatch.GetMasterClock;
              1:
                FSuspendedValue := FPatch.GetMasterClockRun;
            end;
          ptVoiceMode:
            case FParamIndex of
              0:
                FSuspendedValue := FPatch.GetVoiceCount;
              1:
                FSuspendedValue := FPatch.GetVoiceMode;
            end;
        end;
      end
      else
        ...
        case FParamType of
          ptParam:
            begin;
              ...
              for Variation := FromVariation to ToVariation do
              begin
                FPatch.SetParamValue(FLocation, FModuleIndex, FParamIndex,
                  Variation, FSuspendedValue)
              end;
            end;
          ptMode:
            FPatch.SetModeValue(FLocation, FModuleIndex, FParamIndex,
              FSuspendedValue);
          ptMasterClock:
            begin
              case FParamIndex of
                0:
                  FPatch.SetMasterClock(FSuspendedValue);
                1:
                  FPatch.SetMasterClockRun(FSuspendedValue);
              end;
            end;
          ptVoiceMode:
            begin
              case FParamIndex of
                0:
                  FPatch.SetVoiceCount(FSuspendedValue);
                1:
                  FPatch.SetVoiceMode(FSuspendedValue);
              end;
            end;
        end;
      end;
...
begin
  ...
  else if assigned(FPatch) then
  begin
    case FParamType of
      ptParam:
        Result := FPatch.GetParameterValue(FLocation, FModuleIndex, FParamIndex,
          FPatch.ActiveVariation);
      ptMode:
      ...
      ptMasterClock:
        case FParamIndex of
          0:
            Result := FPatch.GetMasterClock;
          1:
            Result := FPatch.GetMasterClockRun;
        end;
      ptVoiceMode:
        case FParamIndex of
          0:
            Result := FPatch.GetVoiceCount;
          1:
            Result := FPatch.GetVoiceMode;
        end;
    end;
  end;
end;
...
begin
  ...
  begin
    case FParamType of
      ...
      ptMode:
        FPatch.SetModeValue(FLocation, FModuleIndex, FParamIndex, aValue);
      ptMasterClock:
        begin
          case FParamIndex of
            0:
              FPatch.SetMasterClock(aValue);
            1:
              FPatch.SetMasterClockRun(aValue);
          end;
        end;
      ptVoiceMode:
        begin
          case FParamIndex of
            0:
              FPatch.SetVoiceCount(aValue);
            1:
              FPatch.SetVoiceMode(aValue);
          end;
        end;
    end;
  end;
...
begin
  ...
  if Range >= 128 then
    Range := Range - 256;
  inc(Range);
  if (Value + Range) <= FHighValue then
  begin
    if Range < 0 then
...
procedure TG2FileParameter.InitParam(aID: Integer; aModuleName: string;
  aParamIndex: Byte; aParamType: TParamType;
...
begin
  FKnob := FPatch.AssignKnobInPatch(aKnobIndex, FLocation, FModuleIndex,
    FParamIndex);
  FKnob.FParameter := self;
end;
...
procedure TG2FileParameter.AssignGlobalKnob(aPerf: TG2FilePerformance;
aSlotIndex: Byte; aKnobIndex: Integer);
begin
  FGlobalKnob := aPerf.AssignGlobalKnobInPerf(aKnobIndex, aSlotIndex, FLocation,
    FModuleIndex, FParamIndex);
  FGlobalKnob.FParameter := self;
end;

procedure TG2FileParameter.DeassignGlobalKnob(aPerf: TG2FilePerformance;
aKnobIndex: Integer);
begin
...
begin
  FController := FPatch.AssignMidiCCInPatchX(aMidiCC, FLocation, FModuleIndex,
    FParamIndex);
...
begin
  if assigned(FPatch) then
    Result := FPatch.GetParameterLabelCount(FLocation, FModuleIndex,
      FParamIndex)
  else
    Result := 0;
end;
  ...
  begin
    case ModuleIndex of
      ...
      PATCH_VOICES:
        Result := False;
      PATCH_VOLUME:
        Result := ParamIndex <> VOLUME_LEVEL;
    else
      Result := True;
    end;
  end
...
begin
  ...
  begin
    ...
    if assigned(FPatch.PatchPart[ord(FPatch.SelectedLocation)].FocusedModule)
    then
      Result := FPatch.PatchPart[ord(FPatch.SelectedLocation)
        ].FocusedModule.SelectedParam = self;
  end;
end;
...
procedure TG2FileParameter.AddTextDependency(aParamType: TParamType;
  aParamIndex: Byte);
var
  i: Integer;
begin
...
begin
  if aIndex < Length(FTextDependencieArray) then
  begin
...
procedure TG2FileParameter.AddGraphDependency(aParamType: TParamType;
aParamIndex: Byte);
var
  i: Integer;
begin
...
begin
  if aIndex < Length(FGraphDependencieArray) then
  begin
...
function TG2FileParameter.DelayDispValue(aType: Integer; aRange: Integer;
  aValue: Integer): string;
var
  DlyRange, DlyMin, DlyMax: Single;
begin
  case aType of
    0:
      begin // ranges 5m,25m,100m,500m,1sm,2s,2.7s
        case aRange of
          0:
            begin
              DlyMin := 0.05;
              ...
              if aValue = 0 then
                ...
                Result := G2FloatToStr(DlyMin + (DlyMax - DlyMin) * (aValue - 1)
                  / 126, 4) + 'm';
            end;
          1:
            begin
          ...
          5:
            begin
              ...
              if aValue = 0 then
                ...
                Result := G2FloatToStr
                  ((DlyMin + (DlyMax - DlyMin) * (aValue - 1) / 126) /
                  1000, 5) + 's'
              else
                Result := G2FloatToStr(DlyMin + (DlyMax - DlyMin) * (aValue - 1)
                  / 126, 4) + 'm';
    ...
    1:
      begin // ranges 5m,25m,100m,500m,1sm,2s,2.7s for DelayEight
        DlyMin := 0;
        DlyMax := 0;
        case aRange of
          0:
        ...
        Result := G2FloatToStr(DlyMin + (DlyMax - DlyMin) * aValue /
          127, 4) + 'm';
      end;
    2:
      begin // ranges 500m,1s,2s,2.7s
        case aRange of
          0:
            begin
              DlyRange := 500;
...
var
  ...
  Param: TG2FileParameter;
begin
  if (TextFunctionIndex <> 0) and assigned(Module) and
    (Length(FTextDependencieArray) > 0) then
  ...
  case aIndex of
    ...
    12:
      begin // Mystery modules PolarFade/PolarPan...
    ...
    17:
      begin // RndPattern PatternA / FltPhase FB / NoteZone SendTrans
        case aValue of
          127:
            Result := IntToStr(64);
        else
    ...
    22:
      begin // Drumsynth, Masterfreq
        Result := G2FloatToStr(20.02 * power(2, aValue / 24), 4) + 'Hz';
      end;
    23:
      begin // Drumsynth, SlaveRatio
        TempValue := aValue;
        case TempValue of
          0:
            Result := '1:1';
    ...
    28:
      begin // Env Attack, Decay, Release
        t := ENV_TIMES[aValue];
    ...
    35:
      begin // Note Quant, Range
        TempValue := aValue;
        if TempValue = 0 then
          Result := '0'
        else if TempValue = 127 then
          Result := '+-64'
        else
    ...
    36:
      begin // EqPeak Gain
        case aValue of
          127:
            Result := G2FloatToStrFixed(64 / 3.55555, 4) + 'dB';
        else
    ...
    45:
      begin // ClkGen Tempo
        if assigned(Module) and (Length(FTextDependencieArray) = 3) then
        begin
          iValue1 := GetTextDependendParamValue(1);
          iValue2 := GetTextDependendParamValue(2);
          if iValue1 = 0 then
            Result := '--'
          else if iValue2 = 1 then
            Result := 'MASTER'
          else
    ...
    52:
      begin // Fade2-1 Mix
        ...
        if TempValue = 0 then
          Result := 'Mute'
        else if TempValue = -64 then
          Result := 'I1:127'
        else if TempValue < 0 then
          Result := 'I1:' + IntToStr(-TempValue * 2)
        else if TempValue = 63 then
          Result := 'I2:127'
        else
    ...
    69:
      begin // NoteScaler, range
        ...
        if (TempValue mod 24 = 0) then
          Result := Result + '-Oct'
        else if ((TempValue mod 24) mod 20 = 0) then
          Result := Result + '-7th'
        else if ((TempValue mod 24) mod 14 = 0) then
          Result := Result + '-5th';
      end;
    70:
      begin // Level scaler, Gain
        TempValue := aValue;
        if TempValue < 64 then
    ...
    108:
      begin // Midi Channel Send
        TempValue := aValue;
        case TempValue of
          0 .. 15:
            Result := IntToStr(TempValue + 1);
          16:
            Result := 'This';
          17:
            Result := 'Slot A';
    ...
    122:
      begin // logic Pulse/Delay Range
        if assigned(Module) and (Length(FTextDependencieArray) = 2) then
        begin
    ...
    174:
      begin // Compressor Attack
        Result := COMPR_ATTACK_TIMES[aValue];
      end;
    175:
      begin // Compressor Release
        Result := COMPR_RELEASE_TIMES[aValue];
      end;
    176:
      begin // Compressor Treshold
        case aValue of
          42:
            Result := 'Off';
        else
    ...
    185:
      begin // Osc dual Saw phase (0..357)
        Result := IntToStr(Round(aValue / 128 * 360));
      end;
    187:
      begin // ClkGen Source
    ...
    196:
      begin // Operator FreqDetune
        Result := IntToStr(aValue - 7);
      end;
    197:
      begin // Operator BrPoint
        Result := GetKeyName(aValue + 9);
      end;
    199:
      begin // Operator RDepthMode
        Result := IntToStr(aValue);
      end;
    200:
      begin // Operator freq mode
        case aValue of
          0:
            Result := 'Ratio';
          1:
      ...
      begin // Scratch Ratio
        case aValue of
          64:
            Result := 'x0';
          127:
            Result := 'x' + G2FloatToStrFixed(4, 4);
        else
          Result := 'x' + G2FloatToStrFixed((aValue - 64) / 16, 4);
        end;
      end;
    210:
      begin // NoteZone ThruMode
        case aValue of
          0:
            Result := 'Notes';
          1:
    ...
    220:
      begin // Pitch track and Noise gate threshhold
    ...
    500:
      begin // SeqCtrl (Not found in original moduledef)
        if assigned(Module) then
        begin
          TempValue := aValue;
...
begin
  if (FTextFunctionIndex <> 0) and assigned(Module) and
    (Length(FTextDependencieArray) > 0) then
    aValue := GetTextDependendParamValue(0)
  else
    aValue := GetValue;
  case FTextFunctionIndex of
    ...
    17:
      begin // NoteZone SendTrans
        Result := InfoFunction(17);
      end;
    27:
      begin // OscShpB, shape Mod
    ...
    96:
      begin // Constant Level
        Result := InfoFunction(17);
      end;
    99:
      begin // Sw1-4 Sel CHECK
        Result := IntToStr(aValue * 4);
      end;
    100:
      begin // SwOnOffM On, SwOnOffT On, Sw2-1M Sel, Sw2-1 Sel, Sw1-2M Sel, Sw1-2 Sel CHECK
        Result := IntToStr(aValue * 4);
      end;
    101:
      begin // Sw4-1 Sel, Sw8-1 Sel, Sw1-8 Sel CHECK
        Result := IntToStr(aValue * 4);
      end;
    102:
      begin // MixFader Lev1
    ...
    103:
      begin // LFO Freq
        ...
        begin
          ...
          case iValue1 of
            0:
              Result := G2FloatToStr(699 / (aValue + 1), 4) + 's';
            1:
              if aValue < 32 then
                Result := G2FloatToStr(1 / (0.0159 * power(2, aValue / 12)
                  ), 4) + 's'
              else
                Result := G2FloatToStr(0.0159 * power(2, aValue / 12),
                  4) + 'Hz';
            2:
              Result := G2FloatToStr(0.2555 * power(2, aValue / 12), 4) + 'Hz';
            3:
              Result := G2BPM(aValue);
            4:
              begin; // Clock
    ...
    109:
      begin // CtrlRcv Ch, NoteRcv Ch, NoteZone RcvCh
    ...
    198:
      begin // Operator freq
        ...
        begin
          ...
          case iValue2 of
            0:
              begin // Ratio  x0.50...x16.00..x31.00
                if aValue = 0 then
                begin
                  Fact := 0.5;
                end
            ...
            1:
              begin // Fixed 1Hz,10Hz,100Hz,1000Hz etc
...
procedure TGlobalKnobList.SetGlobalKnob(aIndex: Integer;
const aValue: TGlobalKnob);
begin
  inherited Items[aIndex] := aValue;
end;
...
function TGlobalKnobList.FindGlobalKnobIndex(aSlotIndex: Byte;
  aLocation: TLocationType; aModuleIndex, aParamIndex: Byte): Integer;
var
  i: Integer;
begin
  i := 0;
  while (i < count) and not((Items[i].FAssigned = 1) and
    (Items[i].FSlotIndex = aSlotIndex) and
    (Items[i].FLocation = TBits2(aLocation)) and
    (Items[i].FModuleIndex = aModuleIndex) and
    (Items[i].FParamIndex = aParamIndex)) do
    inc(i);
...
procedure TGlobalKnobList.DeleteModule(aSlotIndex, aLocation,
  aModuleIndex: Byte);
var
  i: Integer;
begin
  for i := 0 to count - 1 do
    if (Items[i].FModuleIndex = aModuleIndex) and
      (Items[i].FLocation = aLocation) and (Items[i].FSlotIndex = aSlotIndex)
    then
      Items[i].FAssigned := 0;
end;
...
begin
  ...
  begin
    Items[i].Read(aChunk);
    if assigned(aChunk.FLogLines) and (Items[i].FAssigned = 1) then
      aChunk.FLogLines.Add(Format('%3d %3d %3d %3d %3d %3d',
        [i, Items[i].FLocation, Items[i].FModuleIndex, Items[i].FIsLed,
        Items[i].FParamIndex, Items[i].FSlotIndex]));
  end;
end;
  ...
  begin
    ...
    begin
      case TLocationType(Knob.FLocation) of
        ...
        ltPatch, ltFX, ltVA:
          begin
            Module := FSlotArray[Knob.FSlotIndex].GetPatch.GetModule
              (Knob.FLocation, Knob.FModuleIndex);
            if assigned(Module) then
            begin
              Param := Module.Parameter[Knob.FParamIndex];
              if assigned(Param) then
      ...
      C_PERF_SETTINGS:
        begin // performance description
          ...
          begin
            ...
            aChunk.FLogLines.Add('Keyb range : ' +
              IntToStr(FKeyboardRangeEnabled));
...
function TG2FilePerformance.LoadFromFile(aStream: TStream;
aLogLines: TStrings): boolean;
var
  sl: TStringList;
  Chunk: TPatchChunk;
...
class function TG2FilePerformance.LoadFileStream(AOwner: TComponent;
...
procedure TG2FilePerformance.DeleteModuleFromPerf(aSlotIndex: Byte;
aLocation: TLocationType; aModule: TG2FileModule);
var
  aModuleIndex: Byte;
begin
...
function TG2FilePerformance.AssignGlobalKnobInPerf(aKnobIndex: Integer;
  aSlotIndex: Byte; aLocation: TLocationType; aModuleIndex, aParamIndex: Byte): TGlobalKnob;
var
  Knob: TGlobalKnob;
begin
  Result := nil;
  if aKnobIndex < 0 then
    Exit;
  if aKnobIndex < FGlobalKnobList.count then
  begin
...
begin
  if (KnobIndex >= 0) and (KnobIndex < FGlobalKnobList.count) then
  begin
...
function TG2FileDataStream.LoadFileHeader(aStream: TStream;
aChunk: TPatchChunk): boolean;
var
  sl: TStringList;
  s: string;
  b: Byte;
...
function TG2FileDataStream.CreateFromMidiStream(const aStream: TStream;
  aLogLines: TStrings): TMemoryStream;
var
  MData, i, SlotIndex, Offs: Integer;
  b: Byte;
  FName: string;
  BlockCount: Integer;
  ...
  try
    ...
    Begin
      ...
      if ParseMidiSeptet(aStream) <> MData then // Checksum
        raise Exception.Create
          (Format('Invalid MIDI data, checksum error in block %d',
          [CurrentBlock]));
...
procedure TG2FileDataStream.CreateMidiBlock(aBlockNr, aBlockCount, anOffset,
  aSize: Integer; aChunk: TPatchChunk; aMidiStream: TStream);
var
  i, bits_left: Integer;
  b: Byte;
Begin
  ...
  case PatchType of
    PATCH_DATA:
      AddMidiByte(aMidiStream, $20);
    PERF_DATA:
      AddMidiByte(aMidiStream, $28);
  else
...
begin
  ...
  try
    ...
    BlockCount := (Chunk.GetReadBufferBitsLeft div 7 + MIDI_BLOCK_SIZE - 1)
      div MIDI_BLOCK_SIZE;
    BlockNr := 0;
    Offset := 0;
    SaveSize := Min(Remaining, MIDI_BLOCK_SIZE);
    // while Remaining > 0 do begin
    while Chunk.GetReadBufferBitsLeft > 0 do
    begin
...
function TG2FileDataStream.ParseMidiConst(const aStream: TStream;
aValue: Byte): Byte;
var
  b: Byte;
Begin
  ...
  if b <> aValue then
    raise Exception.Create
      (Format('Invalid MIDI dump, %.2x expected but %.2x read', [aValue, b]));
...
Begin
  ...
  if b > $7F then
    raise Exception.Create
      (Format('Invalid MIDI dump, %.2x is not MIDI data', [b]));
  Result := b;
  FChecksum := (FChecksum + Result) And $7F;
end;
...
class function TG2FileDataStream.LoadFileData(AOwner: TComponent;
  aStream: TStream; aLogLines: TStrings): TG2FileDataStream;
var
  Chunk: TPatchChunk;
  bm, bl: Byte;
  Crc: Word;
...
class function TG2FileDataStream.LoadMidiData(AOwner: TComponent;
  aStream: TStream; aLogLines: TStrings): TG2FileDataStream;
var
  Chunk: TPatchChunk;
  b: Byte;
...
class function TG2FileDataStream.LoadFileStream(AOwner: TComponent;
aStream: TStream; aChunk: TPatchChunk): TG2FileDataStream;
begin
  // Abstract
  Result := nil;
...
class function TG2FileDataStream.LoadMidiStream(AOwner: TComponent;
...
function TBankList.FindIndex(aPatchFileType: TPatchFileType;
aBank, aPatch: Byte): Integer;
var
  i: Integer;
begin
  i := 0;
  while (i < count) and not((aPatchFileType = GetBankItem(i).PatchFileType) and
    (aBank = GetBankItem(i).Bank) and (aPatch = GetBankItem(i).Patch)) do
    inc(i);
  if (i < count) then
    Result := i
  else
...
begin
  // Find bank
  i := 0;
  while (i < count) and not((Items[i].PatchFileType = aPatchFileType) and
    (Items[i].Bank = aBank)) do
    inc(i);
  if (i < count) then
  begin
    while (i < count) and ((Items[i].PatchFileType = aPatchFileType) and
      (Items[i].Bank = aBank) and (Items[i].Patch <= aPatch)) do
      inc(i);
    if (i < count) and ((Items[i].PatchFileType = aPatchFileType) and
      (Items[i].Bank = aBank)) then
      Result := Items[i]
    else
      Result := nil;
  end
...
function TBankList.FindFirstLast(aPatchFileType: TPatchFileType; aBank: Byte;
  var aFirstLocation, aLastLocation: Byte): boolean;
var
  i: Integer;
begin
...
begin
  i := 0;
  while (i < FMidiToKnobList.count) and
    not((FMidiToKnobList[i].FSource = aSource) and
    (FMidiToKnobList[i].FControlType = aControlType) and
    (FMidiToKnobList[i].FKnob = aKnob)) do
    inc(i);
  if not(i < FMidiToKnobList.count) then
  begin
...
begin
  i := 0;
  while (i < FMidiToKnobList.count) and
    not(FMidiToKnobList[i].FMidiCC = aMidiCC) do
    inc(i);
  Result := (i < FMidiToKnobList.count);
end;

function TG2File.TextFunction(No: Integer;
Param1, Param2, Param3: Byte): string;
begin
  case No of
    0:
      begin
...
begin
  if (FLogLevel < 2) or ((not assigned(FLogLines)) and
    (not assigned(FOnLogLine))) then
    Exit;
  FLogLock.Acquire;
  try
    c := 0;
