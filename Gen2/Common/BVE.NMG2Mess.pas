...
uses
  System.Classes, System.SysUtils, System.Generics.Collections, System.Character,
  System.Generics.Defaults,
  BVE.NMG2Types, BVE.NMG2Data, BVE.NMG2File;
...
  TPatchNameChangeEvent = procedure(Sender: TObject; SenderID : integer; PatchIndex : integer; PatchName : string) of object;
  TPerfNameChangeEvent = procedure(Sender: TObject; SenderID : integer; PerfName : string) of object;
...
  TSetModuleColorEvent = procedure(Sender: TObject; SenderID : integer; PatchIndex : byte; Location : TLocationType;  ModuleIndex : byte) of object;
  ...
  published
    ...
    property    OnBeforePatchUpdate : TPatchUpdateEvent read FOnBeforePatchUpdate write FOnBeforePatchUpdate;
    ...
    property    OnPerfUpdate : TPerfUpdateEvent read FOnPerfUpdate write FOnPerfUpdate;
    ...
    property    OnSetModuleColor : TSetModuleColorEvent read FOnSetModuleColor write FOnSetModuleColor;
  ...
  public
    ...
    function    CreateSetPerformanceMessage( aPerfName : string; aPerf : TG2FilePerformance): TG2SendMessage;
  ...
  private
    ...
    procedure   AddCopyParametersMessage( SendMessage : TG2SendMessage; aSrcePatch : TG2FilePatchPart; aToLocation : TLocationType; aFromVariation, aToVariation : integer);
    ...
    procedure   AddSetModuleParamLabelsMessage( SendMessage : TG2SendMessage; aLocation: TLocationType; aModuleIndex, aParamIndex, aLabelIndex: byte; aName: string);
    procedure   AddSetModuleLabelMessage( SendMessage : TG2SendMessage; aLocation: TLocationType; aModuleIndex: byte; aName: string);
  ...
  protected
    ...
    function    CreateCopyParametersMessage( aSrcePatch : TG2FilePatchPart; aToLocation : TLocationType; aFromVariation, aToVariation : integer): TG2SendMessage;
    ...
    function    CreateSetModuleParamLabelsMessage( aLocation : TLocationType; aModuleIndex, aParamIndex, aLabelIndex : byte; aName : string): TG2SendMessage;
    function    CreateSetModuleLabelMessage( aLocation : TLocationType; aModuleIndex : byte; aName : string): TG2SendMessage;
  ...
  public
    ...
    function    CheckParametersFit( aSrcePatch: TG2FilePatchPart; aToLocation: TLocationType): boolean;
...
begin
  ...
  Result := (PStaticByteBuffer( Memory)^[1] in [$00, $01, $02, $03, $08, $09, $0A, $0B])
        and (PStaticByteBuffer( Memory)^[3] in [R_RESOURCES_USED]);
end;
...
procedure TG2Mess.DoAfterClear( PatchFileType : TPatchFileType; BankIndex, PatchIndex : byte);
begin
                          ...
                          repeat
                                    ...
                                    while MemStream.Read(b, 1) = 1 do begin
                                      ...
                                      $05 : begin; // last patch in message read
                                        ...
                                        if b = $00 then begin
                                          ...
                                          FslBanks.Add('Category : ' + IntToStr(aCategory) + ' Mode:' + IntToStr( ord(aPatchFileType)) + ' Bank: ' + IntToStr(aBank)
                            ...
                            S_PATCH_BANK_DATA :
                                  begin
                                    ...
                                    patch_name := '';
                                    while (Patch_name.Length < 16) and (MemStream.Read(b, 1) = 1) and (b <> 0) do begin
                                      Patch_name := Patch_name + Char(b);
                                    end;
               ...
               S_CLEAR_BANK :
                ...
                S_RETREIVE :
                  begin
                    ...
                    if Slot = 4 then
                      (Performance as TG2MessPerformance).DoAfterRetreivePatch( Slot, Bank, Patch)
                    else
...
var SubCmd, b, hb, lb, Version, Slot : byte;
    perf_name : string;
    Chunk : TPatchChunk;
begin
  // Return True if processed
    ...
    if Version = FPerfVersion then begin
      ...
      C_PERF_SETTINGS : // Strange, sometimes received after store : because patch bank location is in slot settings...
            begin // Performance settings
              // its actially first perf name and then the chunk
              perf_name := '';
              while (perf_name.Length < 16) and (MemStream.Read(b, 1) = 1) and (b <> 0) do begin
                perf_name := perf_name + Char(b);
              end;
...
function TG2MessPerformance.CreateSetPerformanceMessage( aPerfName : string; aPerf : TG2FilePerformance): TG2SendMessage;
var Chunk : TPatchChunk;
    i : integer;
begin
...
function TG2MessPerformance.CreateSetPerfNameMessage( aPerfName : string): TG2SendMessage;
begin
...
begin
  ...
  if assigned(G2) then begin
    if assigned((G2 as TG2Mess).FOnvariationChange) then
     (G2 as TG2Mess).FOnVariationChange(G2, G2.ID, SlotIndex, Patch.ActiveVariation);
  end else
...
var aVersion, aSubCmd, aLocation, aModuleIndex, aParamIndex, b, mask, unknown,
    ...
    if aVersion = FPatchVersion then begin
      ...
      S_SEL_PARAM :
            begin // selected parameter
              ...
              Patch.SelectParamInPatch( TLocationType(aLocation), aModuleIndex, aParamIndex);
      ...
      R_VOLUME_DATA :
            begin // Volume indicator data
              ...
              while (i < Patch.LedStripList.Count) do begin
                ...
                //try
                  ...
                  if assigned(Patch.OnLed) then
                    Patch.OnLed( Patch, Led.Location, Led.ModuleIndex, Led.GroupID, 0, b);
      ...
      R_LED_DATA : begin // Led data
              ...
              while (i < Patch.LedList.Count) do begin
                ...
                if assigned(Patch.OnLed) then
                  Patch.OnLed( Patch, Led.Location, Led.ModuleIndex, Led.GroupID, 0, (b and mask) shr j);
      ...
      S_SET_PARAM :
            begin // ParamChange
              ...
              if aModuleIndex <> 0 then
                ...
                Patch.SetParamInPatch(TLocationType(aLocation), aModuleIndex, aParamIndex, aVariation, aValue);
            end;
      R_RESOURCES_USED :
              ...
              if not Result then begin
                ...
                S_SET_MODE :
                      begin
                        ...
                        Patch.SetModeInPatch( TLocationType(aLocation), aModuleIndex, aParameterIndex, aValue);
                      end;
                S_COPY_VARIATION :
                      begin
                        ...
                        DoCopyVariation;
                        if aToVariation = Patch.ActiveVariation then begin
...
function TG2MessSlot.CreateSetPatchMessage( aPatchName : string; aPatch : TG2FilePatch): TG2SendMessage;
var Chunk : TPatchChunk;
begin
  ...
  try
    ...
    aPatch.Write(Chunk, N_USB_VARIATIONS); // 10 Variations must be in a patch for USB, unclear what #10 holds
  finally
    Chunk.Free;
  end;
end;
...
procedure TG2MessPatch.AddSetModuleParamLabelsMessage( SendMessage : TG2SendMessage; aLocation: TLocationType; aModuleIndex, aParamIndex, aLabelIndex : byte; aName: string);
var MemStream : TG2Message;
    Chunk : TPatchChunk;
...
begin
  MemStream := TG2Message.Create;
  try
    ParamLabelModuleList := ParameterLabelList[ ord(aLocation)].FindParamLabelModule( aModuleIndex);
    if assigned( ParamLabelModuleList) then begin
      // Record with Param labels for the module already exists, copy record and change label
      NewParamLabelModule := TParamLabelModuleList.CopyCreate( True, PatchPart[ord(aLocation)], ParamLabelModuleList);
      NewParamLabelModule.AddParamLabel( aParamIndex, aLabelIndex, aName);
    end else begin
      // Create a new record
      NewParamLabelModule := TParamLabelModuleList.Create( True, PatchPart[ord(aLocation)]);
...
begin
  MemStream := TG2Message.Create;
  try
    ParamLabelModuleList := ParameterLabelList[ ord(aLocation)].FindParamLabelModule( aModule.ModuleIndex);
    if assigned( ParamLabelModuleList) then begin
      // Record with Param labels for the module already exists, copy record and change label
      NewParamLabelModuleList := TParamLabelModuleList.CopyCreate( True, PatchPart[ord(aLocation)], ParamLabelModuleList);
    end else begin
      // Create a new empty record
      NewParamLabelModuleList := TParamLabelModuleList.Create( True, PatchPart[ord(aLocation)]);
...
procedure TG2MessPatch.AddSetModuleLabelMessage( SendMessage : TG2SendMessage; aLocation: TLocationType; aModuleIndex: byte; aName: string);
var MemStream : TG2Message;
begin
  MemStream := TG2Message.Create;
  try
  ...
  try
    ...
    try
      ...
      if aModule.ParameterCount = 0 then begin
        ...
        for j := 0 to N_USB_VARIATIONS - 2 do begin // Variations
          Chunk.WriteBits( j, 8);

          for k := 0 to aModule.ParameterCount - 1 do begin
            //Chunk.WriteBits( aModule.Parameter[k].GetParameterValue, 7);
            Chunk.WriteBits( aModule.PatchPart.FindParamValue( aModule.ModuleIndex, j, k), 7);
          end;
        end;
      ...
      try
        ...
        for m := 0 to aModulesToReplaceList.Count - 1 do begin
          ...
          if Module.Col = i then begin
            ...
            j := 0;
            while (j<DestColList.Count) and (DestColList[j].Row < DestModuleOrder.Row) do
              inc(j);
        ...
        for m := 0 to aModulesMovedList.Count - 1 do begin
          ...
          if Module.NewCol = i then begin
            ...
            j := 0;
            while (j<SrceColList.Count) and (SrceColList[j].Row < SrceModuleOrder.Row) do
              inc(j);
        ...
        for m := 0 to SrceColList.Count - 1 do begin
          j := 0;
          while (j<DestColList.Count) and ((DestColList[j]).Row < SrceColList[m].Row) do
            inc(j);

          SrceModuleOrder :=  TModuleColOrder.Create;
          SrceModuleOrder.ModuleIndex := TModuleColOrder(SrceColList[m]).ModuleIndex;
        ...
        for m := 1 to DestColList.Count - 1 do begin
          if new_row > DestColList[m].Row then begin
        ...
        for m := 0 to DestColList.Count - 1 do begin
          ...
          if (TModuleColOrder(DestColList[m])).Source then begin
            // source patch, just alter the row in the srcepatch
            j := 0;
            while (j<aModulesMovedList.Count) and (aModulesMovedList[j].ModuleIndex <> DestColList[m].ModuleIndex) do
              inc(j);
            ...
            // Destination patch
            j := 0;
            while (j < aModulesToReplaceList.Count) and ( aModulesToReplaceList[j].ModuleIndex <> DestColList[m].ModuleIndex) do
              inc(j);
            ...
            if (j < aModulesToReplaceList.Count) then begin
              ...
              if Module.Row <> DestColList[m].Row then begin
                AddMoveModuleMessage( FUndoMessage, Module, Module.Col, Module.Row);
                AddMoveModuleMessage( SendMessage, Module, Module.Col, DestColList[m].Row);
              end;
            end else begin
              // Shouldnt happen....
            end;
...
begin
  ...
  try
    ...
    ModuleName := ModuleDefs[mi].ModuleName + IntToStr(PatchPart[ ord(aLocation)].GetUniqueModuleNameSeqNr( ModuleDefs[mi].ModuleName));
    ...
    try
      ...
      if count > 0 then begin
        ...
        for i := 0 to N_USB_VARIATIONS - 1 do begin               // Variations
          Chunk.WriteBits( i, 8);
      ...
      if aModuleTypeID = 121 then begin
        ...
        count := GetDataParamCount(aModuleTypeID);
        ParameterLabelList := TParameterLabelList.Create(True, PatchPart[ord(aLocation)]);
        if count > 0 then begin
          try
            for i := 0 to count - 1 do begin
              pi := GetDataModuleParamIndex(aModuleTypeID, i);
              if ModuleParams[pi].slParamLabel <> '' then begin
                ParameterLabelList.AddParamLabels( aNewModuleIndex, i, ModuleParams[pi].slParamLabel);
              end;
            end;
...
var Chunk : TPatchChunk;
  ...
  begin
    i := 0;
    while (i<RenumberTable.Count) and (RenumberTable[i].OldModuleIndex <> aOldModuleIndex) do
      inc(i);
  ...
  try
    ...
    for i := 0 to aSrcePatch.ModuleList.Count - 1 do begin
      ...
      if RenumberModules then begin
        ...
        repeat
          j := 0;
          while (j<RenumberTable.Count) and (RenumberTable[j].NewModuleName <> aSrcePatch.ModuleList[i].ModuleFileName + IntToStr(ext)) do
            inc(j);
        ...
        until not found;
        ModuleIndex.NewModuleName := aSrcePatch.ModuleList[i].ModuleFileName + IntToStr(ext);
      end else begin
    ...
    try
      ...
      for i := 0 to aSrcePatch.ParameterLabelList.Count - 1 do begin
        Chunk.WriteBits( GetNewModuleIndex( aSrcePatch.ParameterLabelList[i].ModuleIndex), 8);
        Chunk.WriteBits( aSrcePatch.ParameterLabelList[i].GetModuleLabelsLength, 8);
        for j := 0 to aSrcePatch.ParameterLabelList[i].Count - 1 do begin
      ...
      for i := 0 to  aSrcePatch.ModuleLabelList.Count - 1 do begin

        Chunk.WriteBits( GetNewModuleIndex( aSrcePatch.ModuleLabelList[i].ModuleIndex), 8);
        //Chunk.WriteName( aSrcePatch.ModuleLabelList[i].ModuleLabel);
        Chunk.WriteName( GetNewModuleName( aSrcePatch.ModuleLabelList[i].ModuleIndex));
      end;
      Chunk.WriteChunk( $5a);
...
function TG2MessPatch.CheckParametersFit( aSrcePatch: TG2FilePatchPart; aToLocation: TLocationType): boolean;
var SrceModuleList, DestModuleList : TList<TG2FileModule>;
    sl : TStringList;
    i, m : integer;
begin
  Result := False;
  ...
  try
    ...
    for i := 0 to sl.Count - 1 do begin
      ...
      for m := 0 to PatchPart[ ord(aToLocation)].SelectedModuleList.Count - 1 do begin
        if IntToStr(PatchPart[ ord(aToLocation)].SelectedModuleList[m].TypeID) = sl[i] then
...
procedure TG2MessPatch.AddCopyParametersMessage(SendMessage: TG2SendMessage;
  aSrcePatch: TG2FilePatchPart; aToLocation: TLocationType; aFromVariation,
  aToVariation : integer);
var PositionComparison : TComparison<TG2FileModule>;
 ...
 begin
  ...
  PositionComparison :=
    function(const Module1, Module2: TG2FileModule): integer
    begin
  ...
  try
    SrceModuleList := TList<TG2FileModule>.Create(TComparer<TG2FileModule>.Construct(PositionComparison));
    DestModuleList := TList<TG2FileModule>.Create(TComparer<TG2FileModule>.Construct(PositionComparison));
    sl := TStringList.Create;
    ...
    try
      ...
      for i := 0 to sl.Count - 1 do begin
        ...
        // Copy Morphs
        for m := 0 to SrceModuleList.Count - 1 do begin

          SrceVariationList := TModuleVariationList.Create(True, nil{, SrceParamList});
          ...
          DestVariationList := TModuleVariationList.Create(True, nil{, DestParamList});
          ...
          for p := 0 to DestModuleList[m].ParameterCount - 1 do begin
            SrceParamValues.ParamValues[p] := aSrcePatch.FindParamValue( SrceModuleList[m].ModuleIndex, aFromVariation, p);
            DestParamValues.ParamValues[p] := GetParameterValue( aToLocation, DestModuleList[m].ModuleIndex, p, aToVariation);

            for r := 0 to 7 do begin

              MorphValue := aSrcePatch.GetMorphValue(SrceModuleList[m].ModuleIndex, p, r, aFromVariation);
              if MorphValue >= 128 then begin
                MorphValueSign := 1;
                MorphValue := abs(MorphValue - 256);
              end else
                MorphValueSign := 0;

              if MorphValue <> 0 then begin
                AddSetMorphMessage( SendMessage,
                                    ord(aToLocation),
                                    DestModuleList[m].ModuleIndex,
                                    p,
                                    r,
                                    MorphValue,
                                    MorphValueSign,
                ...
                MorphValue := GetMorphValue(aToLocation, DestModuleList[m].ModuleIndex, p, r, aToVariation);
                if MorphValue >= 128 then begin
                  MorphValueSign := 1;
                  MorphValue := abs(MorphValue - 256);
                end else
                ...
                AddSetMorphMessage( FUndoMessage,
                                    ord(aToLocation),
                                    DestModuleList[m].ModuleIndex,
                                    p,
                                    r,
...
begin
  Result := CreatePatchMessage;
  AddCopyParametersMessage( Result, aSrcePatch, aToLocation, aFromVariation, aToVariation);
end;
  ...
  try
    ...
    for p := 0 to Module.ParameterCount - 1 do begin
      ...
      for k := 0 to NMORPHS - 1 do begin
        for v := 0 to N_VARIATIONS - 1 do begin // TODO: Maybe N_USB_VARIATIONS?
...
function TG2MessPatch.CreateSetModuleParamLabelsMessage( aLocation: TLocationType; aModuleIndex, aParamIndex, aLabelIndex: byte; aName: string): TG2SendMessage;
begin
  Result := CreatePatchMessage;
...
function TG2MessPatch.CreateSetModuleLabelMessage( aLocation: TLocationType; aModuleIndex: byte; aName: string): TG2SendMessage;
begin
    ...
    repeat
              ...
              if assigned( Module) then begin
                ...
                if assigned(G2) and assigned(Slot) then
                  if assigned((G2 as TG2Mess).OnSetModuleColor) then
                    (G2 as TG2Mess).OnSetModuleColor( G2, G2.ID,  Slot.SlotIndex, aLocation, aModuleIndex);
      ...
      S_SET_MORPH_RANGE :
            begin
              ...
              if aNegative = 1 then
                inherited SetMorphValue( aLocation, aModuleIndex, aParamIndex, aMorphIndex, 256 - aValue, aVariation)
              else
                inherited SetMorphValue( aLocation, aModuleIndex, aParamIndex, aMorphIndex, aValue, aVariation);
            end;
      C_PATCH_DESCR,
      C_MODULE_LIST,
      C_CURRENT_NOTE_2,
