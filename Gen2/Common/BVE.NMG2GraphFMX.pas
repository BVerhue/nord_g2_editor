...
type
  ...
  TCreateGraphModuleFMXEvent = procedure(Sender: TObject;
    aLocation: TLocationType; aModule: TG2GraphModuleFMX) of Object;
  TCreateGraphCableFMXEvent = procedure(Sender: TObject;
    aLocation: TLocationType; aCable: TG2GraphCableFMX) of Object;
  TCreateG2ControlEvent = procedure(Sender: TObject; aControl: TG2BaseControl)
    of Object;
  ...
  public
    ...
    destructor Destroy; override;
    property ModulePanelDefs: TModuleDefList read FModulePanelDefs
      write FModulePanelDefs;
  end;
  ...
  public
    ...
    function CreateModule(aLocation: TLocationType; aModuleIndex: byte;
      aModuleType: byte): TG2FileModule; override;
    function CreateCable(aLocation: TLocationType; aColor: byte;
      ...
      aToConnector: byte): TG2FileCable; override;
    ...
    property SplitterVisible: Boolean read FSplitterVisible
      write FSplitterVisible;
    property SelectMultiple: Boolean read FSelectMultiple write FSelectMultiple
      default False;
    property OnCreateModuleFMX: TCreateGraphModuleFMXEvent
      read FOnCreateModuleFMX write FOnCreateModuleFMX;
    property OnCreateCableFMX: TCreateGraphCableFMXEvent read FOnCreateCableFMX
      write FOnCreateCableFMX;
  end;
  ...
  public
    ...
    constructor CopyCreate(aPatchPart: TG2FilePatchPart;
    ...
    property OnCreateG2Control: TCreateG2ControlEvent read FOnCreateG2Control
      write FOnCreateG2Control;
  end;
...
procedure ParseDependencies(aModule: TG2FileModule;
  aControl: TG2MultiReadControl);
...
begin
  ...
  begin
    ...
    begin
      ...
      for l := 0 to 1 do
      begin
        for c := 0 to Patch.PatchPart[l].CableList.Count - 1 do
        begin
...
begin
  ...
  begin
    ...
    begin
      ...
      for l := 0 to 1 do
      begin
        for m := 0 to Patch.PatchPart[l].ModuleList.Count - 1 do
        begin
          Module := Patch.PatchPart[l].ModuleList[m] as TG2GraphModuleFMX;
          G2Module := Module.G2Module;
          for c := 0 to G2Module.ChildrenCount - 1 do
          begin
...
function TG2GraphPatchFMX.CreateModule(aLocation: TLocationType;
  aModuleIndex, aModuleType: byte): TG2FileModule;
var
  i: Integer;
  Module: TG2GraphModuleFMX;
...
function TG2GraphPatchFMX.CreateCable(aLocation: TLocationType;
  aColor, aFromModule, aFromConnector, aLinkType, aToModule, aToConnector: byte): TG2FileCable;
var
  i: Integer;
...
begin
  ...
  if not assigned(ModuleFrom) then
    raise Exception.Create('ModuleIndex ' + IntToStr(aFromModule) +
      ' not found.');
  ModuleTo := GetModule(ord(aLocation), aToModule) as TG2GraphModuleFMX;
  if not assigned(ModuleTo) then
    raise Exception.Create('ModuleIndex ' + IntToStr(aToModule) +
      ' not found.');
...
function TG2GraphPatchFMX.CreatePatchPart(aLocation: TLocationType): TG2FilePatchPart;
begin
...
begin
  case aColor of
    0:
      Result := PatchDescription.RedVisible = 1;
    1:
      Result := PatchDescription.BlueVisible = 1;
    2:
      Result := PatchDescription.YellowVisible = 1;
    3:
      Result := PatchDescription.OrangeVisible = 1;
    4:
      Result := PatchDescription.GreenVisible = 1;
    5:
      Result := PatchDescription.PurpleVisible = 1;
    6:
      Result := PatchDescription.WhiteVisible = 1;
  else
    Result := False;
  end;
end;
...
begin
  for i := 0 to 1 do
  begin
    for j := 0 to PatchPart[i].CableList.Count - 1 do
    begin
...
var
  ConnectorTo: TG2FileConnector;
  ModuleTo: TG2FileModule;
begin
  ModuleTo := Module.PatchPart.Patch.Modules[ord(Location),
    aToConnector.ModuleIndex];
  ...
  begin
    ...
    if assigned(ConnectorTo) then
      (Module.PatchPart.Patch as TG2GraphPatchFMX).MessAddConnection(Location,
        self, ConnectorTo);
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      FG2Connector.ConnectorColor :=
        ConvertToAlpha(CableColors[ConnectorColor]);
...
var
  ModuleFrom, ModuleTo: TG2Module;
  ConnectorFrom, ConnectorTo: TG2Connector;
  p: TPointF;
begin
  if not(assigned(FG2Cable) and assigned(FromConnector) and
    assigned(ToConnector) and assigned(FromConnector.Module) and
    assigned(ToConnector.Module)) then
    Exit;
...
begin
  ...
  FG2Module.Row := Row;
  FG2Module.Col := Col;
  FG2Module.StateStyleList.DefaultFill.Color :=
    ConvertToAlpha(ModuleColors[ModuleColor]);
  FG2Module.StateStyleList.SelectedFill.Color :=
    ConvertToAlpha(ModuleColors[ModuleColor]);
end;
...
begin
  ...
  begin
    FG2Module.StateStyleList.DefaultFill.Color :=
      ConvertToAlpha(ModuleColors[aValue]);
    FG2Module.StateStyleList.SelectedFill.Color :=
      ConvertToAlpha(ModuleColors[aValue]);
    FG2Module.Redraw;
  end;
...
begin
  ...
  begin
    ...
    if assigned(FG2Module) then
      FG2Module.Selected := aValue;
    if aValue and aExclusive and assigned(FG2Module) and assigned(SelectedParam)
    then
    begin
      // Set focus on selected parameter control
      if FG2Module.ChildrenCount > 0 then
      begin
        for Control in FG2Module.Children do
        begin
          if Control is TG2WriteControl then
          begin
            if (Control as TG2WriteControl).DataWriter = SelectedParam as IG2DataParam
            then
            begin
              (Control as TG2WriteControl).SetFocus;
              Exit;
...
begin
  ...
  try
    ...
    for i := 0 to G2Module.ChildrenCount - 1 do
    begin
      G2Control := (G2Module.Children[i] as TG2BaseControl);
      SaveMatrix := aCanvas.Matrix;
      try
        {$IFDEF Ver260Up}
        aCanvas.SetMatrix(TMatrix.CreateTranslation(G2Control.UnscaledLeft,
...
begin
  ...
  try
    ...
    if aControl.MasterRef >= 0 then
    begin
      i := 0;
      while (i < sl.Count) and (sl[i] <> IntToStr(aControl.MasterRef)) and
        (sl[i] <> 's' + IntToStr(aControl.MasterRef)) do
        inc(i);
      if (i < sl.Count) then
      begin
        // Assign master parameter first
        if (Lowercase(sl[i][1]) = 's') then
        begin // One of the static params
...
var
  ...
  var
    Connector: TG2GraphConnectorFMX;
  begin
    ...
    if not assigned(Connector) then
      raise Exception.Create('Connector ' + IntToStr(aControl.CodeRef) +
  ...
  begin
    ...
    begin
      i := 0;
      while (i < Patch.LedList.Count) and
        not((Patch.LedList[i].Location = aModule.Location) and
        (Patch.LedList[i].ModuleIndex = aModule.ModuleIndex) and
        (Patch.LedList[i].GroupID = aLed.LedGroupID)) do
        inc(i);
      if (i < Patch.LedList.Count) then
      begin
  ...
  begin
    ...
    begin
      i := 0;
      while (i < Patch.LedStripList.Count) and
        not((Patch.LedStripList[i].Location = aModule.Location) and
        (Patch.LedStripList[i].ModuleIndex = aModule.ModuleIndex) and
        (Patch.LedStripList[i].GroupID = aLed.LedGroupID)) do
        inc(i);
      if (i < Patch.LedStripList.Count) then
      begin
  ...
  begin
    ...
    AssignWriterToParameter(Result);
    if assigned(aModule.PatchPart) and assigned(aModule.PatchPart.Patch) and
      assigned(aModule.PatchPart.Patch.G2) then
    begin
