...
type
  ...
  public
    ...
    property ModulePanelDef[const aName : string] : TModuleDefStream read GetModulePanelDef;
  end;
  ...
  public
    ...
    destructor  Destroy; override;
    function    GetNextString( var source : string; EndTokens : array of Char): string;
    function    GetNextByte( var source : string; EndTokens : array of Char): Byte;
    function    GetNextInteger( var source : string; EndTokens : array of Char): Integer;
    procedure   ReadSpaces;
    function    ReadUntil( EndTokens : array of Char): string;
    function    ReadConst( value : string): boolean;
    function    ReadOptions( sl : TStrings; ListTokens, EndTokens : array of Char): integer;
    function    PeekValue( Name : string; ValueEndTokens, EndTokens : array of Char): string;
...
begin
  ...
  try
    ...
    while i < sl.Count do begin
      if sl[i].Contains('<#File:') then begin
        c := 7;
        module_name := '';
        while sl[i].Chars[c] <> '#' do begin
        ...
        Objects[m] := MemStream;
        inc(i);
        while (i < sl.Count) and (not sl[i].Contains('<#File:')) do begin
          line := sl[i];
          for c := 0 to line.Length - 1 do begin
            b := byte(line.Chars[c]);
            MemStream.Write( b, 1);
          end;
          b := 13;
...
function TModuleDefList.GetModulePanelDef(const aName: string): TModuleDefStream;
var i : integer;
begin
  i := IndexOf(aName);
  if i >= 0 then
...
function TModuleDefStream.GetNextString( var source : string; EndTokens : array of Char): string;
var i : integer;
begin
  Result := '';
  i := 0;
...
function TModuleDefStream.GetNextByte( var source : string; EndTokens : array of Char): Byte;
var value : string;
begin
...
function TModuleDefStream.GetNextInteger( var source : string; EndTokens : array of Char): Integer;
var value : string;
begin
  value := GetNextString( source, EndTokens);
  Result := StrToInt(value);
...
var b : byte;
begin
  while (Position < Size) and (Read( b, 1) = 1) and (Char(b).IsInArray([' ', #9, #10, #13])) do begin
  end;
...
begin
  Result := '';
  while ( Position < Size) and ( Read( b, 1) = 1) and not(Char(b).IsInArray(EndTokens)) do begin
...
begin
  Result := false;
  i := 1;
  while ( Position < Size) and (i<=Length(Value)) and ( Read( b, 1) = 1) and (value[i] = Char(b)) do begin
    inc(i);
  end;
  Result := True;
end;

function TModuleDefStream.ReadOptions( sl : TStrings; ListTokens, EndTokens : array of Char): integer;
var b : byte;
    s : string;
begin
  while ( Position < Size) and ( Read( b, 1) = 1) and not(Char(b).IsInArray(EndTokens)) do begin
    if (Char(b).IsInArray(ListTokens)) then begin
      sl.Add(s);
      s := '';
    end else
      s := s + Char(b);
...
function TModuleDefStream.PeekValue( Name : string; ValueEndTokens, EndTokens : array of Char): string;
...
begin
  ...
  try
    ...
    start := Position;
    Read(c, 1);
    while (Position < Size) and not(found) and not( c.IsInArray(EndTokens)) do begin
      if c <> Name.Chars[Position - 1 - start] then begin
        Start := Position;
      end else
    ...
    if Found then begin
      Read(c, 1);
      while ( Position < Size) and not( c.IsInArray(ValueEndTokens)) do begin
        Result := Result + c;
        Read(c, 1);
      end;
    end;
...
begin
  i := 0;
  while (i < Value.Length) and (i<16) do begin
