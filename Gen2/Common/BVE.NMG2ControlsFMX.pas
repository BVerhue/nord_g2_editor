...
type
  ...
  TControlState = (csDefault, csFocused, csSelected, csFocusedSelected,
    csDisabled);
  ...
  TPaintElementEvent = procedure(Sender: TObject;
    const aElementType: TControlElementType; aElementIndex: Integer;
    aStyleSet: TG2StyleSet) of Object;
  ...
  TOctaveButtonChangeEvent = procedure(Sender: TObject; aOctave: Integer;
  ...
  public
    ...
    procedure DrawTextExt(aCanvas: TCanvas; const aFont: TFont;
    ...
    property UnscaledBoundsRect: TRectF read GetUnscaledBoundsRect
      write SetUnscaledBoundsRect;
    ...
    property UnscaledPosition: TPosition read FUnscaledPosition
      write SetUnscaledPosition;
    property UnscaledWidth: Single read GetUnscaledWidth write SetUnscaledWdith;
    property UnscaledHeight: Single read GetUnscaledHeight
      write SetUnscaledHeight;
  published
    property Align;
    property Anchors;
  ...
  [ComponentPlatformsAttribute(PidWin32 Or PidWin64 Or PidOSX32 Or pidOSX64 Or
    PidiOSDevice Or PidiOSSimulator or pidAndroid)]
  {$ELSE}
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    property OnControlGesture: TGestureEvent read FOnControlGesture
      write FOnControlGesture;
    property OnControlMouseUp: TMouseEvent read FOnControlMouseUp
      write FOnControlMouseUp;
    property OnControlMouseDown: TMouseEvent read FOnControlMouseDown
      write FOnControlMouseDown;
    property OnControlMouseMove: TMouseMoveEvent read FOnControlMouseMove
      write FOnControlMouseMove;
    property OnModuleGeture: TGestureEvent read FOnModuleGesture
      write FOnModuleGesture;
    property OnModuleMouseUp: TMouseEvent read FOnModuleMouseUp
      write FOnModuleMouseUp;
    property OnModuleMouseDown: TMouseEvent read FOnModuleMouseDown
      write FOnModuleMouseDown;
    property OnModuleMouseMove: TMouseMoveEvent read FOnModuleMouseMove
      write FOnModuleMouseMove;
  published
    property Zoom: Single read FZoom write SetZoom;
  ...
  public
    ...
    property ActiveMorphColor: TAlphaColor read FActiveMorphColor
      write SetActiveMorphColor;
    property InactiveMorphColor: TAlphaColor read FInactiveMorphColor
      write SetInactiveMorphColor;
  end;
  ...
  published
    ...
    property DefaultStroke: TStrokeBrush read GetDefaultStroke
      write SetDefaultStroke;
    property DefaultFont: TFont read GetDefaultFont write SetDefaultFont;
    property DefaultFontColor: TAlphaColor read GetDefaultFontColor
      write SetDefaultFontColor;
    property FocusedFill: TBrush read GetFocusedFill write SetFocusedFill;
    property FocusedStroke: TStrokeBrush read GetFocusedStroke
      write SetFocusedStroke;
    property FocusedFont: TFont read GetFocusedFont write SetFocusedFont;
    property FocusedFontColor: TAlphaColor read GetFocusedFontColor
      write SetFocusedFontColor;
    property SelectedFill: TBrush read GetSelectedFill write SetSelectedFill;
    property SelectedStroke: TStrokeBrush read GetSelectedStroke
      write SetSlectedStroke;
    property SelectedFont: TFont read GetSelectedFont write SetSelectedFont;
    property SelectedFontColor: TAlphaColor read GetSelectedFontColor
      write SetSelectedFontColor;
    property FocusedSelectedFill: TBrush read GetFocusedSelectedFill
      write SetFocusedSelectedFill;
    property FocusedSelectedStroke: TStrokeBrush read GetFocusedSelectedStroke
      write SetFocusedSelectedStroke;
    property FocusedSelectedFont: TFont read GetFocusedSelectedFont
      write SetFocusedSelectedFont;
    property FocusedSelectedFontColor: TAlphaColor
      read GetFocusedSelectedFontColor write SetFocusedSelectedFontColor;
    property DisabledFill: TBrush read GetDisabledFill write SetDisabledFill;
    property DisabledStroke: TStrokeBrush read GetDisabledStroke
      write SetDisabledStroke;
    property DisabledFont: TFont read GetDisabledFont write SetDisabledFont;
    property DisabledFontColor: TAlphaColor read GetDisabledFontColor
  ...
  published
    property Buffered;
    property StateStyleList: TG2StateStyleList read FStateStyleList
      write SetStateStyleList stored True;
  end;

  TG2BaseControl = class(TG2StyleControl)
  private
    [Weak] FPathDataList: TG2PathDataList;
    FID, FModuleIndex, FZOrder, FCodeRef, FMasterRef, FInfoFunc,
      FTextFunc: Integer;
    FDependencies: string;
    FSelected: Boolean;
    FState: TControlState;
    FLowValue, FHighValue, FValue, FMorphValue: Integer;
    FLocation: TLocationType;
    FNormValue, FNormMorphValue: Single;
    FHasMorph: Boolean;
  ...
  public
    ...
    procedure ProcessMouseDown(Sender: TObject; Shift: TShiftState;
      aTouchID: Integer; X, Y: Single); virtual;
    procedure ProcessMouseMove(Sender: TObject; Shift: TShiftState;
      aTouchID: Integer; X, Y: Single); virtual;
    procedure ProcessMouseUp(Sender: TObject; Shift: TShiftState;
      aTouchID: Integer; X, Y: Single); virtual;
    procedure IncValue;
    procedure DecValue;
    ...
    function ParseProperties(fs: TModuleDefStream; aName: string)
      : Boolean; virtual;
    ...
    property NormMorphValue: Single read GetNormMorphValue
      write SetNormMorphValue;
    ...
    property PathDataList: TG2PathDataList read FPathDataList
      write FPathDataList;
    property MouseEnable: Boolean read FMouseEnable write SetMouseEnable;
    property OnGetTextFunc: TGetTextFuncEvent read FOnGetTextFunc
      write FOnGetTextFunc;
    property OnPaintElement: TPaintElementEvent read FOnPaintElement
      write FOnPaintElement;
  published
  ...
  public
    ...
    property OnSelectControl: TNotifyEvent read FOnSelectControl
      write FOnSelectControl;
    property OnChangeValue: TChangeValueEvent read FOnChangeValue
      write FOnChangeValue;
    property OnChangeMorphValue: TChangeValueEvent read FOnChangeMorphValue
      write FOnChangeMorphValue;
  end;
  ...
  private
    ...
    FBufferIndex, FBufferSize, FMaxBufferSize: Integer;
    // FMaxY, FMinY : Single;
    FCycles: Integer;
  ...
  public
    ...
    procedure PaintOn(aCanvas: TCanvas); override;
    property BufferingActive: Boolean read FBufferingActive
      write SetBufferingActive;
  end;
  ...
  private
    FMiniVUWidth, FMiniVUHeight: Integer;
  ...
  public
    ...
    procedure MouseDown(Button: TMouseButton; Shift: TShiftState;
      X, Y: Single); override;
    property Orientation: TOrientationType read FOrientation
  ...
  public
    ...
    procedure ProcessMouseDown(Sender: TObject; Shift: TShiftState;
      aTouchID: Integer; X, Y: Single); override;
    procedure ProcessMouseUp(Sender: TObject; Shift: TShiftState;
      aTouchID: Integer; X, Y: Single); override;
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure CalcDimensionsFromBtnSize(aBtnWidth, aBtnHeight: Single);
      virtual; abstract;
    procedure ProcessMouseUp(Sender: TObject; Shift: TShiftState;
      aTouchID: Integer; X, Y: Single); override;
    procedure MouseUp(Button: TMouseButton; Shift: TShiftState;
  ...
  public
    ...
    procedure ParsePanelData(fs: TModuleDefStream); override;
    function ParseProperties(fs: TModuleDefStream; aName: string): Boolean; override;
    property ButtonLabel[index: Integer]: string read GetButtonLabel
      write SetButtonLabel;
  published
  ...
  public
    ...
    property OptionsVisible: Boolean read FOptionsVisible
      write SetOptionsVisible;
  published
    property MouseEnable;
    property CanFocus;
    property TabOrder;
    property HighValue;
    property LowValue;
    property OptionListEnabled: Boolean read FOptionListEnabled
      write SetOptionListEnabled;
  end;
  ...
  private
    ...
    FKnobDownNormValue, FKnobDownNormMorphValue: Single;
    ...
    [Weak]
    FSliderKnobStyles: TG2StateStyleList;
  ...
  published
    ...
    property SliderKnobSize: Single read FSliderKnobSize
      write SetSliderKnobSize;
    property Orientation: TOrientationType read FOrientation
      write SetOrientation;
    property KnobControl: TKnobControl read FKnobControl write FKnobControl;
    property SliderKnobStyles: TG2StateStyleList read GetSliderKnobStyles
      write SetSliderKnobStyles;
  ...
  public
    ...
    procedure PaintOn(aCanvas: TCanvas); override;
    function ParseProperties(fs: TModuleDefStream; aName: string): Boolean; override;
    property ConnectorColor: TAlphaColor read FConnectorColor
      write FConnectorColor;
    property ConnectorKind: TConnectorKind read FConnectorKind
      write FConnectorKind;
    ...
    property OnCreateCable: TCreateCableEvent read FOnCreateCable
      write FOnCreateCable;
  published
  ...
  private
    FWhiteKeyWidth, FWhiteKeyLength, FBlackKeyWidth, FBlackKeyLength,
      FOctaveButtonWidth, FOctaveButtonHeight: Single;
    ...
    FLowKey, FHighKey, FMonoKeyTouchID, FSelectedLowKey,
      FSelectedHighKey: Integer;
    FState: TControlState;
    ...
    procedure SetState(const Value: TControlState); override;
    ...
    procedure SetMouseEnable(const Value: Boolean); override;
  protected
    procedure DoMouseLeave; override;
  public
    constructor Create(AOwner: TComponent); override;
    ...
    procedure Resize; override;
    procedure PaintOn(aCanvas: TCanvas); override;
    procedure GetKeyType(const aKey: Integer; var aOctave, aKeyID: Integer;
      var aKeyType: TKeyType);
  ...
  published
    ...
    property SelectedLowKey: Integer read FSelectedLowKey
      write SetSelectedLowKey;
    property SelectedHighKey: Integer read FSelectedHighKey
      write SetSelectedHighKey;
    property Orientation: TKeyboardOrientation read FOrientation
      write SetOrientation;
    ...
    property HitTest;
    property TabOrder;
    property OnKeyboardKeyDown: TKeyboardKeyDownEvent read FOnKeyboardKeyDown
      write FOnKeyboardKeyDown;
    property OnKeyboardKeyUp: TKeyboardKeyUpEvent read FOnKeyboardKeyUp
      write FOnKeyboardKeyUp;
    property OnOctaveButtonChange: TOctaveButtonChangeEvent
      read FOnOctaveButtonChange write FOnOctaveButtonChange;
  end;
...
begin
  if abs(V.X) < 0.000000001 then
  begin // Close to zero! Declare it to be zero!
  ...
  begin
    if V.X > 0 then
    begin // 1. or 4. quadrant
      Result := ArcTan(V.Y / V.X);
      // Easy stuff. Normal ArcTan is valid for 1. and 4.
    end
    else
    begin // 2. or 3. quadrant
...
begin
  ...
  if TFontStyle.fsBold in aFontStyles then
    Result := 'Bold'
  else if TFontStyle.fsItalic in aFontStyles then
    Result := 'Italic'
  else if TFontStyle.fsUnderline in aFontStyles then
    Result := 'Underline';
end;
...
begin
  ...
  Result := '#' + IntToHex(A, 2) + IntToHex(R, 2) + IntToHex(G, 2) +
    IntToHex(B, 2);
end;
...
begin
  Result := RectF(Position.X, Position.Y, Position.X + Width,
    Position.Y + Height);
end;
...
begin
  ...
  else if (FBitmap.Width <> w) or (FBitmap.Height <> h) then
  begin
    FBitmap.Resize(w, h);
    FBitmap.Clear($000000FF);
...
begin
  ...
  begin
    ...
    try
      ...
      sr := RectF(0, 0, w, h);
      dr := RectF(-ScaledMargin, -ScaledMargin, w - ScaledMargin,
        h - ScaledMargin);
...
begin
  ...
  WriteLog('Resize w:' + IntToStr(trunc(Width)) + ', h:' +
    IntToStr(trunc(Height)));
end;
...
begin
  ...
  try
    ...
    if f = 0 then
      Exit;
    aCanvas.SetMatrix(MatrixMultiply(CreateScaleMatrix(1 / f, 1 / f),
      SaveMatrix));
...
begin
  if (FUnscaledWidth <> Value.Right - Value.Left)
  or (FUnscaledHeight <> Value.Bottom - Value.Top) then
  begin
...
var
  z: Single;
begin
  if Value <> FUnscaledPosition.X then
  begin
...
begin
  ...
  BoundsRect := RectF(
    FUnscaledPosition.X * z,
    FUnscaledPosition.Y * z,
...
var
  z: Single;
begin
  if Value <> FUnscaledPosition.Y then
  begin
...
begin
  ...
  begin
    ...
    begin
      if Children[i] is TBufferedControl then
      begin
...
begin
  if (Operation = opRemove) and (AComponent is TG2StateStyleList) and
    (AComponent <> FDefaultStateStyles) then
    StateStyleList := nil;
...
begin
  if aValue <> FStateStyleList then
  begin
    if assigned(aValue) then
    begin
      if assigned(FStateStyleList) and (FStateStyleList <> FDefaultStateStyles)
      then
...
begin
  if aValue <> FNormMorphValue then
  begin
...
var
  aName, aValue: string;
begin
  while (fs.Position < fs.Size) and (aName <> '#>') do
  begin
    fs.ReadSpaces;
    aName := fs.ReadUntil([':', #13]);
    if aName <> '#>' then
    begin
      if not ParseProperties(fs, aName) then
      begin
        // Unknown property
...
begin
  ...
  else if aName = 'ZPos' then
  begin
    sValue := fs.ReadUntil([#13]);
    FZOrder := StrToInt(string(sValue));
  end
  else if aName = 'InfoFunc' then
  begin
    sValue := fs.ReadUntil([#13]);
    InfoFunc := StrToInt(sValue);
  end
  else if aName = 'ImageIDS' then
  begin
    fs.ReadConst('"');
    FImageIDS.Delimiter := ',';
...
procedure TG2BaseControl.ProcessMouseDown(Sender: TObject; Shift: TShiftState;
...
procedure TG2BaseControl.ProcessMouseMove(Sender: TObject; Shift: TShiftState;
  aTouchID: Integer; X, Y: Single);
begin
  //
end;

procedure TG2BaseControl.ProcessMouseUp(Sender: TObject; Shift: TShiftState;
  aTouchID: Integer; X, Y: Single);
begin
  //
end;
...
begin
  ...
  for i := 0 to FDataDependencies.Count - 1 do
  begin
...
procedure TG2MultiReadControl.AddDataDependency(aData: IG2DataParam;
  aDeassignEvent: TNotifyEvent);
begin
...
begin
  case aG2Event of
    ...
    EvtValueChange:
      begin
        if FDataLed.GetGroupCount > 1 then
        begin
...
begin
  Result := True;
  if not inherited ParseProperties(fs, aName) then
  begin
    if aName = 'CodeRef' then
    begin
      aValue := fs.ReadUntil([#13]);
...
var
  Rect: TRectF;
  i, level: Integer;
  level_green, level_yellow, level_red: Single;
begin
  ...
  if FValue = 0 then
    ...
    level := round(FMiniVUHeight * System.Math.log10(FValue) * 0.45);
  // Something like this...
  i := 0;
  // while (i <= Height) and (i <= FLevel) do begin
  while (i <= Height) and (i <= level) do
  begin
...
function TG2MiniVU.ParseProperties(fs: TModuleDefStream; aName: string): Boolean;
var
  aValue: string;
begin
  Result := True;
...
begin
  ...
  if aValue = 0 then
    ...
    FLevel := round(FMiniVUHeight * System.Math.log10(aValue) * 0.45);
  // Something like this...
...
begin
  ...
  begin
    aCanvas.DrawLine(PointF(sx * (i - 1), ty + sy * FBuffer[i - 1]),
      PointF(sx * i, ty + sy * FBuffer[i]), AbsoluteOpacity);
  end;
  inherited;
end;
...
begin
  ...
  begin
    ...
    if FBufferIndex >= Length(FBuffer) then
      FBufferIndex := 0
    else if FBufferSize < FBufferIndex then
...
begin
  if aValue <> FBufferingActive then
  begin
    if aValue then
    begin
      FMaxBufferSize := 50;
...
begin
  ...
  DrawTextExt(aCanvas, StyleSet.Font, PointF(0, 0),
    PointF(UnscaledBoundsRect.Width, UnscaledBoundsRect.Height),
    StyleSet.FontColor, FLabelText, FTextAlign, TTextAlign.taCenter);
  inherited;
end;
...
function TG2Label.ParseProperties(fs: TModuleDefStream; aName: string): Boolean;
var
  aValue: string;
  StyleSet: TG2StyleSet;
begin
...
begin
  ...
  try
    ...
    begin
      ...
      else if (FDataDependencies.Count > 0) and assigned(FDataDependencies[0])
      then
    ...
    StyleSet.FontColor, TextFunc, TTextAlign.taCenter, TTextAlign.taCenter);
  finally
    StyleSet.Free;
  end;
...
procedure TG2Button.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Single);
begin
  if assigned(OnMouseDown) then
    OnMouseDown(Self, Button, Shift, X, Y)
  else if FMouseEnable then
...
function TG2Button.ParseProperties(fs: TModuleDefStream; aName: string): Boolean;
var
  aValue: string;
begin
  Result := True;
  ...
  begin
    ...
    begin
      ...
      else if aName = 'ButtonCount' then
      begin
        aValue := fs.ReadUntil([#13]);
        FButtonCount := StrToInt(string(aValue));
      end
      else if aName = 'ButtonWidth' then
      begin
        aValue := fs.ReadUntil([#13]);
        ...
        else if aName = 'ImageWidth' then
        begin
          aValue := fs.ReadUntil([#13]);
          FImageWidth := StrToInt(string(aValue));
        end
        else if aName = 'ImageCount' then
        begin
          aValue := fs.ReadUntil([#13]);
...
begin
  ...
  begin
    ...
    if trim(LabelText) <> '' then
      DrawTextExt(aCanvas, StyleSet.Font, PointF(UnscaledBoundsRect.Left,
        UnscaledBoundsRect.Top), PointF(UnscaledBoundsRect.Width,
        UnscaledBoundsRect.Height), StyleSet.FontColor, LabelText,
...
begin
  ...
  if Sender = Self then
  begin
    if FButtonKind = bkMomentary then
    begin
      Value := 1;
      // FBtnSelected := 1;
      Redraw;
...
procedure TG2BtnText.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Single);
begin
...
procedure TG2BtnTextEdit.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Single);
begin
  if assigned(OnMouseUp) then
    OnMouseUp(Self, Button, Shift, X, Y)
...
procedure TG2BtnFlat.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Single);
begin
  if assigned(OnMouseUp) then
    OnMouseUp(Self, Button, Shift, X, Y)
...
var
  ...
  begin
    ...
    try
      ...
      if FImageIDS.Count > 0 then
      begin
        if aIndex < FImageIDS.Count then
        begin
          if assigned(FOnPaintElement) then
      ...
      begin
        ...
        DrawTextExt(aCanvas, StyleSet.Font, PointF(Rect.Left, Rect.Top),
          PointF(Rect.Width, Rect.Height), StyleSet.FontColor, LabelText,
          TTextAlign.taCenter, TTextAlign.taCenter);
      end;
    finally
      StyleSet.Free;
...
begin
  if Length(FRectArray) > 0 then
  begin
    for i := 0 to Length(FRectArray) - 1 do
    begin
      if i <> FBtnSelected then
        PaintButton(i);
    end;
    if (FBtnSelected <> -1) and (FBtnSelected < Length(FRectArray)) then
    begin
...
begin
  ...
  begin
    i := 0;
    while (i < FButtonCount) and
      not(PtInRect(FRectArray[i], PointF(X { /Zoom } , Y { /Zoom } ))) do
      inc(i);
    if (i < FButtonCount) then
    begin
      FBtnSelected := i;
      Value := i;
      SetValueDataDependencies(FValue);
...
procedure TG2BtnArray.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Single);
begin
  if assigned(OnMouseUp) then
    OnMouseUp(Self, Button, Shift, X, Y)
...
begin
  ...
  if Orientation = otHorizontal then
  begin
    UnscaledBoundsRect := RectF(0, 0, (ButtonWidth - FBorderWidth) * ButtonCount
      + FBorderWidth, ButtonHeight);
  end
  else
  begin
    UnscaledBoundsRect := RectF(0, 0, ButtonWidth, (ButtonHeight - FBorderWidth)
      * ButtonCount + FBorderWidth);
  end;
...
begin
  ...
  begin
    ...
    FUnscaledHeight := (ButtonHeight - FBorderWidth) * ButtonCount +
      FBorderWidth;
...
begin
  inherited;
  UnscaledBoundsRect := RectF(0, 0, (ButtonWidth - FBorderWidth) * ButtonColumns
    + FBorderWidth, (ButtonHeight - FBorderWidth) * ButtonRows + FBorderWidth);
  CalcDimensions;
end;
...
begin
  ...
  if not inherited ParseProperties(fs, aName) then
  begin
    if aName = 'ButtonColumns' then
    begin
      aValue := fs.ReadUntil([#13]);
      FButtonColumns := StrToInt(string(aValue));
    end
    else if aName = 'ButtonRows' then
    begin
      aValue := fs.ReadUntil([#13]);
...
begin
  ...
  i := 0;
  for R := 0 to FButtonRows - 1 do
  begin
    Rect.Left := 0;
    Rect.Right := FButtonWidth;
    for c := 0 to FButtonColumns - 1 do
    begin
      FRectArray[i] := Rect;
      inc(i);
      Rect.Left := Rect.Left + FButtonWidth - FBorderWidth;
...
procedure TG2BtnIncDec.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Single);
begin
  if assigned(OnMouseDown) then
    OnMouseDown(Self, Button, Shift, X, Y)
...
procedure TG2BtnIncDec.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Single);
begin
  if assigned(OnMouseUp) then
    OnMouseUp(Self, Button, Shift, X, Y)
...
begin
  ...
  begin
    i := 0;
    while (i < Length(FRectArray)) and
      not(PtInRect(FRectArray[i], PointF(X { /Zoom } , Y { /Zoom } ))) do
      inc(i);
    if (i < Length(FRectArray)) then
    begin
      BtnSelected := i;
    end
    else
...
begin
  ...
  begin
    ...
    StyleSet.ApplyStyleSet(aCanvas);
    aCanvas.FillRect(Rect, 0, 0, [], AbsoluteOpacity);
    if FPartSelector.FImageIDS.Count > 0 then
    begin
      if i < FPartSelector.FImageIDS.Count then
      begin
        if assigned(FOnPaintElement) then
...
begin
  i := 0;
  while (i < Length(FRectArray)) and
    not(PtInRect(FRectArray[i], PointF(X, Y))) do
    inc(i);

  if (i < Length(FRectArray)) then
  begin
    if (FOptionSelected <> i) then
    begin
      FOptionSelected := i;
      Redraw;
    end;
...
procedure TG2PartSelector.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Single);
begin
  if assigned(OnMouseUp) then
    OnMouseUp(Self, Button, Shift, X, Y)
...
begin
  if FOptionListEnabled <> Value then
  begin
...
begin
  if FOptionsVisible <> aValue then
  begin
    FOptionList.Visible := aValue;

    if aValue then
    begin
      if (Parent is TG2Module) and assigned(Parent.Parent) then
      begin
      ...
      FOptionList.SetBoundsRect(RectF(ModuleLeft + UnscaledLeft,
        ModuleTop + UnscaledTop, ModuleLeft + UnscaledLeft + FImageWidth,
        // System.Math.Max(UnscaledWidth, FImageWidth),
...
begin
  ...
  if FImageIDS.Count > 0 then
  begin
    if (Value >= 0) and (Value < FImageIDS.Count) then
    begin
  ...
  begin
    ...
    DrawTextExt(aCanvas, StyleSet.Font, PointF(FDisplayRect.Left,
      FDisplayRect.Top), PointF(FDisplayRect.Width, FDisplayRect.Height),
      StyleSet.FontColor, LabelText, TTextAlign.taCenter, TTextAlign.taCenter);
  end;
...
begin
  ...
  Rect := RectF(0, UnscaledHeight, FImageWidth, UnscaledHeight + ImageHeight);
  for i := 0 to Length(FOptionList.FRectArray) - 1 do
  begin
    FOptionList.FRectArray[i] := Rect;
    Rect.Top := Rect.Top + ImageHeight;
    Rect.Bottom := Rect.Bottom + ImageHeight;
...
begin
  ...
  FKnobReset.Stored := False;
  // Otherwise there's a "argument out of range" error when droppen on a frame
  ...
  FKnobIncDecBtns.Stored := False;
  // Otherwise there's a "argument out of range" error when droppen on a frame
  FKnobIncDecBtns.Locked := True;
  FKnobIncDecBtns.HitTest := False;
  FKnobIncDecBtns.UpsideDown := (FOrientation = otVertical) and
    (FKnobType in [ktSlider, ktSeqSlider]);
...
begin
  inherited;
  if (FKnobType <> ktSlider) and (FKnobType <> ktSeqSlider) then
  begin
...
procedure TG2Knob.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Single);
begin
  if assigned(OnMouseDown) then
    OnMouseDown(Self, Button, Shift, X, Y)
...
procedure TG2Knob.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Single);
begin
  if assigned(OnMouseUp) then
    OnMouseUp(Self, Button, Shift, X, Y)
...
begin
  ...
  begin
    FKnobDownPart := kdpNone;
    if assigned(FKnobReset) and FKnobReset.Visible and
      PtInRect(FKnobReset.UnscaledRect, PointF(X { /Zoom } , Y { /Zoom } )) then
    begin
    ...
    else if assigned(FKnobIncDecBtns) and PtInRect(FKnobIncDecBtns.UnscaledRect,
      PointF(X { /Zoom } , Y { /Zoom } )) then
    begin
      FKnobDownPart := kdpBtnIncDec;
      FKnobIncDecBtns.ProcessMouseDown(FKnobIncDecBtns, Shift, aTouchID,
        X - FKnobIncDecBtns.Position.X, Y - FKnobIncDecBtns.Position.Y);
    end
    else if (FKnobType = ktSlider) or (FKnobType = ktSeqSlider) then
    begin
  ...
  begin
    ...
    begin
      case FKnobControl of
        kcCircular:
          begin
            if ((X - FCX) * (X - FCX) + (Y - FCY) * (Y - FCY)) > FR * FR then
            begin
              c := PointF(FCX, FCY);
              angle := ArcTan2(PointF(X - c.X, Y - c.Y)) - PI / 2;
              if angle < 0 then
                angle := angle + 2 * PI;
              if FKnobDownMorph then
              begin
                d := (angle / (2 * PI) - 0.1) / 0.8 -
                  (NormValue + NormMorphValue);
                if abs(d) > 0.75 then
                  Exit;
                if NormMorphValue + d > 1 then
                  NormMorphValue := 1
                else if NormMorphValue + d < -1 then
                  NormMorphValue := -1
                else
          ...
          begin
            ...
            begin
              ...
              if FKnobDownNormMorphValue + d > 1 then
                NormMorphValue := 1
              else if FKnobDownNormMorphValue + d < -1 then
                NormMorphValue := -1
              else
            ...
            begin
              ...
              if FKnobDownNormValue + d > 1 then
                NormValue := 1
              else if FKnobDownNormValue + d < 0 then
                NormValue := 0
              else
    ...
    begin
      ...
      begin
        ...
        begin
          if (FSliderRect.Bottom - FSliderKnobRect.Bottom) <> 0 then
          begin
            NewValue := 1 - (Y - (FSliderKnobRect.Bottom - FSliderKnobRect.Top)
              / 2 - c.Y) / (FSliderRect.Bottom - FSliderKnobRect.Bottom);
            NormMorphValue :=
              System.Math.Max(System.Math.Min(1, NewValue - NormValue), -1);
          end;
        end
        else
        begin
          if (FSliderRect.Right - FSliderKnobRect.Right) <> 0 then
          begin
            NewValue := (X - (FSliderKnobRect.Right - FSliderKnobRect.Left) / 2
              - c.X) / (FSliderRect.Right - FSliderKnobRect.Right);
            NormMorphValue :=
              System.Math.Max(System.Math.Min(1, NewValue - NormValue), -1);
          end;
        end;
...
begin
  ...
  begin
    ...
    else if assigned(FKnobIncDecBtns) then
    begin
      FKnobIncDecBtns.LowValue := LowValue;
      FKnobIncDecBtns.HighValue := HighValue;
      FKnobIncDecBtns.Value := Value;
      FKnobIncDecBtns.ProcessMouseUp(FKnobIncDecBtns, Shift, aTouchID,
        X - FKnobIncDecBtns.Position.X, Y - FKnobIncDecBtns.Position.Y);
...
begin
  ...
  begin
    ...
    Result := RectF(0, range - f * range, FSliderKnobRect.Right,
      range - f * range + h);
  end
  else
  begin
...
var
  angle, StartAngle, SweepAngle: Single;
  f, KnobSize: Single;
  PathData: TPathData;
  StyleSet: TG2StyleSet;
  MorphRect, R: TRectF;
begin
  ...
  begin
    ...
    StyleSet := FStateStyleList.StateStyle[FState];
  if (FKnobType = ktSlider) or (FKnobType = ktSeqSlider) then
  begin
    ...
    begin
      ...
      begin
        ...
        MorphRect.Top := FSliderRect.Bottom - (FSliderRect.Bottom - KnobSize) *
          f - KnobSize / 2;
        MorphRect.Bottom :=
          System.Math.Max(MorphRect.Top - (FSliderRect.Bottom - KnobSize) *
          NormMorphValue, FSliderRect.Top);
      end
      else
      begin
        KnobSize := FSliderKnobRect.Width;
        MorphRect.Left := (FSliderRect.Right - KnobSize) * f + KnobSize / 2;
        MorphRect.Right := System.Math.Min
          (MorphRect.Left + (FSliderRect.Right - KnobSize) * NormMorphValue,
          FSliderRect.Right);
      end;
      aCanvas.Fill.Color := StyleSet.ActiveMorphColor;
  ...
  begin
    ...
    if assigned(FOnPaintElement) then
      FOnPaintElement(Self, ceBackGround, 0, StyleSet);
    StyleSet.ApplyStyleSet(aCanvas);
    aCanvas.FillEllipse(RectF(FCX - FR, FCY - FR, FCX + FR, FCY + FR),
      AbsoluteOpacity);
    aCanvas.DrawEllipse(RectF(FCX - FR, FCY - FR, FCX + FR, FCY + FR),
      AbsoluteOpacity);
    ...
    begin
      ...
      try
        ...
        PathData.AddArc(PointF(FCX, FCY), PointF(FR, FR), StartAngle + 90,
          SweepAngle);
        PathData.ClosePath;
    ...
    aCanvas.DrawLine(PointF(FCX, FCY), PointF(FCX - sin(angle) * FR,
      FCY + cos(angle) * FR), AbsoluteOpacity);
...
function TG2Knob.ParseProperties(fs: TModuleDefStream; aName: string): Boolean;
var
  aValue: string;
begin
  Result := True;
  ...
  begin
    ...
    begin
      ...
      begin
        aValue := fs.ReadUntil([#13]);
        if aValue = '"Reset"' then
        begin
          FKnobType := ktReset;
        end;
        if aValue = '"Reset/medium"' then
        begin
          FKnobType := ktResetMedium;
        end;
        if aValue = '"Small"' then
        begin
          FKnobType := ktSmall;
        end;
        ...
        if aValue = '"Slider"' then
        begin
          FKnobType := ktSlider;
        end;
        if aValue = '"SeqSlider"' then
        begin
...
procedure TG2Knob.CalcDimensions;
  ...
  begin
    ...
    begin
      ...
      FKnobIncDecBtns.FImageIDS.Add(IntToStr(ord(gsArrowUp)));
      FKnobIncDecBtns.FImageIDS.Add(IntToStr(ord(gsArrowDown)));
      FKnobIncDecBtns.UnscaledRect := RectF(0, FSliderRect.Bottom + 1,
        FSliderRect.Right, FUnscaledHeight { FSliderRect.Bottom + 17 } );
    end
    else
    begin
      FKnobIncDecBtns.Orientation := otHorizontal;
      ...
      FKnobIncDecBtns.UnscaledRect := RectF(FSliderRect.Right + 1, 0,
        FUnscaledWidth { FSliderRect.Right + 17 } , FSliderRect.Bottom);
    end;
    FKnobIncDecBtns.OnMouseMove := OnMouseMove;
    FKnobIncDecBtns.Visible := True;
  end;
  ...
  case FKnobType of
    ktReset: // Width 18, Height 26, Radius 9, Reset button: Width 10, Height 4
      begin
        ...
        begin
          FKnobReset.UnscaledRect := RectF(FR - FR * 10 / 18, 0,
            FR + FR * 10 / 18, FR * 4 / 9);
        ...
        begin
          UpdateKnobBtns;
          FKnobIncDecBtns.UnscaledRect := RectF(FR - FR * 10.5 / 9,
            UnscaledHeight - FR * 9 / 9, FR + FR * 10.5 / 9, UnscaledHeight);
        end;
      end;
    ktResetMedium:
    // Width 20, Height 30, Radius 10, Reset button: Width 10, Height 4
      begin
        FR := FUnscaledWidth / 2;
        FCX := FR;
        FCY := FR + FR * 4 / 10 + 1;
        if assigned(FKnobReset) then
        begin
          FKnobReset.UnscaledRect := RectF(FR - FR * 10 / 20, 0,
            FR + FR * 10 / 20, FR * 4 / 10);
          FKnobReset.OnMouseMove := OnMouseMove;
          FKnobReset.Visible := True;
        end;
        ...
        begin
          UpdateKnobBtns;
          FKnobIncDecBtns.UnscaledRect := RectF(FR - FR * 10.5 / 10,
            UnscaledHeight - FR * 9 / 10, FR + FR * 10.5 / 10, UnscaledHeight);
        end;
      end;
    ...
    ktBig: // Width 22, Height 26, Radius 11
      begin
        ...
        begin
          UpdateKnobBtns;
          FKnobIncDecBtns.UnscaledRect := RectF(FR - FR * 11 / 11,
            UnscaledHeight - FR * 9 / 11, FR + FR * 11 / 11, UnscaledHeight);
        end;
      end;
    ktSlider: // width 11, height 45, buttons 17
      begin
        if FOrientation = otHorizontal then
        begin
          FSliderRect := RectF(0, 0, 45 * UnscaledWidth / (45 + 17),
            UnscaledHeight);
          FSliderKnobRect := RectF(0, 0, FSliderKnobSize, UnscaledHeight);
        end
        else
        begin
          FSliderRect := RectF(0, 0, UnscaledWidth, 45 * UnscaledHeight /
            (45 + 17));
    ...
    ktSeqSlider: // width 11, height ?, buttons 17
      begin
        if FOrientation = otHorizontal then
        begin
          FSliderRect := RectF(0, 0, 62 * UnscaledWidth / (62 + 17),
            UnscaledHeight);
          FSliderKnobRect := RectF(0, 0, FSliderKnobSize, UnscaledHeight);
        end
        else
        begin
          FSliderRect := RectF(0, 0, UnscaledWidth, 62 * UnscaledHeight /
            (62 + 17));
          FSliderKnobRect := RectF(0, 0, UnscaledWidth, FSliderKnobSize);
        end;
        if assigned(FKnobIncDecBtns) then
...
begin
  if FSliderKnobSize <> aValue then
  begin
...
begin
  if FSliderKnobStyles <> aValue then
  begin
    if assigned(aValue) then
...
begin
  ...
  if assigned(FOnPaintElement) then
    FOnPaintElement(Self, ceBackGround, 0, StyleSet);
  StyleSet.ApplyStyleSet(aCanvas);
  if FConnectorKind = ckInput then
  begin
...
begin
  FRedraw := True;
  d := 1;
  if FEndNode or FStartNode then
  begin
    Margin := 4;
  end
  else
  ...
  P4.X := X - N.X * d;
  P4.Y := Y - N.Y * d;
  BB.Left := System.Math.Min(System.Math.Min(System.Math.Min(P1.X, P2.X), P3.X),
    P4.X) - Margin;
  BB.Top := System.Math.Min(System.Math.Min(System.Math.Min(P1.Y, P2.Y), P3.Y),
    P4.Y) - Margin;
  BB.Right := System.Math.Max(System.Math.Max(System.Math.Max(P1.X, P2.X),
    P3.X), P4.X) + Margin;
  BB.Bottom := System.Math.Max(System.Math.Max(System.Math.Max(P1.Y, P2.Y),
    P3.Y), P4.Y) + Margin;
  // SetBoundsRect(BB);
  // UnscaledRect := BB;
  UnscaledRect := RectF(BB.Left - FCable.Position.X, BB.Top - FCable.Position.Y,
    BB.Right - FCable.Position.X, BB.Bottom - FCable.Position.Y);
  if FCable.CableStyle = csGradient then
  begin
    G1.X := P1.X + (P2.X - P1.X) / 2;
    G1.Y := P1.Y + (P2.Y - P1.Y) / 2;
    G2.X := P4.X + (P3.X - P4.X) / 2;
    G2.Y := P4.Y + (P3.Y - P4.Y) / 2;
    if ((BB.Right - BB.Left) <> 0) and ((BB.Bottom - BB.Top) <> 0) then
    begin
...
begin
  ...
  l := sqrt((max_x - min_x) * (max_x - min_x) + (max_y - min_y) *
    (max_y - min_y));
  N := FNodeCount - 1;
...
var
  l, min_x, min_y, max_x, max_y: Single;
begin
  min_x := System.Math.Min(FP1.X, FP2.X);
  max_x := System.Math.Max(FP1.X, FP2.X);
  min_y := System.Math.Min(FP1.Y, FP2.Y);
...
begin
  ...
  begin
    if N > 0 then
    begin
      FNodes[N].vx := FNodes[N].vx +
        (FNodes[N].x2 + FNodes[N - 1].X - FNodes[N].X * 2) / TENSION;
      FNodes[N].vy := FNodes[N].vy +
        (FNodes[N].y2 + FNodes[N - 1].Y - FNodes[N].Y * 2) / TENSION;
    end
    else
    begin
      FNodes[N].vx := FNodes[N].vx + (FNodes[N].x2 + FNodes[N].X - FNodes[N].X *
        2) / TENSION;
      FNodes[N].vy := FNodes[N].vy + (FNodes[N].y2 + FNodes[N].Y - FNodes[N].Y *
        2) / TENSION;
    end;
...
begin
  ...
  FSelected := False;
  UnscaledRect := RectF(0, 0, UNITS_COL + UNIT_MARGIN * 2,
...
begin
  ...
  aCanvas.DrawRect(Rect, 0, 0, [], AbsoluteOpacity);
  DrawTextExt(aCanvas, StyleSet.Font, PointF(Rect.Left, Rect.Top),
    PointF(Rect.Width, Rect.Height), StyleSet.FontColor, FModuleLabel,
...
begin
  ...
  if FOrientation = kbHorizontal then
  begin
...
begin
  ...
  case t of
    ...
    ktBlack:
      begin
        d := FWhiteKeyWidth - FBlackKeyWidth / 2 + (k - lk) * FWhiteKeyWidth +
          (o - lo) * (FWhiteKeyWidth * 7);
        if FOrientation = kbHorizontal then
        begin
          Result.Top := FOctaveButtonHeight;
          Result.Left := d;
...
procedure TG2KeyBoard.GetKeyType(const aKey: Integer;
  var aOctave, aKeyID: Integer; var aKeyType: TKeyType);
var
  OctaveKey: Integer;
begin
  aOctave := aKey div 12;
...
var
  i: Integer;
begin
  for i := 0 to High(FKeyTouchID) do
  begin
    FKeyTouchID[i] := -1;
    FKeyUpdate[i] := 1;
  end;
...
var
  i: Integer;
begin
  for i := 0 to High(FOctaveButtonValue) do
  begin
...
procedure TG2KeyBoard.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Single);
begin
  if assigned(OnMouseDown) then
    OnMouseUp(Self, Button, Shift, X, Y)
...
procedure TG2KeyBoard.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Single);
begin
  if assigned(OnMouseUp) then
    OnMouseUp(Self, Button, Shift, X, Y)
...
begin
  ...
  if FRangeSelect and (aKey >= FSelectedLowKey) and (aKey <= FSelectedHighKey)
  then
    StyleSet := FStateStyleList.StateStyle[csSelected]
  else
    StyleSet := FStateStyleList.StateStyle[FState];
  ...
  case t of
    ktWhite:
      begin
        ...
        try
          {$IFDEF Ver260Up}
          aCanvas.SetMatrix(SaveMatrix * TMatrix.CreateTranslation
            (R.Left, R.Top));
          {$ELSE}
          aCanvas.SetMatrix(MatrixMultiply(SaveMatrix,
            CreateTranslateMatrix(R.Left, R.Top)));
         {$ENDIF}
          case k of
            0, 3:
              begin
...
procedure TG2KeyBoard.PaintOctaveButton(aCanvas: TCanvas;
  const aOctave: Integer);
var
  R: TRectF;
  StyleSet: TG2StyleSet;
begin
  ...
  aCanvas.FillRect(R, 0, 0, [], AbsoluteOpacity);
  aCanvas.DrawRect(R, 0, 0, [], AbsoluteOpacity);
  DrawTextExt(aCanvas, StyleSet.Font, PointF(R.Left, R.Top),
    PointF(R.Width, R.Height), StyleSet.FontColor, IntToStr(aOctave),
    TTextAlign.taCenter, TTextAlign.taCenter);
end;
...
begin
  ...
  begin
    if (KeyTypes[i mod 12].KeyType = ktWhite) then
    begin
      PaintKey(aCanvas, i);
    end;
  end;
  for i := FLowKey to FHighKey do
  begin
    if (KeyTypes[i mod 12].KeyType = ktBlack) then
    begin
      PaintKey(aCanvas, i);
    end;
  end;
...
begin
  ...
  if FMonoKey and (FMonoKeyTouchID <> -1) then
    Exit;
  i := FLowKey;
  while (i <= FHighKey) and not((KeyTypes[i mod 12].KeyType = ktBlack) and
    PtInRect(GetKeyRect(i), PointF(X, Y))) do
    inc(i);
  if not(i <= FHighKey) then
  begin
    i := FLowKey;
    while (i <= FHighKey) and not((KeyTypes[i mod 12].KeyType = ktWhite) and
      PtInRect(GetKeyRect(i), PointF(X, Y))) do
      inc(i);
  end;
  if (i <= FHighKey) then
  begin
    ...
    begin
      ...
      begin
        for i := FLowKey to FHighKey do
        begin
          if (i <> Key) and (FKeyTouchID[i] <> -1) then
          begin
            FKeyTouchID[i] := -1;
            FKeyUpdate[i] := 1;
            Redraw;
...
begin
  ...
  if FMonoKey then
  begin
    if aTouchID = FMonoKeyTouchID then
    begin
      i := FLowKey;
...
begin
  if FSelectedHighKey <> Value then
  begin
...
function TG2StateStyleList.AddStateStyle(aControlState: TControlState)
  : TG2StyleSet;
var
  i: Integer;
begin
...
function TG2StateStyleList.GetStateStyle(aControlState: TControlState)
  : TG2StyleSet;
var
  i: Integer;
begin
  ...
  if (i < FItems.Count) then
    ...
    raise Exception.Create('Styleset ' + IntToStr(Integer(aControlState)) +
      ' not found.');
end;
...
RegisterFmxClasses([TG2BufferedLayout, TArrayLayout, TG2Scope, TG2LedGreen,
  TG2TextField, TG2BtnText, TG2BtnTextEdit, TG2BtnFlat, TG2BtnRadio,
  TG2BtnRadioEdit, TG2BtnIncDec, TG2ResetButton, TG2PartSelectorList,
  TG2PartSelector, TG2Knob, TG2Connector, TG2CableNode, TG2Cable, TG2KeyBoard,
  TG2Module]);
