...
type
  ...
  public
    ...
    procedure ServerBroadcastResponseMessage(ResponseMessage
      : TG2ResponseMessage);
    procedure ServerBroadcastSendMessage(ClientSendMessage: TClientSendMessage);
    function ServerProcessClientResponselessMessage(ClientMessage
      : TClientSendMessage): Boolean;
    procedure ClientProcessServerSendMessage(MemStream: TMemoryStream;
      SenderID: Integer);
  ...
  published
    ...
    property TimerBroadcastLedMessages: Integer read FTimerBroadcastLedMessages
      write FTimerBroadcastLedMessages default 500;
    property OnAddClient: TOnAddClient read FOnAddClient write FOnAddClient;
    property OnDeleteClient: TOnDeleteClient read FOnDeleteClient
      write FOnDeleteClient;
  end;
  ...
  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    procedure SendSetParamMessage(aLocation, aModule, aParam, aValue,
      aVariation: byte); override;
    procedure SendSelParamMessage(aLocation, aModule, aParam: Integer); override;
    procedure SendSetMorphMessage(aLocation, aModule, aParam, aMorph, aValue,
      aNegative, aVariation: byte); override;
  end;
...
begin
  if FIsServer then
    Result := inherited
  else if assigned(FIdTCPClient) then
...
begin
  ...
  begin
    ...
    begin
      ...
      FTCPErrorMessage := '';
      try
        add_log_line('Connect to server, ip = ' + FHost + ', port = ' +
          IntToStr(FPort), LOGCMD_NUL);
  ...
  begin
    ...
    // Disconnect
    inherited;
    if (not FIsServer) and assigned(FIdTCPClient) and FIdTCPClient.Connected
    then
    begin
...
begin
  ...
  if not((ClientSendMessage.FClientContext = nil) and
    not(ClientSendMessage.HasResponse)) then
  begin
    // inherited;
...
begin
  ...
  begin
    ...
    add_log_line('Send message, size = ' + IntToStr(aSendMessage.Size),
      LOGCMD_NUL);
...
begin
  ...
  add_log_line('Client connect : ' + AContext.Binding.PeerIP + ', port : ' +
    IntToStr(AContext.Binding.PeerPort) +
    ', remote host : ' { + GStack.WSGetHostByAddr(AContext.Binding.PeerIP) }
    + ', type : ' + IntToStr(ord(LClientContext.Client.ClientType)),
    LOGCMD_NUL); // <-- here
...
begin
  ...
  Move(ClientSendMessage.Memory^, PStaticByteBuffer(LBuffer.Memory)
    ^[szProtocol], LProtocol.DataSize);
  Lock;
  try
    // write the buffer
...
begin
  ...
  LBuffer.Size := szProtocol + LProtocol.DataSize;
  // move message to buffer
  Move(MemStream.Memory^, PStaticByteBuffer(LBuffer.Memory)^[szProtocol],
    LProtocol.DataSize);
  Lock;
  try
    // write the buffer
...
var
  ...
  LProtocol: TProtocol;
  LClientContext: TClientContext;
  // we need to HARD CAST AContext to TClientContext in order to access our custom methods(procedures)
  ...
  begin
    ...
    try
      ...
      case LProtocol.Command of
        cmdConnect:
          begin
            add_log_line('<connected> : ' + IntToStr(LProtocol.Sender.ID),
              LOGCMD_NUL);
            LClientContext.Client := LProtocol.Sender;
          end;
        cmdDisconnect:
          begin
            add_log_line('<disconnected> %s' + IntToStr(LProtocol.Sender.ID),
              LOGCMD_NUL);
          end;
        cmdSendMessage:
          begin
            ...
            LClientContext.Connection.IOHandler.ReadStream(MyNotify.FBuffer,
              LProtocol.DataSize);
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if (LClientContext.Client.ClientType <> ctVST) and
          (LClientContext.Initialized) and
          (TThread.GetTickCount - Cardinal(LClientContext.LastWrite) >
          Cardinal(FTimerBroadcastLedMessages)) then
        begin
          LClientContext.Lock;
          try
...
begin
  ...
  try
    ...
    begin
      LClientContext := TClientContext(LClients[i]);
      SenderIsThisClient := (ClientSendMessage.FClientContext <> nil) and
        (LClientContext.FClient.ID = ClientSendMessage.FClientContext.
        FClient.ID);
      // Don't send responseless messages back to sender
      if not(SenderIsThisClient and not(ClientSendMessage.HasResponse)) then
      begin
        LClientContext.Lock;
        try
...
begin
  ...
  if not FG2.idTCPClient.IOHandler.InputBufferIsEmpty then
  begin
    LBuffer := TMemoryStream.Create;
    try
        ...
        case LProtocol.Command of
          cmdSendMessage:
            begin
              ...
              FG2.Lock;
              try
                FG2.idTCPClient.IOHandler.ReadStream(LMessage,
                  LProtocol.DataSize);
              finally
                FG2.Unlock;
              end;
              ...
              case LProtocol.MessageType of
                mdtSendMessage:
                  DoClientProcessServerSendMessage;
                mdtResponseMessage:
                  DoClientProcessServerResponseMessage;
              end;
              {$ELSE}
              case LProtocol.MessageType of
                mdtSendMessage:
                  Synchronize(DoClientProcessServerSendMessage);
                mdtResponseMessage:
                  Synchronize(DoClientProcessServerResponseMessage);
              end;
              {$ENDIF}
            end;
        end;
...
begin
  ...
  if PStaticByteBuffer(MemStream.Memory)^[0] = CMD_INIT then
    Cmd := CMD_INIT
  else if (PStaticByteBuffer(MemStream.Memory)^[0] and $F) = 2 then
  begin
  ...
  try
    if not ProcessResponseMessage(MemStream, 0) then
      add_log_line(DateTimeToStr(Now) + ' Response message not processed.',
        LOGCMD_ERR);
  except
    on E: Exception do
  ...
  begin
    ...
    begin
      ...
      else if assigned(GetSlot(1)) and assigned(GetSlot(1).OnNextInitStep) then
        GetSlot(1).OnNextInitStep(self)
      else if assigned(GetSlot(2)) and assigned(GetSlot(2).OnNextInitStep) then
        GetSlot(2).OnNextInitStep(self)
      else if assigned(GetSlot(3)) and assigned(GetSlot(3).OnNextInitStep) then
        GetSlot(3).OnNextInitStep(self)
      else if assigned(Performance) and assigned(GetPerformance.OnNextInitStep)
      then
        GetPerformance.OnNextInitStep(self)
      else if assigned(OnNextInitStep) then
        OnNextInitStep(self);
    end;
  end;
end;

procedure TG2TCPIP.ClientProcessServerSendMessage(MemStream: TMemoryStream;
  SenderID: Integer);
begin
  ...
  try
    if not ProcessSendMessage(MemStream, SenderID) then
      add_log_line(DateTimeToStr(Now) + ' Send message not processed.',
        LOGCMD_ERR);
  except
    on E: Exception do
      add_log_line(DateTimeToStr(Now) + E.Message, LOGCMD_ERR);
...
begin
  ...
  try
    LBuffer.Size := szProtocol + LProtocol.DataSize;
    Move(PStaticByteBuffer(MemStream.Memory)^[0],
      PStaticByteBuffer(LBuffer.Memory)^[szProtocol], LProtocol.DataSize);
    FIdTCPClient.IOHandler.Write(LBuffer);
  finally
    LBuffer.Free;
    Unlock;
...
begin
  ...
  ResponseMessage := CreateResponseMessage(ClientMessage,
    ClientMessage.FClientContext.FInitialized);
  if ResponseMessage <> nil then
  begin
    try
      ResponseMessage.CalcCRC;
      ClientMessage.FClientContext.ServerSendClientResponseMessage
        (ResponseMessage);
    finally
...
var
  Cmd, SubCmd, aVariation, aLocation, aModuleIndex, aParameterIndex, aValue,
    aMorph, aNegative, b, bh, bl: byte;
  Size: Integer;
begin
  Result := False;
  try
    // ?
    if (ClientMessage.Size - ClientMessage.Position) < 6 then
    begin
        ...
        case SubCmd of
          S_SET_PARAM:
            begin
              ...
              case Cmd and $0F of
                $08:
                  (GetSlot(0) as TG2TCPIPSlot).AddParamUpdRec(SubCmd, aLocation,
                    aModuleIndex, aParameterIndex, 0, aValue, 0, aVariation);
                $09:
                  (GetSlot(1) as TG2TCPIPSlot).AddParamUpdRec(SubCmd, aLocation,
                    aModuleIndex, aParameterIndex, 0, aValue, 0, aVariation);
                $0A:
                  (GetSlot(2) as TG2TCPIPSlot).AddParamUpdRec(SubCmd, aLocation,
                    aModuleIndex, aParameterIndex, 0, aValue, 0, aVariation);
                $0B:
                  (GetSlot(3) as TG2TCPIPSlot).AddParamUpdRec(SubCmd, aLocation,
                    aModuleIndex, aParameterIndex, 0, aValue, 0, aVariation);
              end;
              Result := True;
            end;
          ...
          S_SEL_PARAM:
            begin
              ...
              case Cmd and $0F of
                $08:
                  (GetSlot(0) as TG2TCPIPSlot).AddParamUpdRec(SubCmd, aLocation,
                    aModuleIndex, aParameterIndex, 0, 0, 0, 0);
                $09:
                  (GetSlot(1) as TG2TCPIPSlot).AddParamUpdRec(SubCmd, aLocation,
                    aModuleIndex, aParameterIndex, 0, 0, 0, 0);
                $0A:
          ...
          S_SET_MORPH_RANGE:
            begin
              ...
              case Cmd and $0F of
                $08:
                  (GetSlot(0) as TG2TCPIPSlot).AddParamUpdRec(SubCmd, aLocation,
                    aModuleIndex, aParameterIndex, aMorph, aValue, aNegative,
                    aVariation);
                $09:
                  (GetSlot(1) as TG2TCPIPSlot).AddParamUpdRec(SubCmd, aLocation,
                    aModuleIndex, aParameterIndex, aMorph, aValue, aNegative,
...
function TG2TCPIPPerformance.CreateSlot:
  TG2FileSlot;
begin
...
begin
  inherited;
  if (G2 as TG2TCPIP).IsServer then
  begin
...
procedure TG2TCPIPSlot.SendSetParamMessage(aLocation, aModule, aParam,
  aValue, aVariation: byte);
var
  SendStream: TG2SendMessage;
begin
  if AddParamUpdRec(S_SET_PARAM, aLocation, aModule, aParam, 0,
    aValue, 0, aVariation) then
  begin
    SendStream := CreateSetParamMessage(aLocation, aModule, aParam,
      aValue, aVariation);
    try
...
procedure TG2TCPIPSlot.SendSelParamMessage(aLocation, aModule,
  aParam: Integer);
var
  SendStream: TG2SendMessage;
begin
  if AddParamUpdRec(S_SEL_PARAM, aLocation, aModule, aParam, 0, 0,
    0, 0) then
  begin
...
procedure TG2TCPIPSlot.SendSetMorphMessage(aLocation, aModule, aParam,
  aMorph, aValue, aNegative, aVariation: byte);
var
  SendStream: TG2SendMessage;
begin
  if AddParamUpdRec(S_SET_MORPH_RANGE, aLocation, aModule, aParam,
    aMorph, aValue, aNegative, aVariation) then
  begin
    SendStream := CreateSetMorphMessage(aLocation, aModule, aParam,
      aMorph, aValue, aNegative, aVariation);
    try
      if not(G2 as TG2TCPIP).IsServer then
        (G2 as TG2TCPIP).SendOnClientMessage(SendStream)
      else
